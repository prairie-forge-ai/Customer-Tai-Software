...
(()=>{function W(){return typeof Excel!="undefined"&&typeof Excel.run=="function"}var Ke="SS_PF_Config";async function Ct(e,t=[Ke]){var o;let n=e.workbook.tables;n.load("items/name"),await e.sync();let a=(o=n.items)==null?void 0:o.find(s=>t.includes(s.name));return a?e.workbook.tables.getItem(a.name):(console.warn("Config table not found. Looking for:",t),null)}function Et(e){let t=e.map(n=>String(n||"").trim().toLowerCase());return{field:t.findIndex(n=>n==="field"||n==="field name"||n==="setting"),value:t.findIndex(n=>n==="value"||n==="setting value"),type:t.findIndex(n=>n==="type"||n==="category"),title:t.findIndex(n=>n==="title"||n==="display name"),permanent:t.findIndex(n=>n==="permanent"||n==="persist")}}async function Qe(e=[Ke]){if(!W())return{};try{return await Excel.run(async t=>{let n=await Ct(t,e);if(!n)return{};let a=n.getDataBodyRange(),o=n.getHeaderRowRange();a.load("values"),o.load("values"),await t.sync();let s=o.values[0]||[],l=Et(s);if(l.field===-1||l.value===-1)return console.warn("Config table missing FIELD or VALUE columns. Headers:",s),{};let i={};return(a.values||[]).forEach(c=>{var p;let f=String(c[l.field]||"").trim();f&&(i[f]=(p=c[l.value])!=null?p:"")}),console.log("Configuration loaded:",Object.keys(i).length,"fields"),i})}catch(t){return console.error("Failed to load configuration:",t),{}}}async function Pe(e,t,n=[Ke]){if(!W())return!1;try{return await Excel.run(async a=>{let o=await Ct(a,n);if(!o){console.warn("Config table not found for write");return}let s=o.getDataBodyRange(),l=o.getHeaderRowRange();s.load("values"),l.load("values"),await a.sync();let i=l.values[0]||[],r=Et(i);if(r.field===-1||r.value===-1){console.error("Config table missing FIELD or VALUE columns");return}let f=(s.values||[]).findIndex(p=>String(p[r.field]||"").trim()===e);if(f>=0)s.getCell(f,r.value).values=[[t]];else{let p=new Array(i.length).fill("");r.type>=0&&(p[r.type]="Run Settings"),p[r.field]=e,p[r.value]=t,r.permanent>=0&&(p[r.permanent]="N"),r.title>=0&&(p[r.title]=""),o.rows.add(null,[p]),console.log("Added new config row:",e,"=",t)}await a.sync(),console.log("Saved config:",e,"=",t)}),!0}catch(a){return console.error("Failed to save config:",e,a),!1}}var Sn="SS_PF_Config",xn="module-prefix",Xe="system",ke={PR_:"payroll-recorder",PTO_:"pto-accrual",CC_:"credit-card-expense",COM_:"commission-calc",SS_:"system"};async function _t(){if(!W())return{...ke};try{return await Excel.run(async e=>{var f,p;let t=e.workbook.worksheets.getItemOrNullObject(Sn);if(await e.sync(),t.isNullObject)return console.log("[Tab Visibility] Config sheet not found, using defaults"),{...ke};let n=t.getUsedRangeOrNullObject();if(n.load("values"),await e.sync(),n.isNullObject||!((f=n.values)!=null&&f.length))return{...ke};let a=n.values,o=_n(a[0]),s=o.get("category"),l=o.get("field"),i=o.get("value");if(s===void 0||l===void 0||i===void 0)return console.warn("[Tab Visibility] Missing required columns, using defaults"),{...ke};let r={},c=!1;for(let u=1;u<a.length;u++){let d=a[u];if(je(d[s])===xn){let y=String((p=d[l])!=null?p:"").trim().toUpperCase(),w=je(d[i]);y&&w&&(r[y]=w,c=!0)}}return c?(console.log("[Tab Visibility] Loaded prefix config:",r),r):(console.log("[Tab Visibility] No module-prefix rows found, using defaults"),{...ke})})}catch(e){return console.warn("[Tab Visibility] Error reading prefix config:",e),{...ke}}}async function Ze(e){if(!W())return;let t=je(e);console.log(`[Tab Visibility] Applying visibility for module: ${t}`);try{let n=await _t();await Excel.run(async a=>{let o=a.workbook.worksheets;o.load("items/name,visibility"),await a.sync();let s={};for(let[u,d]of Object.entries(n))s[d]||(s[d]=[]),s[d].push(u);let l=s[t]||[],i=s[Xe]||[],r=[];for(let[u,d]of Object.entries(s))u!==t&&u!==Xe&&r.push(...d);console.log(`[Tab Visibility] Active prefixes: ${l.join(", ")}`),console.log(`[Tab Visibility] Other module prefixes (to hide): ${r.join(", ")}`),console.log(`[Tab Visibility] System prefixes (always hide): ${i.join(", ")}`);let c=[],f=[];o.items.forEach(u=>{let d=u.name,g=d.toUpperCase(),y=l.some(C=>g.startsWith(C)),w=r.some(C=>g.startsWith(C)),h=i.some(C=>g.startsWith(C));y?(c.push(u),console.log(`[Tab Visibility] SHOW: ${d} (matches active module prefix)`)):h?(f.push(u),console.log(`[Tab Visibility] HIDE: ${d} (system sheet)`)):w?(f.push(u),console.log(`[Tab Visibility] HIDE: ${d} (other module prefix)`)):console.log(`[Tab Visibility] SKIP: ${d} (no prefix match, leaving as-is)`)});for(let u of c)u.visibility=Excel.SheetVisibility.visible;if(await a.sync(),o.items.filter(u=>u.visibility===Excel.SheetVisibility.visible).length>f.length){for(let u of f)try{u.visibility=Excel.SheetVisibility.hidden}catch(d){console.warn(`[Tab Visibility] Could not hide "${u.name}":`,d.message)}await a.sync()}else console.warn("[Tab Visibility] Skipping hide - would leave no visible sheets");console.log(`[Tab Visibility] Done! Showed ${c.length}, hid ${f.length} tabs`)})}catch(n){console.warn("[Tab Visibility] Error applying visibility:",n)}}async function Cn(){if(!W()){console.log("Excel not available");return}try{await Excel.run(async e=>{let t=e.workbook.worksheets;t.load("items/name,visibility"),await e.sync();let n=0;t.items.forEach(a=>{a.visibility!==Excel.SheetVisibility.visible&&(a.visibility=Excel.SheetVisibility.visible,console.log(`[ShowAll] Made visible: ${a.name}`),n++)}),await e.sync(),console.log(`[ShowAll] Done! Made ${n} sheets visible. Total: ${t.items.length}`)})}catch(e){console.error("[Tab Visibility] Unable to show all sheets:",e)}}async function En(){if(!W()){console.log("Excel not available");return}try{let e=await _t(),t=[];for(let[n,a]of Object.entries(e))a===Xe&&t.push(n);await Excel.run(async n=>{let a=n.workbook.worksheets;a.load("items/name,visibility"),await n.sync(),a.items.forEach(o=>{let s=o.name.toUpperCase();t.some(l=>s.startsWith(l))&&(o.visibility=Excel.SheetVisibility.visible,console.log(`[Unhide] Made visible: ${o.name}`))}),await n.sync(),console.log("[Unhide] System sheets are now visible!")})}catch(e){console.error("[Tab Visibility] Unable to unhide system sheets:",e)}}function _n(e=[]){let t=new Map;return e.forEach((n,a)=>{let o=je(n);o&&t.set(o,a)}),t}function je(e){return String(e!=null?e:"").trim().toLowerCase().replace(/[\s_]+/g,"-")}typeof window!="undefined"&&(window.PrairieForge=window.PrairieForge||{},window.PrairieForge.showAllSheets=Cn,window.PrairieForge.unhideSystemSheets=En,window.PrairieForge.applyModuleTabVisibility=Ze);var Pt={COMPANY_NAME:"Prairie Forge LLC",PRODUCT_NAME:"Prairie Forge Tools",SUPPORT_URL:"https://prairieforge.ai/support",ADA_IMAGE_URL:"https://assets.prairieforge.ai/storage/v1/object/public/Other%20Public%20Material/Prairie%20Forge/Ada%20Image.png"};var Rt=Pt.ADA_IMAGE_URL;async function Le(e,t,n){if(typeof Excel=="undefined"){console.warn("Excel runtime not available for homepage sheet");return}try{await Excel.run(async a=>{let o=a.workbook.worksheets.getItemOrNullObject(e);o.load("isNullObject, name, visibility"),await a.sync();let s;o.isNullObject?(s=a.workbook.worksheets.add(e),await a.sync(),await It(a,s,t,n)):(s=o,s.visibility!==Excel.SheetVisibility.visible&&(s.visibility=Excel.SheetVisibility.visible,await a.sync()),await It(a,s,t,n)),s.activate(),s.getRange("A1").select(),await a.sync()})}catch(a){console.error(`Error activating homepage sheet ${e}:`,a)}}async function It(e,t,n,a){try{let c=t.getUsedRangeOrNullObject();c.load("isNullObject"),await e.sync(),c.isNullObject||(c.clear(),await e.sync())}catch{}t.showGridlines=!1,t.getRange("A:A").format.columnWidth=400,t.getRange("B:B").format.columnWidth=50,t.getRange("1:1").format.rowHeight=60,t.getRange("2:2").format.rowHeight=30;let o=[[n,""],[a,""],["",""],["",""]],s=t.getRangeByIndexes(0,0,4,2);s.values=o;let l=t.getRange("A1:Z100");l.format.fill.color="#0f0f0f";let i=t.getRange("A1");i.format.font.bold=!0,i.format.font.size=36,i.format.font.color="#ffffff",i.format.font.name="Segoe UI Light",i.format.verticalAlignment="Center";let r=t.getRange("A2");r.format.font.size=14,r.format.font.color="#a0a0a0",r.format.font.name="Segoe UI",r.format.verticalAlignment="Top",t.freezePanes.freezeRows(0),t.freezePanes.freezeColumns(0),await e.sync()}var Tt={"module-selector":{sheetName:"SS_Homepage",title:"ForgeSuite",subtitle:"Select a module from the side panel to get started."},"payroll-recorder":{sheetName:"PR_Homepage",title:"Payroll Recorder",subtitle:"Normalize payroll exports, enforce controls, and prep journal entries without leaving Excel."},"pto-accrual":{sheetName:"PTO_Homepage",title:"PTO Accrual",subtitle:"Calculate employee PTO liabilities, compare period-over-period changes, and prepare accrual journal entries."}};function Be(e){return Tt[e]||Tt["module-selector"]}function At(){tt();let e=document.createElement("button");return e.className="pf-ada-fab",e.id="pf-ada-fab",e.setAttribute("aria-label","Ask Ada"),e.setAttribute("title","Ask Ada"),e.innerHTML=`
    ...
    `,document.body.appendChild(t),requestAnimationFrame(()=>{t.classList.add("is-visible")});let n=document.getElementById("ada-modal-close");n==null||n.addEventListener("click",et),t.addEventListener("click",o=>{o.target===t&&et()});let a=o=>{o.key==="Escape"&&(et(),document.removeEventListener("keydown",a))};document.addEventListener("keydown",a)}function et(){let e=document.getElementById("pf-ada-modal-overlay");e&&(e.classList.remove("is-visible"),setTimeout(()=>{e.remove()},300))}var In=["January","February","March","April","May","June","July","August","September","October","November","December"],$t=["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"],Tn=["Su","Mo","Tu","We","Th","Fr","Sa"],X=null,se=null;function Dt(e,t={}){let n=document.getElementById(e);if(!n)return;let{onChange:a=null,minDate:o=null,maxDate:s=null,readonly:l=!1}=t,i=n.closest(".pf-datepicker-wrapper");i||(i=document.createElement("div"),i.className="pf-datepicker-wrapper",n.parentNode.insertBefore(i,n),i.appendChild(n)),n.type="text",n.placeholder="Select date...",n.classList.add("pf-datepicker-input"),n.readOnly=!0;let r=n.value?Nt(n.value):null;r&&(n.value=at(r),n.dataset.value=Ie(r));let c=i.querySelector(".pf-datepicker-icon");c||(c=document.createElement("span"),c.className="pf-datepicker-icon",c.innerHTML='<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="18" height="18" x="3" y="4" rx="2" ry="2"/><line x1="16" x2="16" y1="2" y2="6"/><line x1="8" x2="8" y1="2" y2="6"/><line x1="3" x2="21" y1="10" y2="10"/></svg>',i.appendChild(c));let f={inputId:e,input:n,selectedDate:r,viewDate:r?new Date(r):new Date,onChange:a,minDate:o,maxDate:s};function p(){l||(se=f,Rn())}return n.addEventListener("click",p),c.addEventListener("click",p),{getValue:()=>f.selectedDate?Ie(f.selectedDate):"",setValue:u=>{let d=Nt(u);f.selectedDate=d,f.viewDate=d?new Date(d):new Date,d?(n.value=at(d),n.dataset.value=Ie(d)):(n.value="",n.dataset.value="")},open:p,close:Me}}function Rn(){se&&(X||(X=document.createElement("div"),X.className="pf-datepicker-modal",X.id="pf-datepicker-modal",document.body.appendChild(X)),Lt(),requestAnimationFrame(()=>{X.classList.add("is-open")}),document.addEventListener("keydown",jt))}function Me(){X&&X.classList.remove("is-open"),document.removeEventListener("keydown",jt),se=null}function jt(e){e.key==="Escape"&&Me()}function Lt(){if(!X||!se)return;let{viewDate:e,selectedDate:t,minDate:n,maxDate:a}=se,o=e.getFullYear(),s=e.getMonth();X.innerHTML=`
    ...
    `,An()}function An(){var e;X&&((e=X.querySelector(".pf-datepicker-backdrop"))==null||e.addEventListener("click",Me),X.querySelectorAll(".pf-datepicker-nav").forEach(t=>{t.addEventListener("click",n=>{n.preventDefault(),n.stopPropagation();let a=t.dataset.action;Nn(a)})}),X.querySelectorAll(".pf-datepicker-day:not(.disabled)").forEach(t=>{t.addEventListener("click",n=>{n.preventDefault(),n.stopPropagation();let a=parseInt(t.dataset.day),o=parseInt(t.dataset.month),s=parseInt(t.dataset.year);nt(new Date(s,o,a))})}),X.querySelectorAll(".pf-datepicker-btn").forEach(t=>{t.addEventListener("click",n=>{n.preventDefault(),n.stopPropagation();let a=t.dataset.action;a==="today"?nt(new Date):a==="clear"&&nt(null)})}))}function Nn(e){if(!se)return;let t=se.viewDate;switch(e){case"prev-year":t.setFullYear(t.getFullYear()-1);break;case"prev-month":t.setMonth(t.getMonth()-1);break;case"next-month":t.setMonth(t.getMonth()+1);break;case"next-year":t.setFullYear(t.getFullYear()+1);break}Lt()}function nt(e){if(!se)return;let{input:t,onChange:n}=se;se.selectedDate=e,e?(t.value=at(e),t.dataset.value=Ie(e),se.viewDate=new Date(e)):(t.value="",t.dataset.value=""),n&&n(e?Ie(e):""),t.dispatchEvent(new Event("change",{bubbles:!0})),Me()}function $n(e,t,n,a,o){let s=new Date(e,t,1).getDay(),l=new Date(e,t+1,0).getDate(),i=new Date(e,t,0).getDate(),r=new Date;r.setHours(0,0,0,0),n&&(n=new Date(n),n.setHours(0,0,0,0));let c="";for(let d=s-1;d>=0;d--){let g=i-d,y=t===0?11:t-1,w=t===0?e-1:e;c+=`<button type="button" class="pf-datepicker-day other-month" data-day="${g}" data-month="${y}" data-year="${w}">${g}</button>`}for(let d=1;d<=l;d++){let g=new Date(e,t,d);g.setHours(0,0,0,0);let y=g.getTime()===r.getTime(),w=n&&g.getTime()===n.getTime(),h="pf-datepicker-day";y&&(h+=" today"),w&&(h+=" selected");let C=!1;a&&g<a&&(C=!0),o&&g>o&&(C=!0),C&&(h+=" disabled"),c+=`<button type="button" class="${h}" data-day="${d}" data-month="${t}" data-year="${e}" ${C?"disabled":""}>${d}</button>`}let f=42,p=s+l,u=f-p;for(let d=1;d<=u;d++){let g=t===11?0:t+1,y=t===11?e+1:e;c+=`<button type="button" class="pf-datepicker-day other-month" data-day="${d}" data-month="${g}" data-year="${y}">${d}</button>`}return c}function Nt(e){if(!e)return null;if(/^\d{4}-\d{2}-\d{2}$/.test(e)){let[a,o,s]=e.split("-").map(Number);return new Date(a,o-1,s)}let t=e.match(/^(\w+)\s+(\d+),\s+(\d{4})$/);if(t){let a=$t.findIndex(o=>o.toLowerCase()===t[1].toLowerCase().substring(0,3));if(a>=0)return new Date(parseInt(t[3]),a,parseInt(t[2]))}if(/^\d{1,2}\/\d{1,2}\/\d{4}$/.test(e)){let[a,o,s]=e.split("/").map(Number);return new Date(s,a-1,o)}let n=new Date(e);return isNaN(n.getTime())?null:n}function at(e){return e?`${$t[e.getMonth()]} ${e.getDate()}, ${e.getFullYear()}`:""}function Ie(e){if(!e)return"";let t=e.getFullYear(),n=String(e.getMonth()+1).padStart(2,"0"),a=String(e.getDate()).padStart(2,"0");return`${t}-${n}-${a}`}var Bt=`
    <svg
...
`.trim(),Dn={config:`
    ...
    `};function Ht(e){return e&&Dn[e]||""}var ot=`
    <svg
...
`.trim();function Ae(e){return e==null?"":String(e).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function M(e,t){return`
    ...
    `}function me({reviewerInputId:e,reviewerValue:t,signoffInputId:n,signoffValue:a,isComplete:o,saveButtonId:s,isSaved:l=!1,completeButtonId:i,subtext:r="Sign-off below. Click checkmark icon. Done.",prevButtonId:c=null,nextButtonId:f=null}){let p=c||`${i}-prev`,u=f||`${i}-next`,d=`<button type="button" class="pf-action-toggle ${o?"is-active":""}" id="${i}" aria-pressed="${!!o}" title="Mark step complete">${Te}</button>`,g=`<button type="button" class="pf-action-toggle pf-nav-toggle" id="${p}" title="Previous step">${Fe}</button>`,y=`<button type="button" class="pf-action-toggle pf-nav-toggle" id="${u}" title="Next step">${Ue}</button>`;return`
    ...
    `}function cn(){if(!O.loaded)return`
        ...
        `;let e=tn(te(k.payrollDate)),t=tn(te(k.accountingPeriod)),n=te(k.journalEntryId),a=te(k.accountingSoftware),o=te(k.payrollProvider),s=te(k.companyName),l=te(k.reviewerName),i=we(0),r=!!O.permanents[0],c=!!(bn(O.completes[0])||i.signOffDate),f=be(i==null?void 0:i.reviewer),p=(i==null?void 0:i.signOffDate)||"";return`
        ...
        <section class="pf-step-guide">
            ...
            ${me({reviewerInputId:"config-reviewer",reviewerValue:f,signoffInputId:"config-signoff-date",signoffValue:p,isComplete:c,saveButtonId:"config-signoff-save",completeButtonId:"config-signoff-toggle"})}
        </section>
    `}function ta(e){let t=we(1),n=!!O.permanents[1],a=be(t==null?void 0:t.reviewer),o=(t==null?void 0:t.signOffDate)||"",s=!!(Ce(O.completes[1])||o),l=te(k.payrollProvider);return`
        <section class="pf-hero" id="pf-step-hero">
            <p class="pf-hero-copy">${b(ue)} | Step ${e.id}</p>
            <h2 class="pf-hero-title">${b(e.title)}</h2>
            <p class="pf-hero-copy">${b(e.summary||"")}</p>
        ...
        <section class="pf-step-guide">
            <article class="pf-step-card pf-step-detail pf-config-card">
                ...
                <div class="pf-signoff-action">
                    ...
                    ${M(`<button type="button" class="pf-action-toggle pf-clickable" id="import-clear-btn" title="Clear PTO_Data to start over">${Jt}</button>`,"Clear")}
                </div>
            </article>
    ...
    `}function na(e){let t=ae.find(i=>i.id===e);if(!t)return"";if(e===0)return cn();if(e===1)return ta(t);if(e===2)return Va(t);if(e===3)return Fa(t);if(e===4)return Ua(t);if(e===5)return Ga(t);if(t.id===6)return ea(t);let n=we(e),a=!!O.permanents[e],o=be(n==null?void 0:n.reviewer),s=(n==null?void 0:n.signOffDate)||"",l=!!(Ce(O.completes[e])||s);return`
        <section class="pf-hero" id="pf-step-hero">
    ...
    `}function aa(){var n,a,o,s,l;(n=document.getElementById("nav-home"))==null||n.addEventListener("click",async()=>{var r;let i=Be($e);await Le(i.sheetName,i.title,i.subtitle),xe({activeView:"home",activeStepId:null}),(r=document.getElementById("pf-hero"))==null||r.scrollIntoView({behavior:"smooth",block:"start"})}),(a=document.getElementById("nav-selector"))==null||a.addEventListener("click",()=>{window.location.href=Mn}),(o=document.getElementById("nav-prev"))==null||o.addEventListener("click",()=>Je(-1)),(s=document.getElementById("nav-next"))==null||s.addEventListener("click",()=>Je(1));let e=document.getElementById("nav-quick-toggle"),t=document.getElementById("quick-access-dropdown");e==null||e.addEventListener("click",i=>{i.stopPropagation(),t==null||t.classList.toggle("hidden"),e.classList.toggle("is-active")}),document.addEventListener("click",i=>{!(t!=null&&t.contains(i.target))&&!(e!=null&&e.contains(i.target))&&(t==null||t.classList.add("hidden"),e==null||e.classList.remove("is-active"))}),(l=document.getElementById("nav-config"))==null||l.addEventListener("click",async()=>{t==null||t.classList.add("hidden"),e==null||e.classList.remove("is-active"),await Sa()}),document.querySelectorAll("[data-step-card]").forEach(i=>{let r=Number(i.getAttribute("data-step-index")),c=Number(i.getAttribute("data-step-id"));i.addEventListener("click",()=>De(r,c))}),R.activeView==="config"?sa():R.activeView==="step"&&R.activeStepId!=null&&oa(R.activeStepId)}function oa(e){var u,d,g,y,w,h,C,E,T,_,j,L,B,Q,U,q,I,m;let t=e===2?document.getElementById("step-notes-input"):document.getElementById(`step-notes-${e}`),n=e===2?document.getElementById("step-reviewer-name"):document.getElementById(`step-reviewer-${e}`),a=e===2?document.getElementById("step-signoff-date"):document.getElementById(`step-signoff-${e}`),o=document.getElementById("step-back-btn"),s=e===2?document.getElementById("step-notes-lock-2"):document.getElementById(`step-notes-lock-${e}`),l=e===2?document.getElementById("step-notes-save-2"):document.getElementById(`step-notes-save-${e}`);l==null||l.addEventListener("click",async()=>{let v=(t==null?void 0:t.value)||"";await ne(e,"notes",v),de(l,!0)});let i=e===2?document.getElementById("headcount-signoff-save"):document.getElementById(`step-signoff-save-${e}`);i==null||i.addEventListener("click",async()=>{let v=(n==null?void 0:n.value)||"";await ne(e,"reviewer",v),de(i,!0)}),lt();let r=e===2?"headcount-signoff-toggle":`step-signoff-toggle-${e}`,c=`${r}-prev`,f=`${r}-next`,p=e===2?"step-signoff-date":`step-signoff-${e}`;vn(e,{buttonId:r,inputId:p,canActivate:e===2?()=>{var P;return!wn()||((P=document.getElementById("step-notes-input"))==null?void 0:P.value.trim())||""?!0:(J("Please enter a brief explanation of the headcount differences before completing this step.","info"),!1)}:null,onComplete:ia(e)}),fn(c,f),o==null||o.addEventListener("click",async()=>{let v=Be($e);await Le(v.sheetName,v.title,v.subtitle),xe({activeView:"home",activeStepId:null})}),s==null||s.addEventListener("click",async()=>{let v=!s.classList.contains("is-locked");rt(s,v),await hn(e,v)}),e===6&&((u=document.getElementById("archive-run-btn"))==null||u.addEventListener("click",()=>{ha()})),e===1&&((d=document.getElementById("import-open-data-btn"))==null||d.addEventListener("click",()=>gn("PTO_Data")),(g=document.getElementById("import-clear-btn"))==null||g.addEventListener("click",()=>wa())),e===2&&((y=document.getElementById("headcount-skip-btn"))==null||y.addEventListener("click",()=>{A.skipAnalysis=!A.skipAnalysis;let v=document.getElementById("headcount-skip-btn");v==null||v.classList.toggle("is-active",A.skipAnalysis),A.skipAnalysis&&rn(),sn()}),(w=document.getElementById("headcount-run-btn"))==null||w.addEventListener("click",()=>pt()),(h=document.getElementById("headcount-refresh-btn"))==null||h.addEventListener("click",()=>pt()),Wa(),A.skipAnalysis&&rn(),sn()),e===3&&((C=document.getElementById("quality-run-btn"))==null||C.addEventListener("click",()=>Qt()),(E=document.getElementById("quality-refresh-btn"))==null||E.addEventListener("click",()=>Qt()),(T=document.getElementById("quality-acknowledge-btn"))==null||T.addEventListener("click",()=>da())),e===4&&((_=document.getElementById("analysis-refresh-btn"))==null||_.addEventListener("click",()=>Xt()),(j=document.getElementById("analysis-run-btn"))==null||j.addEventListener("click",()=>Xt()),(L=document.getElementById("payrate-save-btn"))==null||L.addEventListener("click",Kt),(B=document.getElementById("payrate-ignore-btn"))==null||B.addEventListener("click",ca),(Q=document.getElementById("payrate-input"))==null||Q.addEventListener("keydown",v=>{v.key==="Enter"&&Kt()})),e===5&&((U=document.getElementById("je-create-btn"))==null||U.addEventListener("click",()=>pa()),(q=document.getElementById("je-run-btn"))==null||q.addEventListener("click",()=>pn()),(I=document.getElementById("je-export-btn"))==null||I.addEventListener("click",()=>ga()),(m=document.getElementById("je-upload-btn"))==null||m.addEventListener("click",()=>ma()))}function sa(){var i,r,c,f,p;Dt("config-payroll-date",{onChange:u=>{if(le(k.payrollDate,u),!!u){if(O.overrides.accountingPeriod=!1,O.overrides.journalId=!1,!O.overrides.accountingPeriod){let d=$a(u);if(d){let g=document.getElementById("config-accounting-period");g&&(g.value=d),le(k.accountingPeriod,d)}}if(!O.overrides.journalId){let d=Da(u);if(d){let g=document.getElementById("config-journal-id");g&&(g.value=d),le(k.journalEntryId,d)}}}}});let e=document.getElementById("config-accounting-period");e==null||e.addEventListener("change",u=>{O.overrides.accountingPeriod=!!u.target.value,le(k.accountingPeriod,u.target.value||"")});let t=document.getElementById("config-journal-id");t==null||t.addEventListener("change",u=>{O.overrides.journalId=!!u.target.value,le(k.journalEntryId,u.target.value.trim())}),(i=document.getElementById("config-company-name"))==null||i.addEventListener("change",u=>{le(k.companyName,u.target.value.trim())}),(r=document.getElementById("config-payroll-provider"))==null||r.addEventListener("change",u=>{le(k.payrollProvider,u.target.value.trim())}),(c=document.getElementById("config-accounting-link"))==null||c.addEventListener("change",u=>{le(k.accountingSoftware,u.target.value.trim())}),(f=document.getElementById("config-user-name"))==null||f.addEventListener("change",u=>{le(k.reviewerName,u.target.value.trim())});let n=document.getElementById("config-notes");n==null||n.addEventListener("input",u=>{ne(0,"notes",u.target.value)});let a=document.getElementById("config-notes-lock");a==null||a.addEventListener("click",async()=>{let u=!a.classList.contains("is-locked");rt(a,u),await hn(0,u)});let o=document.getElementById("config-notes-save");o==null||o.addEventListener("click",async()=>{n&&(await ne(0,"notes",n.value),de(o,!0))});let s=document.getElementById("config-reviewer");s==null||s.addEventListener("change",u=>{let d=u.target.value.trim();ne(0,"reviewer",d);let g=document.getElementById("config-signoff-date");if(d&&g&&!g.value){let y=mt();g.value=y,ne(0,"signOffDate",y),yn(0,!0)}}),(p=document.getElementById("config-signoff-date"))==null||p.addEventListener("change",u=>{ne(0,"signOffDate",u.target.value||"")});let l=document.getElementById("config-signoff-save");l==null||l.addEventListener("click",async()=>{var g,y;let u=((g=s==null?void 0:s.value)==null?void 0:g.trim())||"",d=((y=document.getElementById("config-signoff-date"))==null?void 0:y.value)||"";await ne(0,"reviewer",u),await ne(0,"signOffDate",d),de(l,!0)}),lt(),vn(0,{buttonId:"config-signoff-toggle",inputId:"config-signoff-date",onComplete:()=>{Ma(),dn(0),un()}}),fn("config-signoff-toggle-prev","config-signoff-toggle-next")}function De(e,t=null){if(e<0||e>=ae.length)return;ze=e;let n=t!=null?t:ae[e].id;xe({focusedIndex:e,activeView:n===0?"config":"step",activeStepId:n});let o=Hn[n];o&&gn(o),n===2&&!A.hasAnalyzed&&(kn(),pt())}function ia(e){return e===6?null:()=>dn(e)}function dn(e){let t=ae.findIndex(a=>a.id===e);if(t===-1)return;let n=t+1;n<ae.length&&(De(n,ae[n].id),un())}function un(){let e=[document.querySelector(".pf-root"),document.querySelector(".pf-step-guide"),document.body];for(let t of e)t&&t.scrollTo({top:0,behavior:"smooth"});window.scrollTo({top:0,behavior:"smooth"})}function Je(e){let t=R.focusedIndex+e,n=Math.max(0,Math.min(ae.length-1,t));De(n,ae[n].id),window.scrollTo({top:0,behavior:"smooth"})}function fn(e,t){var n,a;(n=document.getElementById(e))==null||n.addEventListener("click",()=>Je(-1)),(a=document.getElementById(t))==null||a.addEventListener("click",()=>Je(1))}function ra(){if(ze===null)return;let e=document.querySelector(`[data-step-index="${ze}"]`);ze=null,e==null||e.scrollIntoView({behavior:"smooth",block:"center"})}function la(e){var a;let t=bn(O.completes[e]),n=!!((a=O.steps[e])!=null&&a.signOffDate);return t||n}function xe(e){e.stepStatuses&&(R.stepStatuses={...R.stepStatuses,...e.stepStatuses}),Object.assign(R,{...e,stepStatuses:R.stepStatuses}),ie()}function oe(){return typeof Excel!="undefined"&&typeof Excel.run=="function"}async function Kt(){let e=document.getElementById("payrate-input");if(!e)return;let t=parseFloat(e.value),n=e.dataset.employee,a=parseInt(e.dataset.row,10);if(isNaN(t)||t<=0){J("Please enter a valid pay rate greater than 0.","info");return}if(!n||isNaN(a)){console.error("Missing employee data on input");return}Z(!0,"Updating pay rate...");try{await Excel.run(async o=>{let s=o.workbook.worksheets.getItem("PTO_Analysis"),l=s.getCell(a-1,3);l.values=[[t]];let i=s.getCell(a-1,8);i.load("values"),await o.sync();let c=(Number(i.values[0][0])||0)*t,f=s.getCell(a-1,9);f.values=[[c]];let p=s.getCell(a-1,10);p.load("values"),await o.sync();let u=Number(p.values[0][0])||0,d=c-u,g=s.getCell(a-1,11);g.values=[[d]],await o.sync()}),ee.missingPayRates=ee.missingPayRates.filter(o=>o.name!==n),Z(!1),De(3,3)}catch(o){console.error("Failed to save pay rate:",o),J(`Failed to save pay rate: ${o.message}`,"error"),Z(!1)}}function ca(){let e=document.getElementById("payrate-input");if(!e)return;let t=e.dataset.employee;t&&(ee.ignoredMissingPayRates.add(t),ee.missingPayRates=ee.missingPayRates.filter(n=>n.name!==t)),De(3,3)}async function Qt(){if(!oe()){J("Excel is not available. Open this module inside Excel to run quality check.","info");return}K.loading=!0,Z(!0,"Analyzing data quality..."),de(document.getElementById("quality-save-btn"),!1);try{await Excel.run(async t=>{var w;let a=t.workbook.worksheets.getItem("PTO_Data").getUsedRangeOrNullObject();a.load("values"),await t.sync();let o=a.isNullObject?[]:a.values||[];if(!o.length||o.length<2)throw new Error("PTO_Data is empty or has no data rows.");let s=(o[0]||[]).map(h=>$(h));console.log("[Data Quality] PTO_Data headers:",o[0]);let l=s.findIndex(h=>h==="employee name"||h==="employeename");l===-1&&(l=s.findIndex(h=>h.includes("employee")&&h.includes("name"))),l===-1&&(l=s.findIndex(h=>h==="name"||h.includes("name")&&!h.includes("company")&&!h.includes("form"))),console.log("[Data Quality] Employee name column index:",l,"Header:",(w=o[0])==null?void 0:w[l]);let i=D(s,["balance"]),r=D(s,["accrual rate","accrualrate"]),c=D(s,["carry over","carryover"]),f=D(s,["ytd accrued","ytdaccrued"]),p=D(s,["ytd used","ytdused"]),u=[],d=[],g=[],y=o.slice(1);y.forEach((h,C)=>{let E=C+2,T=l!==-1?String(h[l]||"").trim():`Row ${E}`;if(!T)return;let _=i!==-1&&Number(h[i])||0,j=r!==-1&&Number(h[r])||0,L=c!==-1&&Number(h[c])||0,B=f!==-1&&Number(h[f])||0,Q=p!==-1&&Number(h[p])||0,U=L+B;_<0?u.push({name:T,issue:`Negative balance: ${_.toFixed(2)} hrs`,rowIndex:E}):Q>U&&U>0&&u.push({name:T,issue:`Used ${Q.toFixed(0)} hrs but only ${U.toFixed(0)} available`,rowIndex:E}),_===0&&(L>0||B>0)&&d.push({name:T,rowIndex:E}),j>8&&g.push({name:T,accrualRate:j,rowIndex:E})}),K.balanceIssues=u,K.zeroBalances=d,K.accrualOutliers=g,K.totalIssues=u.length,K.totalEmployees=y.filter(h=>h.some(C=>C!==null&&C!=="")).length,K.hasRun=!0});let e=K.balanceIssues.length>0;xe({stepStatuses:{3:e?"blocked":"complete"}})}catch(e){console.error("Data quality check error:",e),J(`Quality check failed: ${e.message}`,"error"),K.hasRun=!1}finally{K.loading=!1,Z(!1),ie()}}function da(){K.acknowledged=!0,xe({stepStatuses:{3:"complete"}}),ie()}async function ua(){if(oe())try{await Excel.run(async e=>{let t=e.workbook.worksheets.getItem("PTO_Data"),n=e.workbook.worksheets.getItemOrNullObject("PTO_Analysis"),a=t.getUsedRangeOrNullObject();if(a.load("values"),n.load("isNullObject"),await e.sync(),n.isNullObject){ee.completenessCheck={accrualRate:null,carryOver:null,ytdAccrued:null,ytdUsed:null,balance:null};return}let o=n.getUsedRangeOrNullObject();o.load("values"),await e.sync();let s=a.isNullObject?[]:a.values||[],l=o.isNullObject?[]:o.values||[];if(!s.length||!l.length){ee.completenessCheck={accrualRate:null,carryOver:null,ytdAccrued:null,ytdUsed:null,balance:null};return}let i=(f,p,u)=>{let d=(f[0]||[]).map(w=>$(w)),g=D(d,p);return g===-1?null:f.slice(1).reduce((w,h)=>w+(Number(h[g])||0),0)},r=[{key:"accrualRate",aliases:["accrual rate","accrualrate"]},{key:"carryOver",aliases:["carry over","carryover","carry_over"]},{key:"ytdAccrued",aliases:["ytd accrued","ytdaccrued","ytd_accrued"]},{key:"ytdUsed",aliases:["ytd used","ytdused","ytd_used"]},{key:"balance",aliases:["balance"]}],c={};for(let f of r){let p=i(s,f.aliases,"PTO_Data"),u=i(l,f.aliases,"PTO_Analysis");if(p===null||u===null)c[f.key]=null;else{let d=Math.abs(p-u)<.01;c[f.key]={match:d,ptoData:p,ptoAnalysis:u}}}ee.completenessCheck=c})}catch(e){console.error("Completeness check failed:",e)}}async function Xt(){if(!oe()){J("Excel is not available. Open this module inside Excel to run analysis.","info");return}Z(!0,"Running analysis...");try{await kn(),await ua(),ee.cleanDataReady=!0,ie()}catch(e){console.error("Full analysis error:",e),J(`Analysis failed: ${e.message}`,"error")}finally{Z(!1)}}async function pn(){if(!oe()){J("Excel is not available. Open this module inside Excel to run journal checks.","info");return}z.loading=!0,z.lastError=null,de(document.getElementById("je-save-btn"),!1),ie();try{let e=await Excel.run(async t=>{let a=t.workbook.worksheets.getItem("PTO_JE_Draft").getUsedRangeOrNullObject();a.load("values");let o=t.workbook.worksheets.getItemOrNullObject("PTO_Analysis");o.load("isNullObject"),await t.sync();let s=a.isNullObject?[]:a.values||[];if(!s.length)throw new Error("PTO_JE_Draft is empty. Generate the JE first.");let l=(s[0]||[]).map(E=>$(E)),i=D(l,["debit"]),r=D(l,["credit"]),c=D(l,["lineamount","line amount"]),f=D(l,["account number","accountnumber"]);if(i===-1||r===-1)throw new Error("Could not find Debit and Credit columns in PTO_JE_Draft.");let p=0,u=0,d=0,g=0;s.slice(1).forEach(E=>{let T=Number(E[i])||0,_=Number(E[r])||0,j=c!==-1&&Number(E[c])||0,L=f!==-1?String(E[f]||"").trim():"";p+=T,u+=_,d+=j,L&&L!=="21540"&&(g+=j)});let y=0;if(!o.isNullObject){let E=o.getUsedRangeOrNullObject();E.load("values"),await t.sync();let T=E.isNullObject?[]:E.values||[];if(T.length>1){let _=(T[0]||[]).map(L=>$(L)),j=D(_,["change"]);j!==-1&&T.slice(1).forEach(L=>{y+=Number(L[j])||0})}}let w=p-u,h=[];Math.abs(w)>=.01?h.push({check:"Debits = Credits",passed:!1,detail:w>0?`Debits exceed credits by $${Math.abs(w).toLocaleString(void 0,{minimumFractionDigits:2})}`:`Credits exceed debits by $${Math.abs(w).toLocaleString(void 0,{minimumFractionDigits:2})}`}):h.push({check:"Debits = Credits",passed:!0,detail:""}),Math.abs(d)>=.01?h.push({check:"Line Amounts Sum to Zero",passed:!1,detail:`Line amounts sum to $${d.toLocaleString(void 0,{minimumFractionDigits:2})} (should be $0.00)`}):h.push({check:"Line Amounts Sum to Zero",passed:!0,detail:""});let C=Math.abs(g-y);return C>=.01?h.push({check:"JE Matches Analysis Total",passed:!1,detail:`JE expense total ($${g.toLocaleString(void 0,{minimumFractionDigits:2})}) differs from PTO_Analysis Change total ($${y.toLocaleString(void 0,{minimumFractionDigits:2})}) by $${C.toLocaleString(void 0,{minimumFractionDigits:2})}`}):h.push({check:"JE Matches Analysis Total",passed:!0,detail:""}),{debitTotal:p,creditTotal:u,difference:w,lineAmountSum:d,jeChangeTotal:g,analysisChangeTotal:y,issues:h,validationRun:!0}});Object.assign(z,e,{lastError:null})}catch(e){console.warn("PTO JE summary:",e),z.lastError=(e==null?void 0:e.message)||"Unable to calculate journal totals.",z.debitTotal=null,z.creditTotal=null,z.difference=null,z.lineAmountSum=null,z.jeChangeTotal=null,z.analysisChangeTotal=null,z.issues=[],z.validationRun=!1}finally{z.loading=!1,ie()}}var fa={"general & administrative":"64110","general and administrative":"64110","g&a":"64110","research & development":"62110","research and development":"62110","r&d":"62110",marketing:"61610","cogs onboarding":"53110","cogs prof. services":"56110","cogs professional services":"56110","sales & marketing":"61110","sales and marketing":"61110","cogs support":"52110","client success":"61811"},Zt="21540";async function pa(){if(!oe()){J("Excel is not available. Open this module inside Excel to create the journal entry.","info");return}Z(!0,"Creating PTO Journal Entry...");try{await Excel.run(async e=>{let t=[],n=e.workbook.tables.getItemOrNullObject(ce[0]);if(n.load("isNullObject"),await e.sync(),n.isNullObject){let m=e.workbook.worksheets.getItemOrNullObject("SS_PF_Config");if(m.load("isNullObject"),await e.sync(),!m.isNullObject){let v=m.getUsedRangeOrNullObject();v.load("values"),await e.sync();let P=v.isNullObject?[]:v.values||[];t=P.length>1?P.slice(1):[]}}else{let m=n.getDataBodyRange();m.load("values"),await e.sync(),t=m.values||[]}let a=e.workbook.worksheets.getItemOrNullObject("PTO_Analysis");if(a.load("isNullObject"),await e.sync(),a.isNullObject)throw new Error("PTO_Analysis sheet not found. Please ensure the worksheet exists.");let o=a.getUsedRangeOrNullObject();o.load("values");let s=e.workbook.worksheets.getItemOrNullObject("SS_Chart_of_Accounts");s.load("isNullObject"),await e.sync();let l=[];if(!s.isNullObject){let m=s.getUsedRangeOrNullObject();m.load("values"),await e.sync(),l=m.isNullObject?[]:m.values||[]}let i=o.isNullObject?[]:o.values||[];if(!i.length||i.length<2)throw new Error("PTO_Analysis is empty or has no data rows. Run the analysis first (Step 4).");let r={};t.forEach(m=>{let v=String(m[1]||"").trim(),P=m[2];v&&(r[v]=P)}),(!r[k.journalEntryId]||!r[k.payrollDate])&&console.warn("[JE Draft] Missing config values - RefNumber:",r[k.journalEntryId],"TxnDate:",r[k.payrollDate]);let c=r[k.journalEntryId]||"",f=r[k.payrollDate]||"",p=r[k.accountingPeriod]||"",u="";if(f)try{let m;if(typeof f=="number"||/^\d{4,5}$/.test(String(f).trim())){let v=Number(f),P=new Date(1899,11,30);m=new Date(P.getTime()+v*24*60*60*1e3)}else m=new Date(f);if(!isNaN(m.getTime())&&m.getFullYear()>1970){let v=String(m.getMonth()+1).padStart(2,"0"),P=String(m.getDate()).padStart(2,"0"),N=m.getFullYear();u=`${v}/${P}/${N}`}else console.warn("[JE Draft] Date parsing resulted in invalid date:",f,"->",m),u=String(f)}catch(m){console.warn("[JE Draft] Could not parse TxnDate:",f,m),u=String(f)}let d=p?`${p} PTO Accrual`:"PTO Accrual",g={};if(l.length>1){let m=(l[0]||[]).map(N=>$(N)),v=D(m,["account number","accountnumber","account","acct"]),P=D(m,["account name","accountname","name","description"]);v!==-1&&P!==-1&&l.slice(1).forEach(N=>{let Y=String(N[v]||"").trim(),fe=String(N[P]||"").trim();Y&&(g[Y]=fe)})}let y=(i[0]||[]).map(m=>$(m));console.log("[JE Draft] PTO_Analysis headers:",y),console.log("[JE Draft] PTO_Analysis row count:",i.length-1);let w=D(y,["department"]),h=D(y,["change"]);if(console.log("[JE Draft] Column indices - Department:",w,"Change:",h),w===-1||h===-1)throw new Error(`Could not find required columns in PTO_Analysis. Found headers: ${y.join(", ")}. Looking for "Department" (found: ${w!==-1}) and "Change" (found: ${h!==-1}).`);let C={},E=0,T=0,_=0;if(i.slice(1).forEach((m,v)=>{E++;let P=String(m[w]||"").trim(),N=m[h],Y=Number(N)||0;if(v<3&&console.log(`[JE Draft] Row ${v+2}: Dept="${P}", Change raw="${N}", Change num=${Y}`),!P){_++;return}if(Y===0){T++;return}C[P]||(C[P]=0),C[P]+=Y}),console.log(`[JE Draft] Data summary: ${E} rows, ${T} with zero change, ${_} missing dept`),console.log("[JE Draft] Department totals:",C),Object.keys(C).length===0){let m=`No journal entry lines could be created.

`;throw T===E?(m+=`All 'Change' amounts in PTO_Analysis are $0.00.
...
`,m+="Please verify Pay Rate values exist in PTO_Analysis."):_===E?(m+=`All rows are missing Department values.
...
`,m+=`\u2022 ${_} rows missing department`),new Error(m)}let L=["RefNumber","TxnDate","Account Number","Account Name","LineAmount","Debit","Credit","LineDesc","Department"],B=[L],Q=0,U=0;Object.entries(C).forEach(([m,v])=>{if(Math.abs(v)<.01)return;let P=m.toLowerCase().trim(),N=fa[P]||"",Y=g[N]||"",fe=v>0?Math.abs(v):0,S=v<0?Math.abs(v):0;Q+=fe,U+=S,B.push([c,u,N,Y,v,fe,S,d,m])});let q=Q-U;if(Math.abs(q)>=.01){let m=q<0?Math.abs(q):0,v=q>0?Math.abs(q):0,P=g[Zt]||"Accrued PTO";B.push([c,u,Zt,P,-q,m,v,d,""])}let I=e.workbook.worksheets.getItemOrNullObject("PTO_JE_Draft");if(I.load("isNullObject"),await e.sync(),I.isNullObject)I=e.workbook.worksheets.add("PTO_JE_Draft");else{let m=I.getUsedRangeOrNullObject();m.load("isNullObject"),await e.sync(),m.isNullObject||m.clear()}if(B.length>0){let m=I.getRangeByIndexes(0,0,B.length,L.length);m.values=B;let v=I.getRangeByIndexes(0,0,1,L.length);dt(v);let P=B.length-1;P>0&&(he(I,4,P,!0),he(I,5,P),he(I,6,P)),m.format.autofitColumns()}await e.sync(),I.activate(),I.getRange("A1").select(),await e.sync()}),await pn()}catch(e){console.error("Create JE Draft error:",e),J(`Unable to create Journal Entry: ${e.message}`,"error")}finally{Z(!1)}}async function ga(){if(!oe()){J("Excel is not available. Open this module inside Excel to export.","info");return}Z(!0,"Preparing JE CSV...");try{let{rows:e}=await Excel.run(async n=>{let o=n.workbook.worksheets.getItem("PTO_JE_Draft").getUsedRangeOrNullObject();o.load("values"),await n.sync();let s=o.isNullObject?[]:o.values||[];if(!s.length)throw new Error("PTO_JE_Draft is empty.");return{rows:s}}),t=qa(e);Ya(`pto-je-draft-${mt()}.csv`,t)}catch(e){console.error("PTO JE export:",e),J("Unable to export the JE draft. Confirm the sheet has data.","error")}finally{Z(!1)}}async function ma(){let e=te(k.accountingSoftware)||te("SS_Accounting_Software");if(!e&&W())try{let t=await Qe(ce);e=t.SS_Accounting_Software||t.Accounting_Software||t[k.accountingSoftware]}catch(t){console.warn("Error reading accounting software URL:",t)}if(!e){J("No accounting software URL configured. Add SS_Accounting_Software to SS_PF_Config.","info",5e3);return}!e.startsWith("http://")&&!e.startsWith("https://")&&(e="https://"+e),window.open(e,"_blank"),J("Opening accounting software...","success",2e3)}async function ha(){if(!(!R.workbookReady||!oe())){Z(!0,"Archiving PTO outputs...");try{await Excel.run(async e=>{let t=await Ea(e),n=e.workbook.tables.getItemOrNullObject("PTOArchiveLog");n.load("isNullObject"),await e.sync(),n.isNullObject||n.rows.add(null,[[new Date().toISOString(),"Archived PTO run","Completed via module"]]),await Pa(e),await Ta(e,t);let a=["PTO_JE_Draft","PTO_Analysis","PTO_Data"];for(let o of a)await Ca(e,o);await _a(e),await e.sync()}),xe({stepStatuses:{1:"pending",2:"pending",3:"pending",4:"pending",5:"pending",6:"complete"}}),ya()}catch(e){console.error(e),J("Archive failed: "+e.message,"error")}finally{Z(!1)}}}function ya(){document.querySelectorAll(".pf-save-prompt").forEach(n=>n.remove());let e=document.createElement("div");if(e.className="pf-save-prompt",e.innerHTML=`
...
`)],{type:"text/csv"}),s=URL.createObjectURL(o),l=document.createElement("a");l.href=s,l.download=a,document.body.appendChild(l),l.click(),document.body.removeChild(l),URL.revokeObjectURL(s)}async function Ia(e){let t=e.workbook.worksheets.getItemOrNullObject("SS_PF_Config");if(t.load("isNullObject"),await e.sync(),t.isNullObject)return[];let n=t.getUsedRangeOrNullObject();n.load("values"),await e.sync();let a=n.isNullObject?[]:n.values||[];if(!a.length)return[];let o=(a[0]||[]).map(r=>$(r)),s={category:o.findIndex(r=>r==="category"),module:o.findIndex(r=>r==="module"),field:o.findIndex(r=>r==="field"),value:o.findIndex(r=>r==="value")};if(s.category===-1||s.field===-1)return[];let l=$(ue),i=a.slice(1).filter(r=>{let c=$(r[s.category]),f=s.module>=0?$(r[s.module]):"",p=s.value>=0?$(r[s.value]):"";return c==="tab-structure"&&(f===l||p==="pto-accrual")}).map(r=>{var c;return String((c=r[s.field])!=null?c:"").trim()}).filter(Boolean);return Array.from(new Set(i))}async function Ta(e,t=null){let n=e.workbook.worksheets.getItemOrNullObject("PTO_Analysis"),a=e.workbook.worksheets.getItemOrNullObject("PTO_Archive_Summary");if(n.load("isNullObject"),a.load("isNullObject"),await e.sync(),n.isNullObject||a.isNullObject)return;let o=t;if(!o||!o.length){let i=n.getUsedRangeOrNullObject();i.load("values"),await e.sync(),o=i.isNullObject?[]:i.values||[]}if(!o.length)return;let s=a.getUsedRangeOrNullObject();s.load("isNullObject"),await e.sync(),s.isNullObject||s.clear();let l=a.getRangeByIndexes(0,0,o.length,o[0].length);l.values=o,l.format.autofitColumns(),a.getRange("A1").select(),await e.sync()}function te(e){var n,a;let t=String(e!=null?e:"").trim();return(a=(n=O.values)==null?void 0:n[t])!=null?a:""}function be(e){var n;if(e)return e;let t=te(k.reviewerName);if(t)return t;if((n=window.PrairieForge)!=null&&n._sharedConfigCache){let a=window.PrairieForge._sharedConfigCache.get("SS_Default_Reviewer")||window.PrairieForge._sharedConfigCache.get("Default_Reviewer");if(a)return a}return""}function le(e,t,n={}){var l;let a=String(e!=null?e:"").trim();if(!a)return;O.values[a]=t!=null?t:"";let o=(l=n.debounceMs)!=null?l:0;if(!o){let i=Se.get(a);i&&clearTimeout(i),Se.delete(a),Pe(a,t!=null?t:"",ce);return}Se.has(a)&&clearTimeout(Se.get(a));let s=setTimeout(()=>{Se.delete(a),Pe(a,t!=null?t:"",ce)},o);Se.set(a,s)}function $(e){return String(e!=null?e:"").trim().toLowerCase()}function Z(e,t="Working..."){let n=document.getElementById(Vn);n&&(n.style.display="none")}function ft(){Gn()}typeof Office!="undefined"&&Office.onReady?Office.onReady(()=>ft()).catch(()=>ft()):ft();function we(e){return O.steps[e]||{notes:"",reviewer:"",signOffDate:""}}function mn(e){return qe[e]||{}}function Ra(e){return e===0?"config":e===1?"import":e===2?"headcount":e===3?"validate":e===4?"review":e===5?"journal":e===6?"archive":""}async function ne(e,t,n){let a=O.steps[e]||{notes:"",reviewer:"",signOffDate:""};a[t]=n,O.steps[e]=a;let o=mn(e),s=t==="notes"?o.note:t==="reviewer"?o.reviewer:o.signOff;if(s&&W())try{await Pe(s,n,ce)}catch(l){console.warn("PTO: unable to save field",s,l)}}async function hn(e,t){O.permanents[e]=t;let n=mn(e);if(n!=null&&n.note&&W())try{await Excel.run(async a=>{var u;let o=a.workbook.tables.getItemOrNullObject(ce[0]);if(await a.sync(),o.isNullObject)return;let s=o.getDataBodyRange(),l=o.getHeaderRowRange();s.load("values"),l.load("values"),await a.sync();let i=l.values[0]||[],r=i.map(d=>String(d||"").trim().toLowerCase()),c={field:r.findIndex(d=>d==="field"||d==="field name"||d==="setting"),permanent:r.findIndex(d=>d==="permanent"||d==="persist"),value:r.findIndex(d=>d==="value"||d==="setting value"),type:r.findIndex(d=>d==="type"||d==="category"),title:r.findIndex(d=>d==="title"||d==="display name")};if(c.field===-1)return;let p=(s.values||[]).findIndex(d=>String(d[c.field]||"").trim()===n.note);if(p>=0)c.permanent>=0&&(s.getCell(p,c.permanent).values=[[t?"Y":"N"]]);else{let d=new Array(i.length).fill("");c.type>=0&&(d[c.type]="Other"),c.title>=0&&(d[c.title]=""),d[c.field]=n.note,c.permanent>=0&&(d[c.permanent]=t?"Y":"N"),c.value>=0&&(d[c.value]=((u=O.steps[e])==null?void 0:u.notes)||""),o.rows.add(null,[d])}await a.sync()})}catch(a){console.warn("PTO: unable to update permanent flag",a)}}async function yn(e,t){let n=ln[e];if(n&&(O.completes[e]=t?"Y":"",!!W()))try{await Pe(n,t?"Y":"",ce)}catch(a){console.warn("PTO: unable to save completion flag",n,a)}}function en(e,t){e&&(e.classList.toggle("is-active",t),e.setAttribute("aria-pressed",String(t)))}function Aa(){let e={};return Object.keys(qe).forEach(t=>{var s;let n=parseInt(t,10),a=!!((s=O.steps[n])!=null&&s.signOffDate),o=!!O.completes[n];e[n]=a||o}),e}function vn(e,{buttonId:t,inputId:n,canActivate:a=null,onComplete:o=null}){var r;let s=document.getElementById(t);if(!s)return;let l=document.getElementById(n),i=!!((r=O.steps[e])!=null&&r.signOffDate)||!!O.completes[e];en(s,i),s.addEventListener("click",()=>{if(!s.classList.contains("is-active")&&e>0){let p=Aa(),{canComplete:u,message:d}=qt(e,p);if(!u){Yt(d);return}}if(typeof a=="function"&&!a())return;let f=!s.classList.contains("is-active");en(s,f),l&&(l.value=f?mt():"",ne(e,"signOffDate",l.value)),yn(e,f),f&&window.scrollTo({top:0,behavior:"smooth"}),f&&typeof o=="function"&&o()})}function b(e){return String(e!=null?e:"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function Na(e){return String(e!=null?e:"").replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function bn(e){let t=String(e!=null?e:"").trim().toLowerCase();return t==="true"||t==="y"||t==="yes"||t==="1"}function Ce(e){let t=String(e!=null?e:"").trim().toLowerCase();return t==="true"||t==="y"||t==="yes"||t==="1"}function gt(e){if(!e)return null;let t=/^(\d{4})-(\d{2})-(\d{2})$/.exec(String(e));if(!t)return null;let n=Number(t[1]),a=Number(t[2]),o=Number(t[3]);return!n||!a||!o?null:{year:n,month:a,day:o}}function tn(e){if(!e)return"";let t=gt(e);if(!t)return"";let{year:n,month:a,day:o}=t;return`${n}-${String(a).padStart(2,"0")}-${String(o).padStart(2,"0")}`}function $a(e){let t=gt(e);return t?`${["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"][t.month-1]} ${t.year}`:""}function Da(e){let t=gt(e);return t?`PTO-AUTO-${t.year}-${String(t.month).padStart(2,"0")}-${String(t.day).padStart(2,"0")}`:""}function mt(){let e=new Date,t=e.getFullYear(),n=String(e.getMonth()+1).padStart(2,"0"),a=String(e.getDate()).padStart(2,"0");return`${t}-${n}-${a}`}function ja(e){let t=String(e!=null?e:"").trim().toLowerCase();return t==="y"||t==="yes"||t==="true"||t==="t"||t==="1"}function La(e){if(e instanceof Date)return e.getTime();if(typeof e=="number"){let n=Ba(e);return n?n.getTime():null}let t=new Date(e);return Number.isNaN(t.getTime())?null:t.getTime()}function Ba(e){if(!Number.isFinite(e))return null;let t=new Date(Date.UTC(1899,11,30));return new Date(t.getTime()+e*24*60*60*1e3)}function Ma(){let e=n=>{var a,o;return((o=(a=document.getElementById(n))==null?void 0:a.value)==null?void 0:o.trim())||""};[{id:"config-payroll-date",field:k.payrollDate},{id:"config-accounting-period",field:k.accountingPeriod},{id:"config-journal-id",field:k.journalEntryId},{id:"config-company-name",field:k.companyName},{id:"config-payroll-provider",field:k.payrollProvider},{id:"config-accounting-link",field:k.accountingSoftware},{id:"config-user-name",field:k.reviewerName}].forEach(({id:n,field:a})=>{let o=e(n);a&&le(a,o)})}function D(e,t=[]){let n=t.map(a=>$(a));return e.findIndex(a=>n.some(o=>a.includes(o)))}function Va(e){var E,T,_,j,L,B,Q,U,q;let t=we(2),n=(t==null?void 0:t.notes)||"",a=!!O.permanents[2],o=be(t==null?void 0:t.reviewer),s=(t==null?void 0:t.signOffDate)||"",l=!!(Ce(O.completes[2])||s),i=A.roster||{},r=A.hasAnalyzed,c=(T=(E=A.roster)==null?void 0:E.difference)!=null?T:0,f=!A.skipAnalysis&&Math.abs(c)>0,p=(_=i.rosterCount)!=null?_:0,u=(j=i.payrollCount)!=null?j:0,d=(L=i.difference)!=null?L:u-p,g=Array.isArray(i.mismatches)?i.mismatches.filter(Boolean):[],y="";A.loading?y=((Q=(B=window.PrairieForge)==null?void 0:B.renderStatusBanner)==null?void 0:Q.call(B,{type:"info",message:"Analyzing headcount\u2026",escapeHtml:b}))||"":A.lastError&&(y=((q=(U=window.PrairieForge)==null?void 0:U.renderStatusBanner)==null?void 0:q.call(U,{type:"error",message:A.lastError,escapeHtml:b}))||"");let w=(I,m,v,P)=>{let N=!r,Y;N?Y='<span class="pf-je-check-circle pf-je-circle--pending"></span>':P?Y=`<span class="pf-je-check-circle pf-je-circle--pass">
    ...
    `,C=g.length&&!A.skipAnalysis&&r?window.PrairieForge.renderMismatchTiles({mismatches:g,label:"Employees Driving the Difference",sourceLabel:"Roster",targetLabel:"PTO Data",escapeHtml:b}):"";return`
        ...
        <section class="pf-step-guide">
            ...
            ${ge({textareaId:"step-notes-input",value:n,permanentId:"step-notes-lock-2",isPermanent:a,hintId:f?"headcount-notes-hint":"",saveButtonId:"step-notes-save-2"})}
    ...
    `}function Ha(){let e=ee.completenessCheck||{},t=ee.missingPayRates||[],n=[{key:"accrualRate",label:"Accrual Rate",desc:"\u2211 PTO_Data = \u2211 PTO_Analysis"},{key:"carryOver",label:"Carry Over",desc:"\u2211 PTO_Data = \u2211 PTO_Analysis"},{key:"ytdAccrued",label:"YTD Accrued",desc:"\u2211 PTO_Data = \u2211 PTO_Analysis"},{key:"ytdUsed",label:"YTD Used",desc:"\u2211 PTO_Data = \u2211 PTO_Analysis"},{key:"balance",label:"Balance",desc:"\u2211 PTO_Data = \u2211 PTO_Analysis"}],o=n.every(c=>e[c.key]!==null&&e[c.key]!==void 0)&&n.every(c=>{var f;return(f=e[c.key])==null?void 0:f.match}),s=t.length>0,l=c=>{let f=e[c.key],p=f==null,u;return p?u='<span class="pf-je-check-circle pf-je-circle--pending"></span>':f.match?u=`<span class="pf-je-check-circle pf-je-circle--pass">
        ...
        `},i=n.map(c=>l(c)).join(""),r="";if(s){let c=t[0],f=t.length-1;r=`
    ...
    `}function Fa(e){var d,g,y,w,h,C,E,T;let t=we(3),n=!!O.permanents[3],a=be(t==null?void 0:t.reviewer),o=(t==null?void 0:t.signOffDate)||"",s=!!(Ce(O.completes[3])||o),l=K.hasRun,{balanceIssues:i,zeroBalances:r,accrualOutliers:c,totalEmployees:f}=K,p="";if(K.loading)p=((g=(d=window.PrairieForge)==null?void 0:d.renderStatusBanner)==null?void 0:g.call(d,{type:"info",message:"Analyzing data quality...",escapeHtml:b}))||"";else if(l){let _=i.length,j=c.length+r.length;_>0?p=((w=(y=window.PrairieForge)==null?void 0:y.renderStatusBanner)==null?void 0:w.call(y,{type:"error",title:`${_} Balance Issue${_>1?"s":""} Found`,message:"Review the issues below. Fix in PTO_Data and re-run, or acknowledge to continue.",escapeHtml:b}))||"":j>0?p=((C=(h=window.PrairieForge)==null?void 0:h.renderStatusBanner)==null?void 0:C.call(h,{type:"warning",title:"No Critical Issues",message:`${j} informational item${j>1?"s":""} to review (see below).`,escapeHtml:b}))||"":p=((T=(E=window.PrairieForge)==null?void 0:E.renderStatusBanner)==null?void 0:T.call(E,{type:"success",title:"Data Quality Passed",message:`${f} employee${f!==1?"s":""} checked \u2014 no anomalies found.`,escapeHtml:b}))||""}let u=[];return l&&i.length>0&&u.push(`
                ...
                <article class="pf-step-card pf-step-detail">
                    ...
                    <div class="pf-quality-actions-bar">
                        ...
                        <div class="pf-signoff-action">
                            ...
                            ${K.acknowledged?"":M(`<button type="button" class="pf-action-toggle pf-clickable" id="quality-acknowledge-btn" title="Acknowledge issues and continue">${Te}</button>`,"Continue")}
                        </div>
                    </div>
                </article>
            `:""}
    ...
    `}function Ua(e){let t=we(4),n=!!O.permanents[4],a=be(t==null?void 0:t.reviewer),o=(t==null?void 0:t.signOffDate)||"",s=!!(Ce(O.completes[4])||o);return`
        <section class="pf-hero" id="pf-step-hero">
            <p class="pf-hero-copy">${b(ue)} | Step ${e.id}</p>
            <h2 class="pf-hero-title">${b(e.title)}</h2>
            <p class="pf-hero-copy">${b(e.summary||"")}</p>
    ...
    `}function Ga(e){let t=we(5),n=!!O.permanents[5],a=be(t==null?void 0:t.reviewer),o=(t==null?void 0:t.signOffDate)||"",s=!!(Ce(O.completes[5])||o),l=z.lastError?`<p class="pf-step-note">${b(z.lastError)}</p>`:"",i=z.validationRun,r=z.issues||[],c=[{key:"Debits = Credits",desc:"\u2211 Debit column = \u2211 Credit column"},{key:"Line Amounts Sum to Zero",desc:"\u2211 Line Amount = $0.00"},{key:"JE Matches Analysis Total",desc:"\u2211 Expense line amounts = \u2211 PTO_Analysis Change"}],f=g=>{let y=r.find(C=>C.check===g.key),w=!i,h;return w?h='<span class="pf-je-check-circle pf-je-circle--pending"></span>':y!=null&&y.passed?h=`<span class="pf-je-check-circle pf-je-circle--pass">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3"><polyline points="20 6 9 17 4 12"/></svg>
        ...
        `},p=c.map(g=>f(g)).join(""),u=r.filter(g=>!g.passed),d="";return i&&u.length>0&&(d=`
    ...
    `}function za(){var t,n;return Math.abs((n=(t=A.roster)==null?void 0:t.difference)!=null?n:0)>0}function wn(){return!A.skipAnalysis&&za()}async function pt(){if(!W()){A.loading=!1,A.lastError="Excel runtime is unavailable.",ie();return}A.loading=!0,A.lastError=null,de(document.getElementById("headcount-save-btn"),!1),ie();try{let e=await Excel.run(async t=>{let n=t.workbook.worksheets.getItem("SS_Employee_Roster"),a=t.workbook.worksheets.getItem("PTO_Data"),o=t.workbook.worksheets.getItemOrNullObject("PTO_Analysis"),s=n.getUsedRangeOrNullObject(),l=a.getUsedRangeOrNullObject();s.load("values"),l.load("values"),o.load("isNullObject"),await t.sync();let i=null;o.isNullObject||(i=o.getUsedRangeOrNullObject(),i.load("values")),await t.sync();let r=s.isNullObject?[]:s.values||[],c=l.isNullObject?[]:l.values||[],f=i&&!i.isNullObject?i.values||[]:[],p=f.length?f:c;return Ja(r,p)});A.roster=e.roster,A.hasAnalyzed=!0,A.lastError=null}catch(e){console.warn("PTO headcount: unable to analyze data",e),A.lastError="Unable to analyze headcount data. Try re-running the analysis."}finally{A.loading=!1,ie()}}function nn(e){if(!e)return!0;let t=e.toLowerCase().trim();return t?["total","subtotal","sum","count","grand","average","avg"].some(a=>t.includes(a)):!0}function Ja(e,t){let n={rosterCount:0,payrollCount:0,difference:0,mismatches:[]};if(((e==null?void 0:e.length)||0)<2||((t==null?void 0:t.length)||0)<2)return console.warn("Headcount: insufficient data rows",{rosterRows:(e==null?void 0:e.length)||0,payrollRows:(t==null?void 0:t.length)||0}),{roster:n};let a=an(e),o=an(t),s=a.headers,l=o.headers,i={employee:on(s),termination:s.findIndex(d=>d.includes("termination"))},r={employee:on(l)};console.log("Headcount column detection:",{rosterEmployeeCol:i.employee,rosterTerminationCol:i.termination,payrollEmployeeCol:r.employee,rosterHeaders:s.slice(0,5),payrollHeaders:l.slice(0,5)});let c=new Set,f=new Set;for(let d=a.startIndex;d<e.length;d+=1){let g=e[d],y=i.employee>=0?ve(g[i.employee]):"";nn(y)||i.termination>=0&&ve(g[i.termination])||c.add(y.toLowerCase())}for(let d=o.startIndex;d<t.length;d+=1){let g=t[d],y=r.employee>=0?ve(g[r.employee]):"";nn(y)||f.add(y.toLowerCase())}n.rosterCount=c.size,n.payrollCount=f.size,n.difference=n.payrollCount-n.rosterCount,console.log("Headcount results:",{rosterCount:n.rosterCount,payrollCount:n.payrollCount,difference:n.difference});let p=[...c].filter(d=>!f.has(d)),u=[...f].filter(d=>!c.has(d));return n.mismatches=[...p.map(d=>`In roster, missing in PTO_Data: ${d}`),...u.map(d=>`In PTO_Data, missing in roster: ${d}`)],{roster:n}}function an(e){if(!Array.isArray(e)||!e.length)return{headers:[],startIndex:1};let t=e.findIndex((o=[])=>o.some(s=>ve(s).toLowerCase().includes("employee"))),n=t===-1?0:t;return{headers:(e[n]||[]).map(o=>ve(o).toLowerCase()),startIndex:n+1}}function on(e=[]){let t=-1,n=-1;return e.forEach((a,o)=>{let s=a.toLowerCase();if(!s.includes("employee"))return;let l=1;s.includes("name")?l=4:s.includes("id")?l=2:l=3,l>n&&(n=l,t=o)}),t}function ve(e){return e==null?"":String(e).trim()}async function kn(e=null){let t=async n=>{let a=n.workbook.worksheets.getItem("PTO_Data"),o=n.workbook.worksheets.getItemOrNullObject("PTO_Analysis"),s=n.workbook.worksheets.getItemOrNullObject("SS_Employee_Roster"),l=n.workbook.worksheets.getItemOrNullObject("PR_Archive_Summary"),i=n.workbook.worksheets.getItemOrNullObject("PTO_Archive_Summary"),r=a.getUsedRangeOrNullObject();r.load("values"),o.load("isNullObject"),s.load("isNullObject"),l.load("isNullObject"),i.load("isNullObject"),await n.sync();let c=r.isNullObject?[]:r.values||[];if(!c.length)return;let f=(c[0]||[]).map(S=>$(S)),p=f.findIndex(S=>S.includes("employee")&&S.includes("name")),u=p>=0?p:0,d=D(f,["accrual rate"]),g=D(f,["carry over","carryover"]),y=f.findIndex(S=>S.includes("ytd")&&(S.includes("accrued")||S.includes("accrual"))),w=f.findIndex(S=>S.includes("ytd")&&S.includes("used")),h=D(f,["balance","current balance","pto balance"]);console.log("[PTO Analysis] PTO_Data headers:",f),console.log("[PTO Analysis] Column indices found:",{employee:u,accrualRate:d,carryOver:g,ytdAccrued:y,ytdUsed:w,balance:h}),w>=0?console.log(`[PTO Analysis] YTD Used column: "${f[w]}" at index ${w}`):console.warn("[PTO Analysis] YTD Used column NOT FOUND. Headers:",f);let C=c.slice(1).map(S=>ve(S[u])).filter(S=>S&&!S.toLowerCase().includes("total")),E=new Map;c.slice(1).forEach(S=>{let G=$(S[u]);!G||G.includes("total")||E.set(G,S)});let T=new Map;if(s.isNullObject)console.warn("[PTO Analysis] SS_Employee_Roster sheet not found");else{let S=s.getUsedRangeOrNullObject();S.load("values"),await n.sync();let G=S.isNullObject?[]:S.values||[];if(G.length){let V=(G[0]||[]).map(x=>$(x));console.log("[PTO Analysis] SS_Employee_Roster headers:",V);let H=V.findIndex(x=>x.includes("employee")&&x.includes("name"));H<0&&(H=V.findIndex(x=>x==="employee"||x==="name"||x==="full name"));let F=V.findIndex(x=>x.includes("department"));console.log(`[PTO Analysis] Roster column indices - Name: ${H}, Dept: ${F}`),H>=0&&F>=0?(G.slice(1).forEach(x=>{let re=$(x[H]),pe=ve(x[F]);re&&T.set(re,pe)}),console.log(`[PTO Analysis] Built roster map with ${T.size} employees`)):console.warn("[PTO Analysis] Could not find Name or Department columns in SS_Employee_Roster")}}let _=new Map;if(!l.isNullObject){let S=l.getUsedRangeOrNullObject();S.load("values"),await n.sync();let G=S.isNullObject?[]:S.values||[];if(G.length){let V=(G[0]||[]).map(F=>$(F)),H={payrollDate:D(V,["payroll date"]),employee:D(V,["employee"]),category:D(V,["payroll category","category"]),amount:D(V,["amount","gross salary","gross_salary","earnings"])};H.employee>=0&&H.category>=0&&H.amount>=0&&G.slice(1).forEach(F=>{let x=$(F[H.employee]);if(!x)return;let re=$(F[H.category]);if(!re.includes("regular")||!re.includes("earn"))return;let pe=Number(F[H.amount])||0;if(!pe)return;let Ee=La(F[H.payrollDate]),_e=_.get(x);(!_e||Ee!=null&&Ee>_e.timestamp)&&_.set(x,{payRate:pe/80,timestamp:Ee})})}}let j=new Map;if(!i.isNullObject){let S=i.getUsedRangeOrNullObject();S.load("values"),await n.sync();let G=S.isNullObject?[]:S.values||[];if(G.length>1){let V=(G[0]||[]).map(x=>$(x)),H=V.findIndex(x=>x.includes("employee")&&x.includes("name")),F=D(V,["liability amount","liability","accrued pto"]);H>=0&&F>=0&&G.slice(1).forEach(x=>{let re=$(x[H]);if(!re)return;let pe=Number(x[F])||0;j.set(re,pe)})}}let L=te(k.payrollDate)||"",B=[],Q=[],U=C.map((S,G)=>{var vt,bt,wt,kt,Ot,St,xt;let V=$(S),H=T.get(V)||"",F=(bt=(vt=_.get(V))==null?void 0:vt.payRate)!=null?bt:"",x=E.get(V),re=x&&d>=0&&(wt=x[d])!=null?wt:"",pe=x&&g>=0&&(kt=x[g])!=null?kt:"",Ee=x&&y>=0&&(Ot=x[y])!=null?Ot:"",_e=x&&w>=0&&(St=x[w])!=null?St:"";(V.includes("avalos")||V.includes("sarah"))&&console.log(`[PTO Debug] ${S}:`,{ytdUsedIdx:w,rawValue:x?x[w]:"no dataRow",ytdUsed:_e,fullRow:x});let Ye=x&&h>=0&&Number(x[h])||0,ht=G+2;!F&&typeof F!="number"&&B.push({name:S,rowIndex:ht}),H||Q.push({name:S,rowIndex:ht});let We=typeof F=="number"&&Ye?Ye*F:0,yt=(xt=j.get(V))!=null?xt:0,On=(typeof We=="number"?We:0)-yt;return[L,S,H,F,re,pe,Ee,_e,Ye,We,yt,On]});ee.missingPayRates=B.filter(S=>!ee.ignoredMissingPayRates.has(S.name)),ee.missingDepartments=Q,console.log(`[PTO Analysis] Data quality: ${B.length} missing pay rates, ${Q.length} missing departments`);let q=[["Analysis Date","Employee Name","Department","Pay Rate","Accrual Rate","Carry Over","YTD Accrued","YTD Used","Balance","Liability Amount","Accrued PTO $ [Prior Period]","Change"],...U],I=o.isNullObject?n.workbook.worksheets.add("PTO_Analysis"):o,m=I.getUsedRangeOrNullObject();m.load("address"),await n.sync(),m.isNullObject||m.clear();let v=q[0].length,P=q.length,N=U.length,Y=I.getRangeByIndexes(0,0,P,v);Y.values=q;let fe=I.getRangeByIndexes(0,0,1,v);dt(fe),N>0&&(Wt(I,0,N),he(I,3,N),Oe(I,4,N),Oe(I,5,N),Oe(I,6,N),Oe(I,7,N),Oe(I,8,N),he(I,9,N),he(I,10,N),he(I,11,N,!0)),Y.format.autofitColumns(),I.getRange("A1").select(),await n.sync()};W()&&(e?await t(e):await Excel.run(t))}function qa(e=[]){return e.map(t=>(t||[]).map(n=>{if(n==null)return"";let a=String(n);return/[",\n]/.test(a)?`"${a.replace(/"/g,'""')}"`:a}).join(",")).join(`
