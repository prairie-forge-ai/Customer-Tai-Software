...
(()=>{function K(){return typeof Excel!="undefined"&&typeof Excel.run=="function"}var qe="SS_PF_Config";async function kt(e,t=[qe]){var o;let n=e.workbook.tables;n.load("items/name"),await e.sync();let a=(o=n.items)==null?void 0:o.find(s=>t.includes(s.name));return a?e.workbook.tables.getItem(a.name):(console.warn("Config table not found. Looking for:",t),null)}function Ot(e){let t=e.map(n=>String(n||"").trim().toLowerCase());return{field:t.findIndex(n=>n==="field"||n==="field name"||n==="setting"),value:t.findIndex(n=>n==="value"||n==="setting value"),type:t.findIndex(n=>n==="type"||n==="category"),title:t.findIndex(n=>n==="title"||n==="display name"),permanent:t.findIndex(n=>n==="permanent"||n==="persist")}}async function xt(e=[qe]){if(!K())return{};try{return await Excel.run(async t=>{let n=await kt(t,e);if(!n)return{};let a=n.getDataBodyRange(),o=n.getHeaderRowRange();a.load("values"),o.load("values"),await t.sync();let s=o.values[0]||[],r=Ot(s);if(r.field===-1||r.value===-1)return console.warn("Config table missing FIELD or VALUE columns. Headers:",s),{};let i={};return(a.values||[]).forEach(c=>{var f;let p=String(c[r.field]||"").trim();p&&(i[p]=(f=c[r.value])!=null?f:"")}),console.log("Configuration loaded:",Object.keys(i).length,"fields"),i})}catch(t){return console.error("Failed to load configuration:",t),{}}}async function _e(e,t,n=[qe]){if(!K())return!1;try{return await Excel.run(async a=>{let o=await kt(a,n);if(!o){console.warn("Config table not found for write");return}let s=o.getDataBodyRange(),r=o.getHeaderRowRange();s.load("values"),r.load("values"),await a.sync();let i=r.values[0]||[],d=Ot(i);if(d.field===-1||d.value===-1){console.error("Config table missing FIELD or VALUE columns");return}let p=(s.values||[]).findIndex(f=>String(f[d.field]||"").trim()===e);if(p>=0)s.getCell(p,d.value).values=[[t]];else{let f=new Array(i.length).fill("");d.type>=0&&(f[d.type]="Run Settings"),f[d.field]=e,f[d.value]=t,d.permanent>=0&&(f[d.permanent]="N"),d.title>=0&&(f[d.title]=""),o.rows.add(null,[f]),console.log("Added new config row:",e,"=",t)}await a.sync(),console.log("Saved config:",e,"=",t)}),!0}catch(a){return console.error("Failed to save config:",e,a),!1}}var kn="SS_PF_Config",On="module-prefix",Ye="system",ke={PR_:"payroll-recorder",PTO_:"pto-accrual",CC_:"credit-card-expense",COM_:"commission-calc",SS_:"system"};async function St(){if(!K())return{...ke};try{return await Excel.run(async e=>{var p,f;let t=e.workbook.worksheets.getItemOrNullObject(kn);if(await e.sync(),t.isNullObject)return console.log("[Tab Visibility] Config sheet not found, using defaults"),{...ke};let n=t.getUsedRangeOrNullObject();if(n.load("values"),await e.sync(),n.isNullObject||!((p=n.values)!=null&&p.length))return{...ke};let a=n.values,o=En(a[0]),s=o.get("category"),r=o.get("field"),i=o.get("value");if(s===void 0||r===void 0||i===void 0)return console.warn("[Tab Visibility] Missing required columns, using defaults"),{...ke};let d={},c=!1;for(let u=1;u<a.length;u++){let l=a[u];if(je(l[s])===On){let y=String((f=l[r])!=null?f:"").trim().toUpperCase(),b=je(l[i]);y&&b&&(d[y]=b,c=!0)}}return c?(console.log("[Tab Visibility] Loaded prefix config:",d),d):(console.log("[Tab Visibility] No module-prefix rows found, using defaults"),{...ke})})}catch(e){return console.warn("[Tab Visibility] Error reading prefix config:",e),{...ke}}}async function We(e){if(!K())return;let t=je(e);console.log(`[Tab Visibility] Applying visibility for module: ${t}`);try{let n=await St();await Excel.run(async a=>{let o=a.workbook.worksheets;o.load("items/name,visibility"),await a.sync();let s={};for(let[u,l]of Object.entries(n))s[l]||(s[l]=[]),s[l].push(u);let r=s[t]||[],i=s[Ye]||[],d=[];for(let[u,l]of Object.entries(s))u!==t&&u!==Ye&&d.push(...l);console.log(`[Tab Visibility] Active prefixes: ${r.join(", ")}`),console.log(`[Tab Visibility] Other module prefixes (to hide): ${d.join(", ")}`),console.log(`[Tab Visibility] System prefixes (always hide): ${i.join(", ")}`);let c=[],p=[];o.items.forEach(u=>{let l=u.name,g=l.toUpperCase(),y=r.some(S=>g.startsWith(S)),b=d.some(S=>g.startsWith(S)),m=i.some(S=>g.startsWith(S));y?(c.push(u),console.log(`[Tab Visibility] SHOW: ${l} (matches active module prefix)`)):m?(p.push(u),console.log(`[Tab Visibility] HIDE: ${l} (system sheet)`)):b?(p.push(u),console.log(`[Tab Visibility] HIDE: ${l} (other module prefix)`)):console.log(`[Tab Visibility] SKIP: ${l} (no prefix match, leaving as-is)`)});for(let u of c)u.visibility=Excel.SheetVisibility.visible;if(await a.sync(),o.items.filter(u=>u.visibility===Excel.SheetVisibility.visible).length>p.length){for(let u of p)try{u.visibility=Excel.SheetVisibility.hidden}catch(l){console.warn(`[Tab Visibility] Could not hide "${u.name}":`,l.message)}await a.sync()}else console.warn("[Tab Visibility] Skipping hide - would leave no visible sheets");console.log(`[Tab Visibility] Done! Showed ${c.length}, hid ${p.length} tabs`)})}catch(n){console.warn("[Tab Visibility] Error applying visibility:",n)}}async function xn(){if(!K()){console.log("Excel not available");return}try{await Excel.run(async e=>{let t=e.workbook.worksheets;t.load("items/name,visibility"),await e.sync();let n=0;t.items.forEach(a=>{a.visibility!==Excel.SheetVisibility.visible&&(a.visibility=Excel.SheetVisibility.visible,console.log(`[ShowAll] Made visible: ${a.name}`),n++)}),await e.sync(),console.log(`[ShowAll] Done! Made ${n} sheets visible. Total: ${t.items.length}`)})}catch(e){console.error("[Tab Visibility] Unable to show all sheets:",e)}}async function Sn(){if(!K()){console.log("Excel not available");return}try{let e=await St(),t=[];for(let[n,a]of Object.entries(e))a===Ye&&t.push(n);await Excel.run(async n=>{let a=n.workbook.worksheets;a.load("items/name,visibility"),await n.sync(),a.items.forEach(o=>{let s=o.name.toUpperCase();t.some(r=>s.startsWith(r))&&(o.visibility=Excel.SheetVisibility.visible,console.log(`[Unhide] Made visible: ${o.name}`))}),await n.sync(),console.log("[Unhide] System sheets are now visible!")})}catch(e){console.error("[Tab Visibility] Unable to unhide system sheets:",e)}}function En(e=[]){let t=new Map;return e.forEach((n,a)=>{let o=je(n);o&&t.set(o,a)}),t}function je(e){return String(e!=null?e:"").trim().toLowerCase().replace(/[\s_]+/g,"-")}typeof window!="undefined"&&(window.PrairieForge=window.PrairieForge||{},window.PrairieForge.showAllSheets=xn,window.PrairieForge.unhideSystemSheets=Sn,window.PrairieForge.applyModuleTabVisibility=We);var Et={COMPANY_NAME:"Prairie Forge LLC",PRODUCT_NAME:"Prairie Forge Tools",SUPPORT_URL:"https://prairieforge.ai/support",ADA_IMAGE_URL:"https://assets.prairieforge.ai/storage/v1/object/public/Other%20Public%20Material/Prairie%20Forge/Ada%20Image.png"};var Pt=Et.ADA_IMAGE_URL;async function Le(e,t,n){if(typeof Excel=="undefined"){console.warn("Excel runtime not available for homepage sheet");return}try{await Excel.run(async a=>{let o=a.workbook.worksheets.getItemOrNullObject(e);o.load("isNullObject, name, visibility"),await a.sync();let s;o.isNullObject?(s=a.workbook.worksheets.add(e),await a.sync(),await Ct(a,s,t,n)):(s=o,s.visibility!==Excel.SheetVisibility.visible&&(s.visibility=Excel.SheetVisibility.visible,await a.sync()),await Ct(a,s,t,n)),s.activate(),s.getRange("A1").select(),await a.sync()})}catch(a){console.error(`Error activating homepage sheet ${e}:`,a)}}async function Ct(e,t,n,a){try{let c=t.getUsedRangeOrNullObject();c.load("isNullObject"),await e.sync(),c.isNullObject||(c.clear(),await e.sync())}catch{}t.showGridlines=!1,t.getRange("A:A").format.columnWidth=400,t.getRange("B:B").format.columnWidth=50,t.getRange("1:1").format.rowHeight=60,t.getRange("2:2").format.rowHeight=30;let o=[[n,""],[a,""],["",""],["",""]],s=t.getRangeByIndexes(0,0,4,2);s.values=o;let r=t.getRange("A1:Z100");r.format.fill.color="#0f0f0f";let i=t.getRange("A1");i.format.font.bold=!0,i.format.font.size=36,i.format.font.color="#ffffff",i.format.font.name="Segoe UI Light",i.format.verticalAlignment="Center";let d=t.getRange("A2");d.format.font.size=14,d.format.font.color="#a0a0a0",d.format.font.name="Segoe UI",d.format.verticalAlignment="Top",t.freezePanes.freezeRows(0),t.freezePanes.freezeColumns(0),await e.sync()}var _t={"module-selector":{sheetName:"SS_Homepage",title:"ForgeSuite",subtitle:"Select a module from the side panel to get started."},"payroll-recorder":{sheetName:"PR_Homepage",title:"Payroll Recorder",subtitle:"Normalize payroll exports, enforce controls, and prep journal entries without leaving Excel."},"pto-accrual":{sheetName:"PTO_Homepage",title:"PTO Accrual",subtitle:"Calculate employee PTO liabilities, compare period-over-period changes, and prepare accrual journal entries."}};function Be(e){return _t[e]||_t["module-selector"]}function Tt(){Qe();let e=document.createElement("button");return e.className="pf-ada-fab",e.id="pf-ada-fab",e.setAttribute("aria-label","Ask Ada"),e.setAttribute("title","Ask Ada"),e.innerHTML=`
        ...
        />
    `,document.body.appendChild(e),e.addEventListener("click",Cn),e}function Qe(){let e=document.getElementById("pf-ada-fab");e&&e.remove();let t=document.getElementById("pf-ada-modal-overlay");t&&t.remove()}function Cn(){let e=document.getElementById("pf-ada-modal-overlay");e&&e.remove();let t=document.createElement("div");t.className="pf-ada-modal-overlay",t.id="pf-ada-modal-overlay",t.innerHTML=`
    ...
    `,document.body.appendChild(t),requestAnimationFrame(()=>{t.classList.add("is-visible")});let n=document.getElementById("ada-modal-close");n==null||n.addEventListener("click",Ke),t.addEventListener("click",o=>{o.target===t&&Ke()});let a=o=>{o.key==="Escape"&&(Ke(),document.removeEventListener("keydown",a))};document.addEventListener("keydown",a)}function Ke(){let e=document.getElementById("pf-ada-modal-overlay");e&&(e.classList.remove("is-visible"),setTimeout(()=>{e.remove()},300))}var _n=["January","February","March","April","May","June","July","August","September","October","November","December"],Rt=["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"],Pn=["Su","Mo","Tu","We","Th","Fr","Sa"],Q=null,ae=null;function At(e,t={}){let n=document.getElementById(e);if(!n)return;let{onChange:a=null,minDate:o=null,maxDate:s=null,readonly:r=!1}=t,i=n.closest(".pf-datepicker-wrapper");i||(i=document.createElement("div"),i.className="pf-datepicker-wrapper",n.parentNode.insertBefore(i,n),i.appendChild(n)),n.type="text",n.placeholder="Select date...",n.classList.add("pf-datepicker-input"),n.readOnly=!0;let d=n.value?It(n.value):null;d&&(n.value=Ze(d),n.dataset.value=Pe(d));let c=i.querySelector(".pf-datepicker-icon");c||(c=document.createElement("span"),c.className="pf-datepicker-icon",c.innerHTML='<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="18" height="18" x="3" y="4" rx="2" ry="2"/><line x1="16" x2="16" y1="2" y2="6"/><line x1="8" x2="8" y1="2" y2="6"/><line x1="3" x2="21" y1="10" y2="10"/></svg>',i.appendChild(c));let p={inputId:e,input:n,selectedDate:d,viewDate:d?new Date(d):new Date,onChange:a,minDate:o,maxDate:s};function f(){r||(ae=p,Tn())}return n.addEventListener("click",f),c.addEventListener("click",f),{getValue:()=>p.selectedDate?Pe(p.selectedDate):"",setValue:u=>{let l=It(u);p.selectedDate=l,p.viewDate=l?new Date(l):new Date,l?(n.value=Ze(l),n.dataset.value=Pe(l)):(n.value="",n.dataset.value="")},open:f,close:Me}}function Tn(){ae&&(Q||(Q=document.createElement("div"),Q.className="pf-datepicker-modal",Q.id="pf-datepicker-modal",document.body.appendChild(Q)),Dt(),requestAnimationFrame(()=>{Q.classList.add("is-open")}),document.addEventListener("keydown",Nt))}function Me(){Q&&Q.classList.remove("is-open"),document.removeEventListener("keydown",Nt),ae=null}function Nt(e){e.key==="Escape"&&Me()}function Dt(){if(!Q||!ae)return;let{viewDate:e,selectedDate:t,minDate:n,maxDate:a}=ae,o=e.getFullYear(),s=e.getMonth();Q.innerHTML=`
        ...
        <div class="pf-datepicker-container">
            <div class="pf-datepicker-header">
                ...
                <span class="pf-datepicker-title">${_n[s]} ${o}</span>
                <div class="pf-datepicker-nav-group">
    ...
    `,In()}function In(){var e;Q&&((e=Q.querySelector(".pf-datepicker-backdrop"))==null||e.addEventListener("click",Me),Q.querySelectorAll(".pf-datepicker-nav").forEach(t=>{t.addEventListener("click",n=>{n.preventDefault(),n.stopPropagation();let a=t.dataset.action;Rn(a)})}),Q.querySelectorAll(".pf-datepicker-day:not(.disabled)").forEach(t=>{t.addEventListener("click",n=>{n.preventDefault(),n.stopPropagation();let a=parseInt(t.dataset.day),o=parseInt(t.dataset.month),s=parseInt(t.dataset.year);Xe(new Date(s,o,a))})}),Q.querySelectorAll(".pf-datepicker-btn").forEach(t=>{t.addEventListener("click",n=>{n.preventDefault(),n.stopPropagation();let a=t.dataset.action;a==="today"?Xe(new Date):a==="clear"&&Xe(null)})}))}function Rn(e){if(!ae)return;let t=ae.viewDate;switch(e){case"prev-year":t.setFullYear(t.getFullYear()-1);break;case"prev-month":t.setMonth(t.getMonth()-1);break;case"next-month":t.setMonth(t.getMonth()+1);break;case"next-year":t.setFullYear(t.getFullYear()+1);break}Dt()}function Xe(e){if(!ae)return;let{input:t,onChange:n}=ae;ae.selectedDate=e,e?(t.value=Ze(e),t.dataset.value=Pe(e),ae.viewDate=new Date(e)):(t.value="",t.dataset.value=""),n&&n(e?Pe(e):""),t.dispatchEvent(new Event("change",{bubbles:!0})),Me()}function An(e,t,n,a,o){let s=new Date(e,t,1).getDay(),r=new Date(e,t+1,0).getDate(),i=new Date(e,t,0).getDate(),d=new Date;d.setHours(0,0,0,0),n&&(n=new Date(n),n.setHours(0,0,0,0));let c="";for(let l=s-1;l>=0;l--){let g=i-l,y=t===0?11:t-1,b=t===0?e-1:e;c+=`<button type="button" class="pf-datepicker-day other-month" data-day="${g}" data-month="${y}" data-year="${b}">${g}</button>`}for(let l=1;l<=r;l++){let g=new Date(e,t,l);g.setHours(0,0,0,0);let y=g.getTime()===d.getTime(),b=n&&g.getTime()===n.getTime(),m="pf-datepicker-day";y&&(m+=" today"),b&&(m+=" selected");let S=!1;a&&g<a&&(S=!0),o&&g>o&&(S=!0),S&&(m+=" disabled"),c+=`<button type="button" class="${m}" data-day="${l}" data-month="${t}" data-year="${e}" ${S?"disabled":""}>${l}</button>`}let p=42,f=s+r,u=p-f;for(let l=1;l<=u;l++){let g=t===11?0:t+1,y=t===11?e+1:e;c+=`<button type="button" class="pf-datepicker-day other-month" data-day="${l}" data-month="${g}" data-year="${y}">${l}</button>`}return c}function It(e){if(!e)return null;if(/^\d{4}-\d{2}-\d{2}$/.test(e)){let[a,o,s]=e.split("-").map(Number);return new Date(a,o-1,s)}let t=e.match(/^(\w+)\s+(\d+),\s+(\d{4})$/);if(t){let a=Rt.findIndex(o=>o.toLowerCase()===t[1].toLowerCase().substring(0,3));if(a>=0)return new Date(parseInt(t[3]),a,parseInt(t[2]))}if(/^\d{1,2}\/\d{1,2}\/\d{4}$/.test(e)){let[a,o,s]=e.split("/").map(Number);return new Date(s,a-1,o)}let n=new Date(e);return isNaN(n.getTime())?null:n}function Ze(e){return e?`${Rt[e.getMonth()]} ${e.getDate()}, ${e.getFullYear()}`:""}function Pe(e){if(!e)return"";let t=e.getFullYear(),n=String(e.getMonth()+1).padStart(2,"0"),a=String(e.getDate()).padStart(2,"0");return`${t}-${n}-${a}`}var $t=`
    <svg
...
`.trim(),Nn={config:`
    ...
    `};function Bt(e){return e&&Nn[e]||""}var et=`
    <svg
...
`.trim();function Re(e){return e==null?"":String(e).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function G(e,t){return`
    ...
    `}function pe({textareaId:e,value:t,permanentId:n,isPermanent:a,hintId:o,saveButtonId:s,isSaved:r=!1,placeholder:i="Enter notes here..."}){let d=a?tt:et,c=s?`<button type="button" class="pf-action-toggle pf-save-btn ${r?"is-saved":""}" id="${s}" data-save-input="${e}" title="Save notes">${Ft}</button>`:"",p=n?`<button type="button" class="pf-action-toggle pf-notes-lock ${a?"is-locked":""}" id="${n}" aria-pressed="${a}" title="Lock notes (retain after archive)">${d}</button>`:"";return`
    ...
    `}function fe({reviewerInputId:e,reviewerValue:t,signoffInputId:n,signoffValue:a,isComplete:o,saveButtonId:s,isSaved:r=!1,completeButtonId:i,subtext:d="Sign-off below. Click checkmark icon. Done."}){let c=`<button type="button" class="pf-action-toggle ${o?"is-active":""}" id="${i}" aria-pressed="${!!o}" title="Mark step complete">${Te}</button>`;return`
    ...
    `,!document.getElementById("pf-toast-styles")){let o=document.createElement("style");o.id="pf-toast-styles",o.textContent=`
        ...
        `,document.head.appendChild(o)}return document.body.appendChild(a),n>0&&setTimeout(()=>a.remove(),n),a}function $n(e,t={}){let{title:n="Confirm Action",confirmText:a="Continue",cancelText:o="Cancel",icon:s="\u{1F4CB}",destructive:r=!1}=t;return new Promise(i=>{document.querySelectorAll(".pf-confirm-overlay").forEach(c=>c.remove());let d=document.createElement("div");if(d.className="pf-confirm-overlay",d.innerHTML=`
    ...
    `;let s=D.activeView==="home"||D.activeView!=="step"&&D.activeView!=="config",r=document.getElementById("pf-info-fab-pto");if(s)r&&r.remove();else if((i=window.PrairieForge)!=null&&i.mountInfoFab){let d=Yn(D.activeStepId);PrairieForge.mountInfoFab({title:d.title,content:d.content,buttonId:"pf-info-fab-pto"})}ta(),sa(),s?Tt():Qe()}function Yn(e){switch(e){case 0:return{title:"Configuration",content:`
        ...
        <section class="pf-hero" id="pf-hero">
            ...
            <p class="pf-hero-copy">${jn}</p>
        </section>
    `}function Kn(){return`
        <section class="pf-step-guide">
            <div class="pf-step-grid">
                ${ne.map((e,t)=>Qn(e,t)).join("")}
            </div>
        </section>
    `}function Qn(e,t){let n=D.stepStatuses[e.id]||"pending",a=D.activeView==="step"&&D.focusedIndex===t?"pf-step-card--active":"",o=Bt(va(e.id));return`
    ...
    `}function Xn(e){let t=ne.filter(o=>o.id!==6).map(o=>({id:o.id,title:o.title,complete:ia(o.id)})),n=t.every(o=>o.complete),a=t.map(o=>`
            <article class="pf-step-card pf-step-detail pf-config-card">
                <div class="pf-config-head pf-notes-header">
    ...
    `}function ln(){if(!O.loaded)return`
        ...
        `;let e=en(le(w.payrollDate)),t=en(le(w.accountingPeriod)),n=le(w.journalEntryId),a=le(w.accountingSoftware),o=le(w.payrollProvider),s=le(w.companyName),r=le(w.reviewerName),i=we(0),d=!!O.permanents[0],c=!!(yn(O.completes[0])||i.signOffDate),p=be(i==null?void 0:i.reviewer),f=(i==null?void 0:i.signOffDate)||"";return`
        ...
        <section class="pf-step-guide">
            ...
            ${pe({textareaId:"config-notes",value:i.notes||"",permanentId:"config-notes-lock",isPermanent:d,hintId:"",saveButtonId:"config-notes-save"})}
            ${fe({reviewerInputId:"config-reviewer",reviewerValue:p,signoffInputId:"config-signoff-date",signoffValue:f,isComplete:c,saveButtonId:"config-signoff-save",completeButtonId:"config-signoff-toggle"})}
        </section>
    `}function Zn(e){let t=we(1),n=!!O.permanents[1],a=be(t==null?void 0:t.reviewer),o=(t==null?void 0:t.signOffDate)||"",s=!!(Se(O.completes[1])||o),r=le(w.payrollProvider);return`
        <section class="pf-hero" id="pf-step-hero">
            <p class="pf-hero-copy">${v(he)} | Step ${e.id}</p>
            <h2 class="pf-hero-title">${v(e.title)}</h2>
            <p class="pf-hero-copy">${v(e.summary||"")}</p>
    ...
    `}function ea(e){let t=ne.find(i=>i.id===e);if(!t)return"";if(e===0)return ln();if(e===1)return Zn(t);if(e===2)return _a(t);if(e===3)return Ta(t);if(e===4)return Ia(t);if(e===5)return Ra(t);if(t.id===6)return Xn(t);let n=we(e),a=!!O.permanents[e],o=be(n==null?void 0:n.reviewer),s=(n==null?void 0:n.signOffDate)||"",r=!!(Se(O.completes[e])||s);return`
        <section class="pf-hero" id="pf-step-hero">
    ...
    `}function ta(){var n,a,o,s,r;(n=document.getElementById("nav-home"))==null||n.addEventListener("click",async()=>{var d;let i=Be(Ne);await Le(i.sheetName,i.title,i.subtitle),$e({activeView:"home",activeStepId:null}),(d=document.getElementById("pf-hero"))==null||d.scrollIntoView({behavior:"smooth",block:"start"})}),(a=document.getElementById("nav-selector"))==null||a.addEventListener("click",()=>{window.location.href=Ln}),(o=document.getElementById("nav-prev"))==null||o.addEventListener("click",()=>Yt(-1)),(s=document.getElementById("nav-next"))==null||s.addEventListener("click",()=>Yt(1));let e=document.getElementById("nav-quick-toggle"),t=document.getElementById("quick-access-dropdown");e==null||e.addEventListener("click",i=>{i.stopPropagation(),t==null||t.classList.toggle("hidden"),e.classList.toggle("is-active")}),document.addEventListener("click",i=>{!(t!=null&&t.contains(i.target))&&!(e!=null&&e.contains(i.target))&&(t==null||t.classList.add("hidden"),e==null||e.classList.remove("is-active"))}),(r=document.getElementById("nav-config"))==null||r.addEventListener("click",async()=>{t==null||t.classList.add("hidden"),e==null||e.classList.remove("is-active"),await ha()}),document.querySelectorAll("[data-step-card]").forEach(i=>{let d=Number(i.getAttribute("data-step-index")),c=Number(i.getAttribute("data-step-id"));i.addEventListener("click",()=>De(d,c))}),D.activeView==="config"?aa():D.activeView==="step"&&D.activeStepId!=null&&na(D.activeStepId)}function na(e){var p,f,u,l,g,y,b,m,S,E,R,C,j,L,B,W,F;let t=e===2?document.getElementById("step-notes-input"):document.getElementById(`step-notes-${e}`),n=e===2?document.getElementById("step-reviewer-name"):document.getElementById(`step-reviewer-${e}`),a=e===2?document.getElementById("step-signoff-date"):document.getElementById(`step-signoff-${e}`),o=document.getElementById("step-back-btn"),s=e===2?document.getElementById("step-notes-lock-2"):document.getElementById(`step-notes-lock-${e}`),r=e===2?document.getElementById("step-notes-save-2"):document.getElementById(`step-notes-save-${e}`);r==null||r.addEventListener("click",async()=>{let P=(t==null?void 0:t.value)||"";await te(e,"notes",P),ce(r,!0)});let i=e===2?document.getElementById("headcount-signoff-save"):document.getElementById(`step-signoff-save-${e}`);i==null||i.addEventListener("click",async()=>{let P=(n==null?void 0:n.value)||"";await te(e,"reviewer",P),ce(i,!0)}),ot();let d=e===2?"headcount-signoff-toggle":`step-signoff-toggle-${e}`,c=e===2?"step-signoff-date":`step-signoff-${e}`;hn(e,{buttonId:d,inputId:c,canActivate:e===2?()=>{var T;return!vn()||((T=document.getElementById("step-notes-input"))==null?void 0:T.value.trim())||""?!0:(Z("Please enter a brief explanation of the headcount differences before completing this step.","info"),!1)}:null,onComplete:oa(e)}),o==null||o.addEventListener("click",async()=>{let P=Be(Ne);await Le(P.sheetName,P.title,P.subtitle),$e({activeView:"home",activeStepId:null})}),s==null||s.addEventListener("click",async()=>{let P=!s.classList.contains("is-locked");at(s,P),await gn(e,P)}),e===6&&((p=document.getElementById("archive-run-btn"))==null||p.addEventListener("click",()=>{})),e===1&&((f=document.getElementById("import-open-data-btn"))==null||f.addEventListener("click",()=>pn("PTO_Data")),(u=document.getElementById("import-clear-btn"))==null||u.addEventListener("click",()=>fa())),e===2&&((l=document.getElementById("headcount-skip-btn"))==null||l.addEventListener("click",()=>{A.skipAnalysis=!A.skipAnalysis;let P=document.getElementById("headcount-skip-btn");P==null||P.classList.toggle("is-active",A.skipAnalysis),A.skipAnalysis&&sn(),on()}),(g=document.getElementById("headcount-run-btn"))==null||g.addEventListener("click",()=>ct()),(y=document.getElementById("headcount-refresh-btn"))==null||y.addEventListener("click",()=>ct()),ja(),A.skipAnalysis&&sn(),on()),e===3&&((b=document.getElementById("quality-run-btn"))==null||b.addEventListener("click",()=>Kt()),(m=document.getElementById("quality-refresh-btn"))==null||m.addEventListener("click",()=>Kt()),(S=document.getElementById("quality-acknowledge-btn"))==null||S.addEventListener("click",()=>la())),e===4&&((E=document.getElementById("analysis-refresh-btn"))==null||E.addEventListener("click",()=>Qt()),(R=document.getElementById("analysis-run-btn"))==null||R.addEventListener("click",()=>Qt()),(C=document.getElementById("payrate-save-btn"))==null||C.addEventListener("click",Wt),(j=document.getElementById("payrate-ignore-btn"))==null||j.addEventListener("click",ra),(L=document.getElementById("payrate-input"))==null||L.addEventListener("keydown",P=>{P.key==="Enter"&&Wt()})),e===5&&((B=document.getElementById("je-create-btn"))==null||B.addEventListener("click",()=>ua()),(W=document.getElementById("je-run-btn"))==null||W.addEventListener("click",()=>un()),(F=document.getElementById("je-export-btn"))==null||F.addEventListener("click",()=>pa()))}function aa(){var i,d,c,p,f;At("config-payroll-date",{onChange:u=>{if(re(w.payrollDate,u),!!u){if(!O.overrides.accountingPeriod){let l=ka(u);if(l){let g=document.getElementById("config-accounting-period");g&&(g.value=l),re(w.accountingPeriod,l)}}if(!O.overrides.journalId){let l=Oa(u);if(l){let g=document.getElementById("config-journal-id");g&&(g.value=l),re(w.journalEntryId,l)}}}}});let e=document.getElementById("config-accounting-period");e==null||e.addEventListener("change",u=>{O.overrides.accountingPeriod=!!u.target.value,re(w.accountingPeriod,u.target.value||"")});let t=document.getElementById("config-journal-id");t==null||t.addEventListener("change",u=>{O.overrides.journalId=!!u.target.value,re(w.journalEntryId,u.target.value.trim())}),(i=document.getElementById("config-company-name"))==null||i.addEventListener("change",u=>{re(w.companyName,u.target.value.trim())}),(d=document.getElementById("config-payroll-provider"))==null||d.addEventListener("change",u=>{re(w.payrollProvider,u.target.value.trim())}),(c=document.getElementById("config-accounting-link"))==null||c.addEventListener("change",u=>{re(w.accountingSoftware,u.target.value.trim())}),(p=document.getElementById("config-user-name"))==null||p.addEventListener("change",u=>{re(w.reviewerName,u.target.value.trim())});let n=document.getElementById("config-notes");n==null||n.addEventListener("input",u=>{te(0,"notes",u.target.value)});let a=document.getElementById("config-notes-lock");a==null||a.addEventListener("click",async()=>{let u=!a.classList.contains("is-locked");at(a,u),await gn(0,u)});let o=document.getElementById("config-notes-save");o==null||o.addEventListener("click",async()=>{n&&(await te(0,"notes",n.value),ce(o,!0))});let s=document.getElementById("config-reviewer");s==null||s.addEventListener("change",u=>{let l=u.target.value.trim();te(0,"reviewer",l);let g=document.getElementById("config-signoff-date");if(l&&g&&!g.value){let y=ut();g.value=y,te(0,"signOffDate",y),mn(0,!0)}}),(f=document.getElementById("config-signoff-date"))==null||f.addEventListener("change",u=>{te(0,"signOffDate",u.target.value||"")});let r=document.getElementById("config-signoff-save");r==null||r.addEventListener("click",async()=>{var g,y;let u=((g=s==null?void 0:s.value)==null?void 0:g.trim())||"",l=((y=document.getElementById("config-signoff-date"))==null?void 0:y.value)||"";await te(0,"reviewer",u),await te(0,"signOffDate",l),ce(r,!0)}),ot(),hn(0,{buttonId:"config-signoff-toggle",inputId:"config-signoff-date",onComplete:()=>{Ca(),cn(0),dn()}})}function De(e,t=null){if(e<0||e>=ne.length)return;Ue=e;let n=t!=null?t:ne[e].id;$e({focusedIndex:e,activeView:n===0?"config":"step",activeStepId:n});let o=Mn[n];o&&pn(o),n===2&&!A.hasAnalyzed&&(bn(),ct())}function oa(e){return e===6?null:()=>cn(e)}function cn(e){let t=ne.findIndex(a=>a.id===e);if(t===-1)return;let n=t+1;n<ne.length&&(De(n,ne[n].id),dn())}function dn(){let e=[document.querySelector(".pf-root"),document.querySelector(".pf-step-guide"),document.body];for(let t of e)t&&t.scrollTo({top:0,behavior:"smooth"});window.scrollTo({top:0,behavior:"smooth"})}function Yt(e){let t=D.focusedIndex+e,n=Math.max(0,Math.min(ne.length-1,t));De(n,ne[n].id)}function sa(){if(Ue===null)return;let e=document.querySelector(`[data-step-index="${Ue}"]`);Ue=null,e==null||e.scrollIntoView({behavior:"smooth",block:"center"})}function ia(e){return yn(O.completes[e])}function $e(e){e.stepStatuses&&(D.stepStatuses={...D.stepStatuses,...e.stepStatuses}),Object.assign(D,{...e,stepStatuses:D.stepStatuses}),oe()}function se(){return typeof Excel!="undefined"&&typeof Excel.run=="function"}async function Wt(){let e=document.getElementById("payrate-input");if(!e)return;let t=parseFloat(e.value),n=e.dataset.employee,a=parseInt(e.dataset.row,10);if(isNaN(t)||t<=0){Z("Please enter a valid pay rate greater than 0.","info");return}if(!n||isNaN(a)){console.error("Missing employee data on input");return}ee(!0,"Updating pay rate...");try{await Excel.run(async o=>{let s=o.workbook.worksheets.getItem("PTO_Analysis"),r=s.getCell(a-1,3);r.values=[[t]];let i=s.getCell(a-1,8);i.load("values"),await o.sync();let c=(Number(i.values[0][0])||0)*t,p=s.getCell(a-1,9);p.values=[[c]];let f=s.getCell(a-1,10);f.load("values"),await o.sync();let u=Number(f.values[0][0])||0,l=c-u,g=s.getCell(a-1,11);g.values=[[l]],await o.sync()}),X.missingPayRates=X.missingPayRates.filter(o=>o.name!==n),ee(!1),De(3,3)}catch(o){console.error("Failed to save pay rate:",o),Z(`Failed to save pay rate: ${o.message}`,"error"),ee(!1)}}function ra(){let e=document.getElementById("payrate-input");if(!e)return;let t=e.dataset.employee;t&&(X.ignoredMissingPayRates.add(t),X.missingPayRates=X.missingPayRates.filter(n=>n.name!==t)),De(3,3)}async function Kt(){if(!se()){Z("Excel is not available. Open this module inside Excel to run quality check.","info");return}Y.loading=!0,ee(!0,"Analyzing data quality..."),ce(document.getElementById("quality-save-btn"),!1);try{await Excel.run(async t=>{var b;let a=t.workbook.worksheets.getItem("PTO_Data").getUsedRangeOrNullObject();a.load("values"),await t.sync();let o=a.isNullObject?[]:a.values||[];if(!o.length||o.length<2)throw new Error("PTO_Data is empty or has no data rows.");let s=(o[0]||[]).map(m=>z(m));console.log("[Data Quality] PTO_Data headers:",o[0]);let r=s.findIndex(m=>m==="employee name"||m==="employeename");r===-1&&(r=s.findIndex(m=>m.includes("employee")&&m.includes("name"))),r===-1&&(r=s.findIndex(m=>m==="name"||m.includes("name")&&!m.includes("company")&&!m.includes("form"))),console.log("[Data Quality] Employee name column index:",r,"Header:",(b=o[0])==null?void 0:b[r]);let i=$(s,["balance"]),d=$(s,["accrual rate","accrualrate"]),c=$(s,["carry over","carryover"]),p=$(s,["ytd accrued","ytdaccrued"]),f=$(s,["ytd used","ytdused"]),u=[],l=[],g=[],y=o.slice(1);y.forEach((m,S)=>{let E=S+2,R=r!==-1?String(m[r]||"").trim():`Row ${E}`;if(!R)return;let C=i!==-1&&Number(m[i])||0,j=d!==-1&&Number(m[d])||0,L=c!==-1&&Number(m[c])||0,B=p!==-1&&Number(m[p])||0,W=f!==-1&&Number(m[f])||0,F=L+B;C<0?u.push({name:R,issue:`Negative balance: ${C.toFixed(2)} hrs`,rowIndex:E}):W>F&&F>0&&u.push({name:R,issue:`Used ${W.toFixed(0)} hrs but only ${F.toFixed(0)} available`,rowIndex:E}),C===0&&(L>0||B>0)&&l.push({name:R,rowIndex:E}),j>8&&g.push({name:R,accrualRate:j,rowIndex:E})}),Y.balanceIssues=u,Y.zeroBalances=l,Y.accrualOutliers=g,Y.totalIssues=u.length,Y.totalEmployees=y.filter(m=>m.some(S=>S!==null&&S!=="")).length,Y.hasRun=!0});let e=Y.balanceIssues.length>0;$e({stepStatuses:{3:e?"blocked":"complete"}})}catch(e){console.error("Data quality check error:",e),Z(`Quality check failed: ${e.message}`,"error"),Y.hasRun=!1}finally{Y.loading=!1,ee(!1),oe()}}function la(){Y.acknowledged=!0,$e({stepStatuses:{3:"complete"}}),oe()}async function ca(){if(se())try{await Excel.run(async e=>{let t=e.workbook.worksheets.getItem("PTO_Data"),n=e.workbook.worksheets.getItemOrNullObject("PTO_Analysis"),a=t.getUsedRangeOrNullObject();if(a.load("values"),n.load("isNullObject"),await e.sync(),n.isNullObject){X.completenessCheck={accrualRate:null,carryOver:null,ytdAccrued:null,ytdUsed:null,balance:null};return}let o=n.getUsedRangeOrNullObject();o.load("values"),await e.sync();let s=a.isNullObject?[]:a.values||[],r=o.isNullObject?[]:o.values||[];if(!s.length||!r.length){X.completenessCheck={accrualRate:null,carryOver:null,ytdAccrued:null,ytdUsed:null,balance:null};return}let i=(p,f,u)=>{let l=(p[0]||[]).map(b=>z(b)),g=$(l,f);return g===-1?null:p.slice(1).reduce((b,m)=>b+(Number(m[g])||0),0)},d=[{key:"accrualRate",aliases:["accrual rate","accrualrate"]},{key:"carryOver",aliases:["carry over","carryover","carry_over"]},{key:"ytdAccrued",aliases:["ytd accrued","ytdaccrued","ytd_accrued"]},{key:"ytdUsed",aliases:["ytd used","ytdused","ytd_used"]},{key:"balance",aliases:["balance"]}],c={};for(let p of d){let f=i(s,p.aliases,"PTO_Data"),u=i(r,p.aliases,"PTO_Analysis");if(f===null||u===null)c[p.key]=null;else{let l=Math.abs(f-u)<.01;c[p.key]={match:l,ptoData:f,ptoAnalysis:u}}}X.completenessCheck=c})}catch(e){console.error("Completeness check failed:",e)}}async function Qt(){if(!se()){Z("Excel is not available. Open this module inside Excel to run analysis.","info");return}ee(!0,"Running analysis...");try{await bn(),await ca(),X.cleanDataReady=!0,oe()}catch(e){console.error("Full analysis error:",e),Z(`Analysis failed: ${e.message}`,"error")}finally{ee(!1)}}async function un(){if(!se()){Z("Excel is not available. Open this module inside Excel to run journal checks.","info");return}J.loading=!0,J.lastError=null,ce(document.getElementById("je-save-btn"),!1),oe();try{let e=await Excel.run(async t=>{let a=t.workbook.worksheets.getItem("PTO_JE_Draft").getUsedRangeOrNullObject();a.load("values");let o=t.workbook.worksheets.getItemOrNullObject("PTO_Analysis");o.load("isNullObject"),await t.sync();let s=a.isNullObject?[]:a.values||[];if(!s.length)throw new Error("PTO_JE_Draft is empty. Generate the JE first.");let r=(s[0]||[]).map(E=>z(E)),i=$(r,["debit"]),d=$(r,["credit"]),c=$(r,["lineamount","line amount"]),p=$(r,["account number","accountnumber"]);if(i===-1||d===-1)throw new Error("Could not find Debit and Credit columns in PTO_JE_Draft.");let f=0,u=0,l=0,g=0;s.slice(1).forEach(E=>{let R=Number(E[i])||0,C=Number(E[d])||0,j=c!==-1&&Number(E[c])||0,L=p!==-1?String(E[p]||"").trim():"";f+=R,u+=C,l+=j,L&&L!=="21540"&&(g+=j)});let y=0;if(!o.isNullObject){let E=o.getUsedRangeOrNullObject();E.load("values"),await t.sync();let R=E.isNullObject?[]:E.values||[];if(R.length>1){let C=(R[0]||[]).map(L=>z(L)),j=$(C,["change"]);j!==-1&&R.slice(1).forEach(L=>{y+=Number(L[j])||0})}}let b=f-u,m=[];Math.abs(b)>=.01?m.push({check:"Debits = Credits",passed:!1,detail:b>0?`Debits exceed credits by $${Math.abs(b).toLocaleString(void 0,{minimumFractionDigits:2})}`:`Credits exceed debits by $${Math.abs(b).toLocaleString(void 0,{minimumFractionDigits:2})}`}):m.push({check:"Debits = Credits",passed:!0,detail:""}),Math.abs(l)>=.01?m.push({check:"Line Amounts Sum to Zero",passed:!1,detail:`Line amounts sum to $${l.toLocaleString(void 0,{minimumFractionDigits:2})} (should be $0.00)`}):m.push({check:"Line Amounts Sum to Zero",passed:!0,detail:""});let S=Math.abs(g-y);return S>=.01?m.push({check:"JE Matches Analysis Total",passed:!1,detail:`JE expense total ($${g.toLocaleString(void 0,{minimumFractionDigits:2})}) differs from PTO_Analysis Change total ($${y.toLocaleString(void 0,{minimumFractionDigits:2})}) by $${S.toLocaleString(void 0,{minimumFractionDigits:2})}`}):m.push({check:"JE Matches Analysis Total",passed:!0,detail:""}),{debitTotal:f,creditTotal:u,difference:b,lineAmountSum:l,jeChangeTotal:g,analysisChangeTotal:y,issues:m,validationRun:!0}});Object.assign(J,e,{lastError:null})}catch(e){console.warn("PTO JE summary:",e),J.lastError=(e==null?void 0:e.message)||"Unable to calculate journal totals.",J.debitTotal=null,J.creditTotal=null,J.difference=null,J.lineAmountSum=null,J.jeChangeTotal=null,J.analysisChangeTotal=null,J.issues=[],J.validationRun=!1}finally{J.loading=!1,oe()}}var da={"general & administrative":"64110","general and administrative":"64110","g&a":"64110","research & development":"62110","research and development":"62110","r&d":"62110",marketing:"61610","cogs onboarding":"53110","cogs prof. services":"56110","cogs professional services":"56110","sales & marketing":"61110","sales and marketing":"61110","cogs support":"52110","client success":"61811"},Xt="21540";async function ua(){if(!se()){Z("Excel is not available. Open this module inside Excel to create the journal entry.","info");return}ee(!0,"Creating PTO Journal Entry...");try{await Excel.run(async e=>{let t=[],n=e.workbook.tables.getItemOrNullObject(me[0]);if(n.load("isNullObject"),await e.sync(),n.isNullObject){let h=e.workbook.worksheets.getItemOrNullObject("SS_PF_Config");if(h.load("isNullObject"),await e.sync(),!h.isNullObject){let _=h.getUsedRangeOrNullObject();_.load("values"),await e.sync();let I=_.isNullObject?[]:_.values||[];t=I.length>1?I.slice(1):[]}}else{let h=n.getDataBodyRange();h.load("values"),await e.sync(),t=h.values||[]}let a=e.workbook.worksheets.getItemOrNullObject("PTO_Analysis");if(a.load("isNullObject"),await e.sync(),a.isNullObject)throw new Error("PTO_Analysis sheet not found. Please ensure the worksheet exists.");let o=a.getUsedRangeOrNullObject();o.load("values");let s=e.workbook.worksheets.getItemOrNullObject("SS_Chart_of_Accounts");s.load("isNullObject"),await e.sync();let r=[];if(!s.isNullObject){let h=s.getUsedRangeOrNullObject();h.load("values"),await e.sync(),r=h.isNullObject?[]:h.values||[]}let i=o.isNullObject?[]:o.values||[];if(!i.length||i.length<2)throw new Error("PTO_Analysis is empty or has no data rows. Run the analysis first (Step 4).");let d={};t.forEach(h=>{let _=String(h[1]||"").trim(),I=h[2];_&&(d[_]=I)}),(!d[w.journalEntryId]||!d[w.payrollDate])&&console.warn("[JE Draft] Missing config values - RefNumber:",d[w.journalEntryId],"TxnDate:",d[w.payrollDate]);let c=d[w.journalEntryId]||"",p=d[w.payrollDate]||"",f=d[w.accountingPeriod]||"",u="";if(p)try{let h;if(typeof p=="number"||/^\d{4,5}$/.test(String(p).trim())){let _=Number(p),I=new Date(1899,11,30);h=new Date(I.getTime()+_*24*60*60*1e3)}else h=new Date(p);if(!isNaN(h.getTime())&&h.getFullYear()>1970){let _=String(h.getMonth()+1).padStart(2,"0"),I=String(h.getDate()).padStart(2,"0"),N=h.getFullYear();u=`${_}/${I}/${N}`}else console.warn("[JE Draft] Date parsing resulted in invalid date:",p,"->",h),u=String(p)}catch(h){console.warn("[JE Draft] Could not parse TxnDate:",p,h),u=String(p)}let l=f?`${f} PTO Accrual`:"PTO Accrual",g={};if(r.length>1){let h=(r[0]||[]).map(N=>z(N)),_=$(h,["account number","accountnumber","account","acct"]),I=$(h,["account name","accountname","name","description"]);_!==-1&&I!==-1&&r.slice(1).forEach(N=>{let q=String(N[_]||"").trim(),de=String(N[I]||"").trim();q&&(g[q]=de)})}let y=(i[0]||[]).map(h=>z(h));console.log("[JE Draft] PTO_Analysis headers:",y),console.log("[JE Draft] PTO_Analysis row count:",i.length-1);let b=$(y,["department"]),m=$(y,["change"]);if(console.log("[JE Draft] Column indices - Department:",b,"Change:",m),b===-1||m===-1)throw new Error(`Could not find required columns in PTO_Analysis. Found headers: ${y.join(", ")}. Looking for "Department" (found: ${b!==-1}) and "Change" (found: ${m!==-1}).`);let S={},E=0,R=0,C=0;if(i.slice(1).forEach((h,_)=>{E++;let I=String(h[b]||"").trim(),N=h[m],q=Number(N)||0;if(_<3&&console.log(`[JE Draft] Row ${_+2}: Dept="${I}", Change raw="${N}", Change num=${q}`),!I){C++;return}if(q===0){R++;return}S[I]||(S[I]=0),S[I]+=q}),console.log(`[JE Draft] Data summary: ${E} rows, ${R} with zero change, ${C} missing dept`),console.log("[JE Draft] Department totals:",S),Object.keys(S).length===0){let h=`No journal entry lines could be created.
...
This action cannot be undone.`,{title:"Clear PTO Data",icon:"\u{1F5D1}\uFE0F",confirmText:"Clear Data",cancelText:"Keep Data",destructive:!0}))){ee(!0);try{await Excel.run(async t=>{let n=t.workbook.worksheets.getItem("PTO_Data"),a=n.getUsedRangeOrNullObject();a.load("rowCount"),await t.sync(),!a.isNullObject&&a.rowCount>1&&(n.getRangeByIndexes(1,0,a.rowCount-1,20).clear(Excel.ClearApplyTo.contents),await t.sync()),n.activate(),n.getRange("A1").select(),await t.sync()}),Z("PTO_Data cleared successfully. You can now paste new data.","success")}catch(t){console.error("Clear PTO_Data error:",t),Z(`Failed to clear PTO_Data: ${t.message}`,"error")}finally{ee(!1)}}}async function ga(){if(!se())return[];try{return await Excel.run(async e=>{let t=e.workbook.worksheets;return t.load("items/name,visibility"),await e.sync(),t.items.filter(a=>{let s=(a.name||"").toUpperCase();return s.startsWith("SS_")||s.includes("MAPPING")||s.includes("HOMEPAGE")}).map(a=>({name:a.name,visible:a.visibility===Excel.SheetVisibility.visible,isHomepage:(a.name||"").toUpperCase().includes("HOMEPAGE")})).sort((a,o)=>a.isHomepage&&!o.isHomepage?1:!a.isHomepage&&o.isHomepage?-1:a.name.localeCompare(o.name))})}catch(e){return console.error("[Config] Error reading configuration sheets:",e),[]}}function ma(){if(document.getElementById("config-sheet-modal"))return;let e=document.createElement("div");if(e.id="config-sheet-modal",e.className="pf-config-modal hidden",e.innerHTML=`
    ...
    `,document.body.appendChild(e),!document.getElementById("pf-config-modal-styles")){let t=document.createElement("style");t.id="pf-config-modal-styles",t.textContent=`
        ...
        `,document.head.appendChild(t)}}async function ha(){ma();let e=document.getElementById("config-sheet-modal"),t=document.getElementById("config-sheet-list");if(!e||!t)return;t.textContent="Loading\u2026",e.classList.remove("hidden");let n=await ga();n.length?(t.innerHTML="",n.forEach(a=>{let o=document.createElement("button");o.type="button",o.className="pf-config-sheet",o.innerHTML=`<span>${a.name}</span><span class="pf-config-pill">${a.visible?"Visible":"Hidden"}</span>`,o.addEventListener("click",async()=>{await ya(a.name),e.classList.add("hidden")}),t.appendChild(o)})):t.textContent="No configuration sheets found.",e.querySelectorAll("[data-close]").forEach(a=>a.addEventListener("click",()=>e.classList.add("hidden")))}async function ya(e){if(!(!e||!se()))try{await Excel.run(async t=>{let n=t.workbook.worksheets,a=n.getItemOrNullObject(e);a.load("isNullObject,visibility"),await t.sync(),a.isNullObject&&(a=n.add(e)),a.visibility=Excel.SheetVisibility.visible,await t.sync(),a.activate(),a.getRange("A1").select(),await t.sync(),console.log(`[Config] Opened sheet: ${e}`)})}catch(t){console.error("[Config] Error opening sheet",e,t)}}function le(e){var n,a;let t=String(e!=null?e:"").trim();return(a=(n=O.values)==null?void 0:n[t])!=null?a:""}function be(e){var n;if(e)return e;let t=le(w.reviewerName);if(t)return t;if((n=window.PrairieForge)!=null&&n._sharedConfigCache){let a=window.PrairieForge._sharedConfigCache.get("SS_Default_Reviewer")||window.PrairieForge._sharedConfigCache.get("Default_Reviewer");if(a)return a}return""}function re(e,t,n={}){var r;let a=String(e!=null?e:"").trim();if(!a)return;O.values[a]=t!=null?t:"";let o=(r=n.debounceMs)!=null?r:0;if(!o){let i=xe.get(a);i&&clearTimeout(i),xe.delete(a),_e(a,t!=null?t:"",me);return}xe.has(a)&&clearTimeout(xe.get(a));let s=setTimeout(()=>{xe.delete(a),_e(a,t!=null?t:"",me)},o);xe.set(a,s)}function z(e){return String(e!=null?e:"").trim().toLowerCase()}function ee(e,t="Working..."){let n=document.getElementById(Bn);n&&(n.style.display="none")}function lt(){Fn()}typeof Office!="undefined"&&Office.onReady?Office.onReady(()=>lt()).catch(()=>lt()):lt();function we(e){return O.steps[e]||{notes:"",reviewer:"",signOffDate:""}}function fn(e){return Ge[e]||{}}function va(e){return e===0?"config":e===1?"import":e===2?"headcount":e===3?"validate":e===4?"review":e===5?"journal":e===6?"archive":""}async function te(e,t,n){let a=O.steps[e]||{notes:"",reviewer:"",signOffDate:""};a[t]=n,O.steps[e]=a;let o=fn(e),s=t==="notes"?o.note:t==="reviewer"?o.reviewer:o.signOff;if(s&&K())try{await _e(s,n,me)}catch(r){console.warn("PTO: unable to save field",s,r)}}async function gn(e,t){O.permanents[e]=t;let n=fn(e);if(n!=null&&n.note&&K())try{await Excel.run(async a=>{var u;let o=a.workbook.tables.getItemOrNullObject(me[0]);if(await a.sync(),o.isNullObject)return;let s=o.getDataBodyRange(),r=o.getHeaderRowRange();s.load("values"),r.load("values"),await a.sync();let i=r.values[0]||[],d=i.map(l=>String(l||"").trim().toLowerCase()),c={field:d.findIndex(l=>l==="field"||l==="field name"||l==="setting"),permanent:d.findIndex(l=>l==="permanent"||l==="persist"),value:d.findIndex(l=>l==="value"||l==="setting value"),type:d.findIndex(l=>l==="type"||l==="category"),title:d.findIndex(l=>l==="title"||l==="display name")};if(c.field===-1)return;let f=(s.values||[]).findIndex(l=>String(l[c.field]||"").trim()===n.note);if(f>=0)c.permanent>=0&&(s.getCell(f,c.permanent).values=[[t?"Y":"N"]]);else{let l=new Array(i.length).fill("");c.type>=0&&(l[c.type]="Other"),c.title>=0&&(l[c.title]=""),l[c.field]=n.note,c.permanent>=0&&(l[c.permanent]=t?"Y":"N"),c.value>=0&&(l[c.value]=((u=O.steps[e])==null?void 0:u.notes)||""),o.rows.add(null,[l])}await a.sync()})}catch(a){console.warn("PTO: unable to update permanent flag",a)}}async function mn(e,t){let n=rn[e];if(n&&(O.completes[e]=t?"Y":"",!!K()))try{await _e(n,t?"Y":"",me)}catch(a){console.warn("PTO: unable to save completion flag",n,a)}}function Zt(e,t){e&&(e.classList.toggle("is-active",t),e.setAttribute("aria-pressed",String(t)))}function ba(){let e={};return Object.keys(Ge).forEach(t=>{var s;let n=parseInt(t,10),a=!!((s=O.steps[n])!=null&&s.signOffDate),o=!!O.completes[n];e[n]=a||o}),e}function hn(e,{buttonId:t,inputId:n,canActivate:a=null,onComplete:o=null}){var d;let s=document.getElementById(t);if(!s)return;let r=document.getElementById(n),i=!!((d=O.steps[e])!=null&&d.signOffDate)||!!O.completes[e];Zt(s,i),s.addEventListener("click",()=>{if(!s.classList.contains("is-active")&&e>0){let f=ba(),{canComplete:u,message:l}=Jt(e,f);if(!u){zt(l);return}}if(typeof a=="function"&&!a())return;let p=!s.classList.contains("is-active");Zt(s,p),r&&(r.value=p?ut():"",te(e,"signOffDate",r.value)),mn(e,p),p&&typeof o=="function"&&o()})}function v(e){return String(e!=null?e:"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function wa(e){return String(e!=null?e:"").replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function yn(e){let t=String(e!=null?e:"").trim().toLowerCase();return t==="true"||t==="y"||t==="yes"||t==="1"}function Se(e){let t=String(e!=null?e:"").trim().toLowerCase();return t==="true"||t==="y"||t==="yes"||t==="1"}function dt(e){if(!e)return null;let t=/^(\d{4})-(\d{2})-(\d{2})$/.exec(String(e));if(!t)return null;let n=Number(t[1]),a=Number(t[2]),o=Number(t[3]);return!n||!a||!o?null:{year:n,month:a,day:o}}function en(e){if(!e)return"";let t=dt(e);if(!t)return"";let{year:n,month:a,day:o}=t;return`${n}-${String(a).padStart(2,"0")}-${String(o).padStart(2,"0")}`}function ka(e){let t=dt(e);return t?`${["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"][t.month-1]} ${t.year}`:""}function Oa(e){let t=dt(e);return t?`PTO-AUTO-${t.year}-${String(t.month).padStart(2,"0")}-${String(t.day).padStart(2,"0")}`:""}function ut(){let e=new Date,t=e.getFullYear(),n=String(e.getMonth()+1).padStart(2,"0"),a=String(e.getDate()).padStart(2,"0");return`${t}-${n}-${a}`}function xa(e){let t=String(e!=null?e:"").trim().toLowerCase();return t==="y"||t==="yes"||t==="true"||t==="t"||t==="1"}function Sa(e){if(e instanceof Date)return e.getTime();if(typeof e=="number"){let n=Ea(e);return n?n.getTime():null}let t=new Date(e);return Number.isNaN(t.getTime())?null:t.getTime()}function Ea(e){if(!Number.isFinite(e))return null;let t=new Date(Date.UTC(1899,11,30));return new Date(t.getTime()+e*24*60*60*1e3)}function Ca(){let e=n=>{var a,o;return((o=(a=document.getElementById(n))==null?void 0:a.value)==null?void 0:o.trim())||""};[{id:"config-payroll-date",field:w.payrollDate},{id:"config-accounting-period",field:w.accountingPeriod},{id:"config-journal-id",field:w.journalEntryId},{id:"config-company-name",field:w.companyName},{id:"config-payroll-provider",field:w.payrollProvider},{id:"config-accounting-link",field:w.accountingSoftware},{id:"config-user-name",field:w.reviewerName}].forEach(({id:n,field:a})=>{let o=e(n);a&&re(a,o)})}function $(e,t=[]){let n=t.map(a=>z(a));return e.findIndex(a=>n.some(o=>a.includes(o)))}function _a(e){var E,R,C,j,L,B,W,F,P;let t=we(2),n=(t==null?void 0:t.notes)||"",a=!!O.permanents[2],o=be(t==null?void 0:t.reviewer),s=(t==null?void 0:t.signOffDate)||"",r=!!(Se(O.completes[2])||s),i=A.roster||{},d=A.hasAnalyzed,c=(R=(E=A.roster)==null?void 0:E.difference)!=null?R:0,p=!A.skipAnalysis&&Math.abs(c)>0,f=(C=i.rosterCount)!=null?C:0,u=(j=i.payrollCount)!=null?j:0,l=(L=i.difference)!=null?L:u-f,g=Array.isArray(i.mismatches)?i.mismatches.filter(Boolean):[],y="";A.loading?y=((W=(B=window.PrairieForge)==null?void 0:B.renderStatusBanner)==null?void 0:W.call(B,{type:"info",message:"Analyzing headcount\u2026",escapeHtml:v}))||"":A.lastError&&(y=((P=(F=window.PrairieForge)==null?void 0:F.renderStatusBanner)==null?void 0:P.call(F,{type:"error",message:A.lastError,escapeHtml:v}))||"");let b=(T,h,_,I)=>{let N=!d,q;N?q='<span class="pf-je-check-circle pf-je-circle--pending"></span>':I?q=`<span class="pf-je-check-circle pf-je-circle--pass">
    ...
    `,S=g.length&&!A.skipAnalysis&&d?window.PrairieForge.renderMismatchTiles({mismatches:g,label:"Employees Driving the Difference",sourceLabel:"Roster",targetLabel:"PTO Data",escapeHtml:v}):"";return`
        <section class="pf-hero" id="pf-step-hero">
            <p class="pf-hero-copy">${v(he)} | Step ${e.id}</p>
            <h2 class="pf-hero-title">${v(e.title)}</h2>
            <p class="pf-hero-copy">${v(e.summary||"")}</p>
    ...
    `}function Pa(){let e=X.completenessCheck||{},t=X.missingPayRates||[],n=[{key:"accrualRate",label:"Accrual Rate",desc:"\u2211 PTO_Data = \u2211 PTO_Analysis"},{key:"carryOver",label:"Carry Over",desc:"\u2211 PTO_Data = \u2211 PTO_Analysis"},{key:"ytdAccrued",label:"YTD Accrued",desc:"\u2211 PTO_Data = \u2211 PTO_Analysis"},{key:"ytdUsed",label:"YTD Used",desc:"\u2211 PTO_Data = \u2211 PTO_Analysis"},{key:"balance",label:"Balance",desc:"\u2211 PTO_Data = \u2211 PTO_Analysis"}],o=n.every(c=>e[c.key]!==null&&e[c.key]!==void 0)&&n.every(c=>{var p;return(p=e[c.key])==null?void 0:p.match}),s=t.length>0,r=c=>{let p=e[c.key],f=p==null,u;return f?u='<span class="pf-je-check-circle pf-je-circle--pending"></span>':p.match?u=`<span class="pf-je-check-circle pf-je-circle--pass">
            ...
            <div class="pf-je-check-row">
                ${u}
                <span class="pf-je-check-desc-pill">${v(c.label)}: ${v(c.desc)}</span>
            </div>
        `},i=n.map(c=>r(c)).join(""),d="";if(s){let c=t[0],p=t.length-1;d=`
            ...
            <div class="pf-readiness-issue">
                ...
                <p class="pf-readiness-issue-desc">
                    Enter hourly rate for <strong>${v(c.name)}</strong> to calculate liability
                </p>
    ...
    `}function Ta(e){var l,g,y,b,m,S,E,R;let t=we(3),n=!!O.permanents[3],a=be(t==null?void 0:t.reviewer),o=(t==null?void 0:t.signOffDate)||"",s=!!(Se(O.completes[3])||o),r=Y.hasRun,{balanceIssues:i,zeroBalances:d,accrualOutliers:c,totalEmployees:p}=Y,f="";if(Y.loading)f=((g=(l=window.PrairieForge)==null?void 0:l.renderStatusBanner)==null?void 0:g.call(l,{type:"info",message:"Analyzing data quality...",escapeHtml:v}))||"";else if(r){let C=i.length,j=c.length+d.length;C>0?f=((b=(y=window.PrairieForge)==null?void 0:y.renderStatusBanner)==null?void 0:b.call(y,{type:"error",title:`${C} Balance Issue${C>1?"s":""} Found`,message:"Review the issues below. Fix in PTO_Data and re-run, or acknowledge to continue.",escapeHtml:v}))||"":j>0?f=((S=(m=window.PrairieForge)==null?void 0:m.renderStatusBanner)==null?void 0:S.call(m,{type:"warning",title:"No Critical Issues",message:`${j} informational item${j>1?"s":""} to review (see below).`,escapeHtml:v}))||"":f=((R=(E=window.PrairieForge)==null?void 0:E.renderStatusBanner)==null?void 0:R.call(E,{type:"success",title:"Data Quality Passed",message:`${p} employee${p!==1?"s":""} checked \u2014 no anomalies found.`,escapeHtml:v}))||""}let u=[];return r&&i.length>0&&u.push(`
            <div class="pf-quality-issue pf-quality-issue--critical">
                ...
                <ul class="pf-quality-issue-list">
                    ${i.slice(0,5).map(C=>`<li><strong>${v(C.name)}</strong>: ${v(C.issue)}</li>`).join("")}
        ...
        `),r&&c.length>0&&u.push(`
            <div class="pf-quality-issue pf-quality-issue--warning">
                ...
                <ul class="pf-quality-issue-list">
                    ${c.slice(0,5).map(C=>`<li><strong>${v(C.name)}</strong>: ${C.accrualRate.toFixed(2)} hrs/period</li>`).join("")}
    ...
    `}function Ra(e){let t=we(5),n=!!O.permanents[5],a=be(t==null?void 0:t.reviewer),o=(t==null?void 0:t.signOffDate)||"",s=!!(Se(O.completes[5])||o),r=J.lastError?`<p class="pf-step-note">${v(J.lastError)}</p>`:"",i=J.validationRun,d=J.issues||[],c=[{key:"Debits = Credits",desc:"\u2211 Debit column = \u2211 Credit column"},{key:"Line Amounts Sum to Zero",desc:"\u2211 Line Amount = $0.00"},{key:"JE Matches Analysis Total",desc:"\u2211 Expense line amounts = \u2211 PTO_Analysis Change"}],p=g=>{let y=d.find(S=>S.check===g.key),b=!i,m;return b?m='<span class="pf-je-check-circle pf-je-circle--pending"></span>':y!=null&&y.passed?m=`<span class="pf-je-check-circle pf-je-circle--pass">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3"><polyline points="20 6 9 17 4 12"/></svg>
        ...
        `},f=c.map(g=>p(g)).join(""),u=d.filter(g=>!g.passed),l="";return i&&u.length>0&&(l=`
    ...
    `}function Aa(){var t,n;return Math.abs((n=(t=A.roster)==null?void 0:t.difference)!=null?n:0)>0}function vn(){return!A.skipAnalysis&&Aa()}async function ct(){if(!K()){A.loading=!1,A.lastError="Excel runtime is unavailable.",oe();return}A.loading=!0,A.lastError=null,ce(document.getElementById("headcount-save-btn"),!1),oe();try{let e=await Excel.run(async t=>{let n=t.workbook.worksheets.getItem("SS_Employee_Roster"),a=t.workbook.worksheets.getItem("PTO_Data"),o=t.workbook.worksheets.getItemOrNullObject("PTO_Analysis"),s=n.getUsedRangeOrNullObject(),r=a.getUsedRangeOrNullObject();s.load("values"),r.load("values"),o.load("isNullObject"),await t.sync();let i=null;o.isNullObject||(i=o.getUsedRangeOrNullObject(),i.load("values")),await t.sync();let d=s.isNullObject?[]:s.values||[],c=r.isNullObject?[]:r.values||[],p=i&&!i.isNullObject?i.values||[]:[],f=p.length?p:c;return Na(d,f)});A.roster=e.roster,A.hasAnalyzed=!0,A.lastError=null}catch(e){console.warn("PTO headcount: unable to analyze data",e),A.lastError="Unable to analyze headcount data. Try re-running the analysis."}finally{A.loading=!1,oe()}}function tn(e){if(!e)return!0;let t=e.toLowerCase().trim();return t?["total","subtotal","sum","count","grand","average","avg"].some(a=>t.includes(a)):!0}function Na(e,t){let n={rosterCount:0,payrollCount:0,difference:0,mismatches:[]};if(((e==null?void 0:e.length)||0)<2||((t==null?void 0:t.length)||0)<2)return console.warn("Headcount: insufficient data rows",{rosterRows:(e==null?void 0:e.length)||0,payrollRows:(t==null?void 0:t.length)||0}),{roster:n};let a=nn(e),o=nn(t),s=a.headers,r=o.headers,i={employee:an(s),termination:s.findIndex(l=>l.includes("termination"))},d={employee:an(r)};console.log("Headcount column detection:",{rosterEmployeeCol:i.employee,rosterTerminationCol:i.termination,payrollEmployeeCol:d.employee,rosterHeaders:s.slice(0,5),payrollHeaders:r.slice(0,5)});let c=new Set,p=new Set;for(let l=a.startIndex;l<e.length;l+=1){let g=e[l],y=i.employee>=0?ve(g[i.employee]):"";tn(y)||i.termination>=0&&ve(g[i.termination])||c.add(y.toLowerCase())}for(let l=o.startIndex;l<t.length;l+=1){let g=t[l],y=d.employee>=0?ve(g[d.employee]):"";tn(y)||p.add(y.toLowerCase())}n.rosterCount=c.size,n.payrollCount=p.size,n.difference=n.payrollCount-n.rosterCount,console.log("Headcount results:",{rosterCount:n.rosterCount,payrollCount:n.payrollCount,difference:n.difference});let f=[...c].filter(l=>!p.has(l)),u=[...p].filter(l=>!c.has(l));return n.mismatches=[...f.map(l=>`In roster, missing in PTO_Data: ${l}`),...u.map(l=>`In PTO_Data, missing in roster: ${l}`)],{roster:n}}function nn(e){if(!Array.isArray(e)||!e.length)return{headers:[],startIndex:1};let t=e.findIndex((o=[])=>o.some(s=>ve(s).toLowerCase().includes("employee"))),n=t===-1?0:t;return{headers:(e[n]||[]).map(o=>ve(o).toLowerCase()),startIndex:n+1}}function an(e=[]){let t=-1,n=-1;return e.forEach((a,o)=>{let s=a.toLowerCase();if(!s.includes("employee"))return;let r=1;s.includes("name")?r=4:s.includes("id")?r=2:r=3,r>n&&(n=r,t=o)}),t}function ve(e){return e==null?"":String(e).trim()}async function bn(e=null){let t=async n=>{let a=n.workbook.worksheets.getItem("PTO_Data"),o=n.workbook.worksheets.getItemOrNullObject("PTO_Analysis"),s=n.workbook.worksheets.getItemOrNullObject("SS_Employee_Roster"),r=n.workbook.worksheets.getItemOrNullObject("PR_Archive_Summary"),i=n.workbook.worksheets.getItemOrNullObject("PTO_Archive_Summary"),d=a.getUsedRangeOrNullObject();d.load("values"),o.load("isNullObject"),s.load("isNullObject"),r.load("isNullObject"),i.load("isNullObject"),await n.sync();let c=d.isNullObject?[]:d.values||[];if(!c.length)return;let p=(c[0]||[]).map(k=>z(k)),f=p.findIndex(k=>k.includes("employee")&&k.includes("name")),u=f>=0?f:0,l=$(p,["accrual rate"]),g=$(p,["carry over","carryover"]),y=p.findIndex(k=>k.includes("ytd")&&(k.includes("accrued")||k.includes("accrual"))),b=p.findIndex(k=>k.includes("ytd")&&k.includes("used")),m=$(p,["balance","current balance","pto balance"]);console.log("[PTO Analysis] PTO_Data headers:",p),console.log("[PTO Analysis] Column indices found:",{employee:u,accrualRate:l,carryOver:g,ytdAccrued:y,ytdUsed:b,balance:m}),b>=0?console.log(`[PTO Analysis] YTD Used column: "${p[b]}" at index ${b}`):console.warn("[PTO Analysis] YTD Used column NOT FOUND. Headers:",p);let S=c.slice(1).map(k=>ve(k[u])).filter(k=>k&&!k.toLowerCase().includes("total")),E=new Map;c.slice(1).forEach(k=>{let U=z(k[u]);!U||U.includes("total")||E.set(U,k)});let R=new Map;if(s.isNullObject)console.warn("[PTO Analysis] SS_Employee_Roster sheet not found");else{let k=s.getUsedRangeOrNullObject();k.load("values"),await n.sync();let U=k.isNullObject?[]:k.values||[];if(U.length){let M=(U[0]||[]).map(x=>z(x));console.log("[PTO Analysis] SS_Employee_Roster headers:",M);let V=M.findIndex(x=>x.includes("employee")&&x.includes("name"));V<0&&(V=M.findIndex(x=>x==="employee"||x==="name"||x==="full name"));let H=M.findIndex(x=>x.includes("department"));console.log(`[PTO Analysis] Roster column indices - Name: ${V}, Dept: ${H}`),V>=0&&H>=0?(U.slice(1).forEach(x=>{let ie=z(x[V]),ue=ve(x[H]);ie&&R.set(ie,ue)}),console.log(`[PTO Analysis] Built roster map with ${R.size} employees`)):console.warn("[PTO Analysis] Could not find Name or Department columns in SS_Employee_Roster")}}let C=new Map;if(!r.isNullObject){let k=r.getUsedRangeOrNullObject();k.load("values"),await n.sync();let U=k.isNullObject?[]:k.values||[];if(U.length){let M=(U[0]||[]).map(H=>z(H)),V={payrollDate:$(M,["payroll date"]),employee:$(M,["employee"]),category:$(M,["payroll category","category"]),amount:$(M,["amount","gross salary","gross_salary","earnings"])};V.employee>=0&&V.category>=0&&V.amount>=0&&U.slice(1).forEach(H=>{let x=z(H[V.employee]);if(!x)return;let ie=z(H[V.category]);if(!ie.includes("regular")||!ie.includes("earn"))return;let ue=Number(H[V.amount])||0;if(!ue)return;let Ee=Sa(H[V.payrollDate]),Ce=C.get(x);(!Ce||Ee!=null&&Ee>Ce.timestamp)&&C.set(x,{payRate:ue/80,timestamp:Ee})})}}let j=new Map;if(!i.isNullObject){let k=i.getUsedRangeOrNullObject();k.load("values"),await n.sync();let U=k.isNullObject?[]:k.values||[];if(U.length>1){let M=(U[0]||[]).map(x=>z(x)),V=M.findIndex(x=>x.includes("employee")&&x.includes("name")),H=$(M,["liability amount","liability","accrued pto"]);V>=0&&H>=0&&U.slice(1).forEach(x=>{let ie=z(x[V]);if(!ie)return;let ue=Number(x[H])||0;j.set(ie,ue)})}}let L=le(w.payrollDate)||"",B=[],W=[],F=S.map((k,U)=>{var gt,mt,ht,yt,vt,bt,wt;let M=z(k),V=R.get(M)||"",H=(mt=(gt=C.get(M))==null?void 0:gt.payRate)!=null?mt:"",x=E.get(M),ie=x&&l>=0&&(ht=x[l])!=null?ht:"",ue=x&&g>=0&&(yt=x[g])!=null?yt:"",Ee=x&&y>=0&&(vt=x[y])!=null?vt:"",Ce=x&&b>=0&&(bt=x[b])!=null?bt:"";(M.includes("avalos")||M.includes("sarah"))&&console.log(`[PTO Debug] ${k}:`,{ytdUsedIdx:b,rawValue:x?x[b]:"no dataRow",ytdUsed:Ce,fullRow:x});let Je=x&&m>=0&&Number(x[m])||0,pt=U+2;!H&&typeof H!="number"&&B.push({name:k,rowIndex:pt}),V||W.push({name:k,rowIndex:pt});let ze=typeof H=="number"&&Je?Je*H:0,ft=(wt=j.get(M))!=null?wt:0,wn=(typeof ze=="number"?ze:0)-ft;return[L,k,V,H,ie,ue,Ee,Ce,Je,ze,ft,wn]});X.missingPayRates=B.filter(k=>!X.ignoredMissingPayRates.has(k.name)),X.missingDepartments=W,console.log(`[PTO Analysis] Data quality: ${B.length} missing pay rates, ${W.length} missing departments`);let P=[["Analysis Date","Employee Name","Department","Pay Rate","Accrual Rate","Carry Over","YTD Accrued","YTD Used","Balance","Liability Amount","Accrued PTO $ [Prior Period]","Change"],...F],T=o.isNullObject?n.workbook.worksheets.add("PTO_Analysis"):o,h=T.getUsedRangeOrNullObject();h.load("address"),await n.sync(),h.isNullObject||h.clear();let _=P[0].length,I=P.length,N=F.length,q=T.getRangeByIndexes(0,0,I,_);q.values=P;let de=T.getRangeByIndexes(0,0,1,_);it(de),N>0&&(qt(T,0,N),ge(T,3,N),Oe(T,4,N),Oe(T,5,N),Oe(T,6,N),Oe(T,7,N),Oe(T,8,N),ge(T,9,N),ge(T,10,N),ge(T,11,N,!0)),q.format.autofitColumns(),T.getRange("A1").select(),await n.sync()};K()&&(e?await t(e):await Excel.run(t))}function Da(e=[]){return e.map(t=>(t||[]).map(n=>{if(n==null)return"";let a=String(n);return/[",\n]/.test(a)?`"${a.replace(/"/g,'""')}"`:a}).join(",")).join(`
`)}function $a(e,t){let n=new Blob([t],{type:"text/csv;charset=utf-8;"}),a=URL.createObjectURL(n),o=document.createElement("a");o.href=a,o.download=e,document.body.appendChild(o),o.click(),o.remove(),setTimeout(()=>URL.revokeObjectURL(a),1e3)}function on(){let e=document.getElementById("headcount-signoff-toggle");if(!e)return;let t=vn(),n=document.getElementById("step-notes-input"),a=(n==null?void 0:n.value.trim())||"";e.disabled=t&&!a;let o=document.getElementById("headcount-notes-hint");o&&(o.textContent=t?"Please document outstanding differences before signing off.":"")}function sn(){let e=document.getElementById("step-notes-input");if(!e)return;let t=e.value||"",n=t.startsWith(ye)?t.slice(ye.length).replace(/^\s+/,""):t.replace(new RegExp(`^${ye}\\s*`,"i"),"").trimStart(),a=ye+(n?`
