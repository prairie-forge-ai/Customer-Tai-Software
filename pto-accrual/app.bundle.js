...
`.trim(),hn={config:`
    ...
    `};function Tt(e){return e&&hn[e]||""}var We=`
    <svg
...
`.trim();function Ce(e){return e==null?"":String(e).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function H(e,t){return`
    ...
    `}function le({reviewerInputId:e,reviewerValue:t,signoffInputId:n,signoffValue:a,isComplete:o,saveButtonId:s,isSaved:r=!1,completeButtonId:c,subtext:d="Sign-off below. Click checkmark icon. Done."}){let l=`<button type="button" class="pf-action-toggle ${o?"is-active":""}" id="${c}" aria-pressed="${!!o}" title="Mark step complete">${Ee}</button>`;return`
    ...
    `}function Pn(){return`
        <section class="pf-step-guide">
            <div class="pf-step-grid">
                ${ae.map((e,t)=>Rn(e,t)).join("")}
            </div>
        </section>
    `}function Rn(e,t){let n=D.stepStatuses[e.id]||"pending",a=D.activeView==="step"&&D.focusedIndex===t?"pf-step-card--active":"",o=Tt(Jn(e.id));return`
    ...
    `}function Tn(e){let t=ae.filter(o=>o.id!==6).map(o=>({id:o.id,title:o.title,complete:Bn(o.id)})),n=t.every(o=>o.complete),a=t.map(o=>`
            <article class="pf-step-card pf-step-detail pf-config-card">
                <div class="pf-config-head pf-notes-header">
    ...
    `}function en(){if(!k.loaded)return`
        ...
        `;let e=zt(Z(O.payrollDate)),t=zt(Z(O.accountingPeriod)),n=Z(O.journalEntryId),a=Z(O.accountingSoftware),o=Z(O.payrollProvider),s=Z(O.companyName),r=Z(O.reviewerName),c=he(0),d=!!k.permanents[0],l=!!(rn(k.completes[0])||c.signOffDate),u=me(c==null?void 0:c.reviewer),f=(c==null?void 0:c.signOffDate)||"";return`
        ...
        <section class="pf-step-guide">
            ...
            ${le({reviewerInputId:"config-reviewer",reviewerValue:u,signoffInputId:"config-signoff-date",signoffValue:f,isComplete:l,saveButtonId:"config-signoff-save",completeButtonId:"config-signoff-toggle"})}
        </section>
    `}function xn(e){let t=he(1),n=!!k.permanents[1],a=me(t==null?void 0:t.reviewer),o=(t==null?void 0:t.signOffDate)||"",s=!!(we(k.completes[1])||o),r=Z(O.payrollProvider);return`
        <section class="pf-hero" id="pf-step-hero">
            <p class="pf-hero-copy">${w(ue)} | Step ${e.id}</p>
            <h2 class="pf-hero-title">${w(e.title)}</h2>
            <p class="pf-hero-copy">${w(e.summary||"")}</p>
        ...
        <section class="pf-step-guide">
            <article class="pf-step-card pf-step-detail pf-config-card">
                ...
                <div class="pf-signoff-action">
                    ${H(r?`<a href="${w(r)}" target="_blank" rel="noopener noreferrer" class="pf-action-toggle pf-clickable" title="Open payroll provider">${Ke}</a>`:`<button type="button" class="pf-action-toggle pf-clickable" id="import-provider-btn" disabled title="Add provider link in Configuration">${Ke}</button>`,"Provider")}
                    ${H(`<button type="button" class="pf-action-toggle pf-clickable" id="import-open-data-btn" title="Open PTO_Data sheet">${ze}</button>`,"PTO_Data")}
    ...
    `}function An(e){let t=ae.find(c=>c.id===e);if(!t)return"";if(e===0)return en();if(e===1)return xn(t);if(e===2)return ea(t);if(e===3)return na(t);if(e===4)return aa(t);if(e===5)return oa(t);if(t.id===6)return Tn(t);let n=he(e),a=!!k.permanents[e],o=me(n==null?void 0:n.reviewer),s=(n==null?void 0:n.signOffDate)||"",r=!!(we(k.completes[e])||s);return`
        <section class="pf-hero" id="pf-step-hero">
    ...
    `}function Nn(){var n,a,o,s,r,c;(n=document.getElementById("nav-home"))==null||n.addEventListener("click",async()=>{var l;let d=Ae(Pe);await xe(d.sheetName,d.title,d.subtitle),Re({activeView:"home",activeStepId:null}),(l=document.getElementById("pf-hero"))==null||l.scrollIntoView({behavior:"smooth",block:"start"})}),(a=document.getElementById("nav-selector"))==null||a.addEventListener("click",()=>{window.location.href=bn}),(o=document.getElementById("nav-prev"))==null||o.addEventListener("click",()=>Vt(-1)),(s=document.getElementById("nav-next"))==null||s.addEventListener("click",()=>Vt(1));let e=document.getElementById("nav-quick-toggle"),t=document.getElementById("quick-access-dropdown");e==null||e.addEventListener("click",d=>{d.stopPropagation(),t==null||t.classList.toggle("hidden"),e.classList.toggle("is-active")}),document.addEventListener("click",d=>{!(t!=null&&t.contains(d.target))&&!(e!=null&&e.contains(d.target))&&(t==null||t.classList.add("hidden"),e==null||e.classList.remove("is-active"))}),(r=document.getElementById("nav-roster"))==null||r.addEventListener("click",()=>{Jt("SS_Employee_Roster"),t==null||t.classList.add("hidden"),e==null||e.classList.remove("is-active")}),(c=document.getElementById("nav-accounts"))==null||c.addEventListener("click",()=>{Jt("SS_Chart_of_Accounts"),t==null||t.classList.add("hidden"),e==null||e.classList.remove("is-active")}),document.querySelectorAll("[data-step-card]").forEach(d=>{let l=Number(d.getAttribute("data-step-index")),u=Number(d.getAttribute("data-step-id"));d.addEventListener("click",()=>Le(l,u))}),D.activeView==="config"?$n():D.activeView==="step"&&D.activeStepId!=null&&Dn(D.activeStepId)}function Dn(e){var u,f,p,i,g,m,y,h,P,I,R,C,x,j,$,A,v;let t=e===2?document.getElementById("step-notes-input"):document.getElementById(`step-notes-${e}`),n=e===2?document.getElementById("step-reviewer-name"):document.getElementById(`step-reviewer-${e}`),a=e===2?document.getElementById("step-signoff-date"):document.getElementById(`step-signoff-${e}`),o=document.getElementById("step-back-btn"),s=e===2?document.getElementById("step-notes-lock-2"):document.getElementById(`step-notes-lock-${e}`),r=e===2?document.getElementById("step-notes-save-2"):document.getElementById(`step-notes-save-${e}`);r==null||r.addEventListener("click",async()=>{let b=(t==null?void 0:t.value)||"";await K(e,"notes",b),ne(r,!0)});let c=e===2?document.getElementById("headcount-signoff-save"):document.getElementById(`step-signoff-save-${e}`);c==null||c.addEventListener("click",async()=>{let b=(n==null?void 0:n.value)||"";await K(e,"reviewer",b),ne(c,!0)}),Xe();let d=e===2?"headcount-signoff-toggle":`step-signoff-toggle-${e}`,l=e===2?"step-signoff-date":`step-signoff-${e}`;sn(e,{buttonId:d,inputId:l,canActivate:e===2?()=>{var S;return!it()||((S=document.getElementById("step-notes-input"))==null?void 0:S.value.trim())||""?!0:(window.alert("Please enter a brief explanation of the headcount differences before completing this step."),!1)}:null,onComplete:e===2?da:null}),o==null||o.addEventListener("click",async()=>{let b=Ae(Pe);await xe(b.sheetName,b.title,b.subtitle),Re({activeView:"home",activeStepId:null})}),s==null||s.addEventListener("click",async()=>{let b=!s.classList.contains("is-locked");Qe(s,b),await an(e,b)}),e===6&&((u=document.getElementById("archive-run-btn"))==null||u.addEventListener("click",()=>{})),e===1&&((f=document.getElementById("import-open-data-btn"))==null||f.addEventListener("click",()=>je("PTO_Data")),(p=document.getElementById("import-clear-btn"))==null||p.addEventListener("click",()=>Gn())),e===2&&((i=document.getElementById("headcount-skip-btn"))==null||i.addEventListener("click",()=>{T.skipAnalysis=!T.skipAnalysis;let b=document.getElementById("headcount-skip-btn");b==null||b.classList.toggle("is-active",T.skipAnalysis),T.skipAnalysis&&Xt(),Qt()}),(g=document.getElementById("headcount-run-btn"))==null||g.addEventListener("click",()=>at()),(m=document.getElementById("headcount-refresh-btn"))==null||m.addEventListener("click",()=>at()),ca(),T.skipAnalysis&&Xt(),Qt()),e===3&&((y=document.getElementById("quality-run-btn"))==null||y.addEventListener("click",()=>Ut()),(h=document.getElementById("quality-refresh-btn"))==null||h.addEventListener("click",()=>Ut()),(P=document.getElementById("quality-acknowledge-btn"))==null||P.addEventListener("click",()=>Mn())),e===4&&((I=document.getElementById("analysis-refresh-btn"))==null||I.addEventListener("click",()=>Ft()),(R=document.getElementById("analysis-run-btn"))==null||R.addEventListener("click",()=>Ft()),(C=document.getElementById("payrate-save-btn"))==null||C.addEventListener("click",Ht),(x=document.getElementById("payrate-ignore-btn"))==null||x.addEventListener("click",Ln),(j=document.getElementById("payrate-input"))==null||j.addEventListener("keydown",b=>{b.key==="Enter"&&Ht()})),e===5&&(($=document.getElementById("je-create-btn"))==null||$.addEventListener("click",()=>Un()),(A=document.getElementById("je-run-btn"))==null||A.addEventListener("click",()=>tn()),(v=document.getElementById("je-export-btn"))==null||v.addEventListener("click",()=>Fn()))}function $n(){var d,l,u,f,p;let e=document.getElementById("config-payroll-date");e==null||e.addEventListener("change",i=>{let g=i.target.value||"";if(X(O.payrollDate,g),!!g){if(!k.overrides.accountingPeriod){let m=Wn(g);if(m){let y=document.getElementById("config-accounting-period");y&&(y.value=m),X(O.accountingPeriod,m)}}if(!k.overrides.journalId){let m=Yn(g);if(m){let y=document.getElementById("config-journal-id");y&&(y.value=m),X(O.journalEntryId,m)}}}});let t=document.getElementById("config-accounting-period");t==null||t.addEventListener("change",i=>{k.overrides.accountingPeriod=!!i.target.value,X(O.accountingPeriod,i.target.value||"")});let n=document.getElementById("config-journal-id");n==null||n.addEventListener("change",i=>{k.overrides.journalId=!!i.target.value,X(O.journalEntryId,i.target.value.trim())}),(d=document.getElementById("config-company-name"))==null||d.addEventListener("change",i=>{X(O.companyName,i.target.value.trim())}),(l=document.getElementById("config-payroll-provider"))==null||l.addEventListener("change",i=>{X(O.payrollProvider,i.target.value.trim())}),(u=document.getElementById("config-accounting-link"))==null||u.addEventListener("change",i=>{X(O.accountingSoftware,i.target.value.trim())}),(f=document.getElementById("config-user-name"))==null||f.addEventListener("change",i=>{X(O.reviewerName,i.target.value.trim())});let a=document.getElementById("config-notes");a==null||a.addEventListener("input",i=>{K(0,"notes",i.target.value)});let o=document.getElementById("config-notes-lock");o==null||o.addEventListener("click",async()=>{let i=!o.classList.contains("is-locked");Qe(o,i),await an(0,i)});let s=document.getElementById("config-notes-save");s==null||s.addEventListener("click",async()=>{a&&(await K(0,"notes",a.value),ne(s,!0))});let r=document.getElementById("config-reviewer");r==null||r.addEventListener("change",i=>{let g=i.target.value.trim();K(0,"reviewer",g);let m=document.getElementById("config-signoff-date");if(g&&m&&!m.value){let y=st();m.value=y,K(0,"signOffDate",y),on(0,!0)}}),(p=document.getElementById("config-signoff-date"))==null||p.addEventListener("change",i=>{K(0,"signOffDate",i.target.value||"")});let c=document.getElementById("config-signoff-save");c==null||c.addEventListener("click",async()=>{var m,y;let i=((m=r==null?void 0:r.value)==null?void 0:m.trim())||"",g=((y=document.getElementById("config-signoff-date"))==null?void 0:y.value)||"";await K(0,"reviewer",i),await K(0,"signOffDate",g),ne(c,!0)}),Xe(),sn(0,{buttonId:"config-signoff-toggle",inputId:"config-signoff-date",onComplete:Zn})}function Le(e,t=null){if(e<0||e>=ae.length)return;$e=e;let n=t!=null?t:ae[e].id;Re({focusedIndex:e,activeView:n===0?"config":"step",activeStepId:n}),n===1&&je("PTO_Data"),n===2&&!T.hasAnalyzed&&(ln(),at()),n===3&&je("PTO_Data"),n===5&&je("PTO_JE_Draft")}function Vt(e){let t=D.focusedIndex+e,n=Math.max(0,Math.min(ae.length-1,t));Le(n,ae[n].id)}function jn(){if($e===null)return;let e=document.querySelector(`[data-step-index="${$e}"]`);$e=null,e==null||e.scrollIntoView({behavior:"smooth",block:"center"})}function Bn(e){return rn(k.completes[e])}function Re(e){e.stepStatuses&&(D.stepStatuses={...D.stepStatuses,...e.stepStatuses}),Object.assign(D,{...e,stepStatuses:D.stepStatuses}),ee()}function oe(){return typeof Excel!="undefined"&&typeof Excel.run=="function"}async function Ht(){let e=document.getElementById("payrate-input");if(!e)return;let t=parseFloat(e.value),n=e.dataset.employee,a=parseInt(e.dataset.row,10);if(isNaN(t)||t<=0){window.alert("Please enter a valid pay rate greater than 0.");return}if(!n||isNaN(a)){console.error("Missing employee data on input");return}Y(!0,"Updating pay rate...");try{await Excel.run(async o=>{let s=o.workbook.worksheets.getItem("PTO_Analysis"),r=s.getCell(a-1,3);r.values=[[t]];let c=s.getCell(a-1,8);c.load("values"),await o.sync();let l=(Number(c.values[0][0])||0)*t,u=s.getCell(a-1,9);u.values=[[l]];let f=s.getCell(a-1,10);f.load("values"),await o.sync();let p=Number(f.values[0][0])||0,i=l-p,g=s.getCell(a-1,11);g.values=[[i]],await o.sync()}),W.missingPayRates=W.missingPayRates.filter(o=>o.name!==n),Y(!1),Le(3,3)}catch(o){console.error("Failed to save pay rate:",o),window.alert(`Failed to save pay rate: ${o.message}`),Y(!1)}}function Ln(){let e=document.getElementById("payrate-input");if(!e)return;let t=e.dataset.employee;t&&(W.ignoredMissingPayRates.add(t),W.missingPayRates=W.missingPayRates.filter(n=>n.name!==t)),Le(3,3)}async function Ut(){if(!oe()){window.alert("Excel is not available. Open this module inside Excel to run quality check.");return}J.loading=!0,Y(!0,"Analyzing data quality..."),ne(document.getElementById("quality-save-btn"),!1);try{await Excel.run(async t=>{var y;let a=t.workbook.worksheets.getItem("PTO_Data").getUsedRangeOrNullObject();a.load("values"),await t.sync();let o=a.isNullObject?[]:a.values||[];if(!o.length||o.length<2)throw new Error("PTO_Data is empty or has no data rows.");let s=(o[0]||[]).map(h=>F(h));console.log("[Data Quality] PTO_Data headers:",o[0]);let r=s.findIndex(h=>h==="employee name"||h==="employeename");r===-1&&(r=s.findIndex(h=>h.includes("employee")&&h.includes("name"))),r===-1&&(r=s.findIndex(h=>h==="name"||h.includes("name")&&!h.includes("company")&&!h.includes("form"))),console.log("[Data Quality] Employee name column index:",r,"Header:",(y=o[0])==null?void 0:y[r]);let c=N(s,["balance"]),d=N(s,["accrual rate","accrualrate"]),l=N(s,["carry over","carryover"]),u=N(s,["ytd accrued","ytdaccrued"]),f=N(s,["ytd used","ytdused"]),p=[],i=[],g=[],m=o.slice(1);m.forEach((h,P)=>{let I=P+2,R=r!==-1?String(h[r]||"").trim():`Row ${I}`;if(!R)return;let C=c!==-1&&Number(h[c])||0,x=d!==-1&&Number(h[d])||0,j=l!==-1&&Number(h[l])||0,$=u!==-1&&Number(h[u])||0,A=f!==-1&&Number(h[f])||0,v=j+$;C<0?p.push({name:R,issue:`Negative balance: ${C.toFixed(2)} hrs`,rowIndex:I}):A>v&&v>0&&p.push({name:R,issue:`Used ${A.toFixed(0)} hrs but only ${v.toFixed(0)} available`,rowIndex:I}),C===0&&(j>0||$>0)&&i.push({name:R,rowIndex:I}),x>8&&g.push({name:R,accrualRate:x,rowIndex:I})}),J.balanceIssues=p,J.zeroBalances=i,J.accrualOutliers=g,J.totalIssues=p.length,J.totalEmployees=m.filter(h=>h.some(P=>P!==null&&P!=="")).length,J.hasRun=!0});let e=J.balanceIssues.length>0;Re({stepStatuses:{3:e?"blocked":"complete"}})}catch(e){console.error("Data quality check error:",e),window.alert(`Quality check failed: ${e.message}`),J.hasRun=!1}finally{J.loading=!1,Y(!1),ee()}}function Mn(){J.acknowledged=!0,Re({stepStatuses:{3:"complete"}}),ee()}async function Vn(){if(oe())try{await Excel.run(async e=>{let t=e.workbook.worksheets.getItem("PTO_Data"),n=e.workbook.worksheets.getItemOrNullObject("PTO_Analysis"),a=t.getUsedRangeOrNullObject();if(a.load("values"),n.load("isNullObject"),await e.sync(),n.isNullObject){W.completenessCheck={accrualRate:null,carryOver:null,ytdAccrued:null,ytdUsed:null,balance:null};return}let o=n.getUsedRangeOrNullObject();o.load("values"),await e.sync();let s=a.isNullObject?[]:a.values||[],r=o.isNullObject?[]:o.values||[];if(!s.length||!r.length){W.completenessCheck={accrualRate:null,carryOver:null,ytdAccrued:null,ytdUsed:null,balance:null};return}let c=(u,f,p)=>{let i=(u[0]||[]).map(y=>F(y)),g=N(i,f);return g===-1?null:u.slice(1).reduce((y,h)=>y+(Number(h[g])||0),0)},d=[{key:"accrualRate",aliases:["accrual rate","accrualrate"]},{key:"carryOver",aliases:["carry over","carryover","carry_over"]},{key:"ytdAccrued",aliases:["ytd accrued","ytdaccrued","ytd_accrued"]},{key:"ytdUsed",aliases:["ytd used","ytdused","ytd_used"]},{key:"balance",aliases:["balance"]}],l={};for(let u of d){let f=c(s,u.aliases,"PTO_Data"),p=c(r,u.aliases,"PTO_Analysis");if(f===null||p===null)l[u.key]=null;else{let i=Math.abs(f-p)<.01;l[u.key]={match:i,ptoData:f,ptoAnalysis:p}}}W.completenessCheck=l})}catch(e){console.error("Completeness check failed:",e)}}async function Ft(){if(!oe()){window.alert("Excel is not available. Open this module inside Excel to run analysis.");return}Y(!0,"Running analysis...");try{await ln(),await Vn(),W.cleanDataReady=!0,ee()}catch(e){console.error("Full analysis error:",e),window.alert(`Analysis failed: ${e.message}`)}finally{Y(!1)}}async function tn(){if(!oe()){window.alert("Excel is not available. Open this module inside Excel to run journal checks.");return}U.loading=!0,U.lastError=null,ne(document.getElementById("je-save-btn"),!1),ee();try{let e=await Excel.run(async t=>{let a=t.workbook.worksheets.getItem("PTO_JE_Draft").getUsedRangeOrNullObject();a.load("values");let o=t.workbook.worksheets.getItemOrNullObject("PTO_Analysis");o.load("isNullObject"),await t.sync();let s=a.isNullObject?[]:a.values||[];if(!s.length)throw new Error("PTO_JE_Draft is empty. Generate the JE first.");let r=(s[0]||[]).map(I=>F(I)),c=N(r,["debit"]),d=N(r,["credit"]),l=N(r,["lineamount","line amount"]),u=N(r,["account number","accountnumber"]);if(c===-1||d===-1)throw new Error("Could not find Debit and Credit columns in PTO_JE_Draft.");let f=0,p=0,i=0,g=0;s.slice(1).forEach(I=>{let R=Number(I[c])||0,C=Number(I[d])||0,x=l!==-1&&Number(I[l])||0,j=u!==-1?String(I[u]||"").trim():"";f+=R,p+=C,i+=x,j&&j!=="21540"&&(g+=x)});let m=0;if(!o.isNullObject){let I=o.getUsedRangeOrNullObject();I.load("values"),await t.sync();let R=I.isNullObject?[]:I.values||[];if(R.length>1){let C=(R[0]||[]).map(j=>F(j)),x=N(C,["change"]);x!==-1&&R.slice(1).forEach(j=>{m+=Number(j[x])||0})}}let y=f-p,h=[];Math.abs(y)>=.01?h.push({check:"Debits = Credits",passed:!1,detail:y>0?`Debits exceed credits by $${Math.abs(y).toLocaleString(void 0,{minimumFractionDigits:2})}`:`Credits exceed debits by $${Math.abs(y).toLocaleString(void 0,{minimumFractionDigits:2})}`}):h.push({check:"Debits = Credits",passed:!0,detail:""}),Math.abs(i)>=.01?h.push({check:"Line Amounts Sum to Zero",passed:!1,detail:`Line amounts sum to $${i.toLocaleString(void 0,{minimumFractionDigits:2})} (should be $0.00)`}):h.push({check:"Line Amounts Sum to Zero",passed:!0,detail:""});let P=Math.abs(g-m);return P>=.01?h.push({check:"JE Matches Analysis Total",passed:!1,detail:`JE expense total ($${g.toLocaleString(void 0,{minimumFractionDigits:2})}) differs from PTO_Analysis Change total ($${m.toLocaleString(void 0,{minimumFractionDigits:2})}) by $${P.toLocaleString(void 0,{minimumFractionDigits:2})}`}):h.push({check:"JE Matches Analysis Total",passed:!0,detail:""}),{debitTotal:f,creditTotal:p,difference:y,lineAmountSum:i,jeChangeTotal:g,analysisChangeTotal:m,issues:h,validationRun:!0}});Object.assign(U,e,{lastError:null})}catch(e){console.warn("PTO JE summary:",e),U.lastError=(e==null?void 0:e.message)||"Unable to calculate journal totals.",U.debitTotal=null,U.creditTotal=null,U.difference=null,U.lineAmountSum=null,U.jeChangeTotal=null,U.analysisChangeTotal=null,U.issues=[],U.validationRun=!1}finally{U.loading=!1,ee()}}var Hn={"general & administrative":"64110","general and administrative":"64110","g&a":"64110","research & development":"62110","research and development":"62110","r&d":"62110",marketing:"61610","cogs onboarding":"53110","cogs prof. services":"56110","cogs professional services":"56110","sales & marketing":"61110","sales and marketing":"61110","cogs support":"52110","client success":"61811"},Gt="21540";async function Un(){if(!oe()){window.alert("Excel is not available. Open this module inside Excel to create the journal entry.");return}Y(!0,"Creating PTO Journal Entry...");try{await Excel.run(async e=>{let n=e.workbook.tables.getItem(de[0]).getDataBodyRange();n.load("values");let o=e.workbook.worksheets.getItem("PTO_Analysis").getUsedRangeOrNullObject();o.load("values");let s=e.workbook.worksheets.getItemOrNullObject("SS_Chart_of_Accounts");s.load("isNullObject"),await e.sync();let r=[];if(!s.isNullObject){let v=s.getUsedRangeOrNullObject();v.load("values"),await e.sync(),r=v.isNullObject?[]:v.values||[]}let c=n.values||[],d=o.isNullObject?[]:o.values||[];if(!d.length)throw new Error("PTO_Analysis is empty. Run the analysis first.");let l={};c.forEach(v=>{let b=String(v[1]||"").trim(),S=v[2];b&&(l[b]=S)}),(!l[O.journalEntryId]||!l[O.payrollDate])&&console.warn("[JE Draft] Missing config values - RefNumber:",l[O.journalEntryId],"TxnDate:",l[O.payrollDate]);let u=l[O.journalEntryId]||"",f=l[O.payrollDate]||"",p=l[O.accountingPeriod]||"",i="";if(f)try{let v;if(typeof f=="number"||/^\d{4,5}$/.test(String(f).trim())){let b=Number(f),S=new Date(1899,11,30);v=new Date(S.getTime()+b*24*60*60*1e3)}else v=new Date(f);if(!isNaN(v.getTime())&&v.getFullYear()>1970){let b=String(v.getMonth()+1).padStart(2,"0"),S=String(v.getDate()).padStart(2,"0"),z=v.getFullYear();i=`${b}/${S}/${z}`}else console.warn("[JE Draft] Date parsing resulted in invalid date:",f,"->",v),i=String(f)}catch(v){console.warn("[JE Draft] Could not parse TxnDate:",f,v),i=String(f)}let g=p?`${p} PTO Accrual`:"PTO Accrual",m={};if(r.length>1){let v=(r[0]||[]).map(z=>F(z)),b=N(v,["account number","accountnumber","account","acct"]),S=N(v,["account name","accountname","name","description"]);b!==-1&&S!==-1&&r.slice(1).forEach(z=>{let te=String(z[b]||"").trim(),se=String(z[S]||"").trim();te&&(m[te]=se)})}let y=(d[0]||[]).map(v=>F(v)),h=N(y,["department"]),P=N(y,["change"]);if(h===-1||P===-1)throw new Error("Could not find Department or Change columns in PTO_Analysis.");let I={};d.slice(1).forEach(v=>{let b=String(v[h]||"").trim(),S=Number(v[P])||0;b&&S!==0&&(I[b]||(I[b]=0),I[b]+=S)});let R=["RefNumber","TxnDate","Account Number","Account Name","LineAmount","Debit","Credit","LineDesc","Department"],C=[R],x=0,j=0;Object.entries(I).forEach(([v,b])=>{if(Math.abs(b)<.01)return;let S=v.toLowerCase().trim(),z=Hn[S]||"",te=m[z]||"",se=b>0?Math.abs(b):0,G=b<0?Math.abs(b):0;x+=se,j+=G,C.push([u,i,z,te,b,se,G,g,v])});let $=x-j;if(Math.abs($)>=.01){let v=$<0?Math.abs($):0,b=$>0?Math.abs($):0,S=m[Gt]||"Accrued PTO";C.push([u,i,Gt,S,-$,v,b,g,""])}let A=e.workbook.worksheets.getItemOrNullObject("PTO_JE_Draft");if(A.load("isNullObject"),await e.sync(),A.isNullObject)A=e.workbook.worksheets.add("PTO_JE_Draft");else{let v=A.getUsedRangeOrNullObject();v.load("isNullObject"),await e.sync(),v.isNullObject||v.clear()}if(C.length>0){let v=A.getRangeByIndexes(0,0,C.length,R.length);v.values=C;let b=A.getRangeByIndexes(0,0,1,R.length);et(b);let S=C.length-1;S>0&&(ce(A,4,S,!0),ce(A,5,S),ce(A,6,S)),v.format.autofitColumns()}await e.sync(),A.activate(),A.getRange("A1").select(),await e.sync()}),await tn()}catch(e){console.error("Create JE Draft error:",e),window.alert(`Unable to create Journal Entry: ${e.message}`)}finally{Y(!1)}}async function Fn(){if(!oe()){window.alert("Excel is not available. Open this module inside Excel to export.");return}Y(!0,"Preparing JE CSV...");try{let{rows:e}=await Excel.run(async n=>{let o=n.workbook.worksheets.getItem("PTO_JE_Draft").getUsedRangeOrNullObject();o.load("values"),await n.sync();let s=o.isNullObject?[]:o.values||[];if(!s.length)throw new Error("PTO_JE_Draft is empty.");return{rows:s}}),t=ra(e);la(`pto-je-draft-${st()}.csv`,t)}catch(e){console.error("PTO JE export:",e),window.alert("Unable to export the JE draft. Confirm the sheet has data.")}finally{Y(!1)}}async function je(e){if(!(!e||!oe()))try{await Excel.run(async t=>{let n=t.workbook.worksheets.getItem(e);n.activate(),n.getRange("A1").select(),await t.sync()})}catch(t){console.error(t)}}async function Gn(){if(!(!oe()||!window.confirm("This will clear all data in PTO_Data. Are you sure?"))){Y(!0);try{await Excel.run(async t=>{let n=t.workbook.worksheets.getItem("PTO_Data"),a=n.getUsedRangeOrNullObject();a.load("rowCount"),await t.sync(),!a.isNullObject&&a.rowCount>1&&(n.getRangeByIndexes(1,0,a.rowCount-1,20).clear(Excel.ClearApplyTo.contents),await t.sync()),n.activate(),n.getRange("A1").select(),await t.sync()}),window.alert("PTO_Data cleared successfully. You can now paste new data.")}catch(t){console.error("Clear PTO_Data error:",t),window.alert(`Failed to clear PTO_Data: ${t.message}`)}finally{Y(!1)}}}async function Jt(e){if(!e||!oe())return;let t={SS_Employee_Roster:["Employee","Department","Pay_Rate","Status","Hire_Date"],SS_Chart_of_Accounts:["Account_Number","Account_Name","Type","Category"]};try{await Excel.run(async n=>{let a=n.workbook.worksheets.getItemOrNullObject(e);if(a.load("isNullObject"),await n.sync(),a.isNullObject){a=n.workbook.worksheets.add(e);let o=t[e]||["Column1","Column2"],s=a.getRange(`A1:${String.fromCharCode(64+o.length)}1`);s.values=[o],s.format.font.bold=!0,s.format.fill.color="#f0f0f0",s.format.autofitColumns(),await n.sync()}a.activate(),a.getRange("A1").select(),await n.sync()})}catch(n){console.error("Error opening reference sheet:",n)}}function Z(e){var n,a;let t=String(e!=null?e:"").trim();return(a=(n=k.values)==null?void 0:n[t])!=null?a:""}function me(e){var n;if(e)return e;let t=Z(O.reviewerName);if(t)return t;if((n=window.PrairieForge)!=null&&n._sharedConfigCache){let a=window.PrairieForge._sharedConfigCache.get("SS_Default_Reviewer")||window.PrairieForge._sharedConfigCache.get("Default_Reviewer");if(a)return a}return""}function X(e,t,n={}){var r;let a=String(e!=null?e:"").trim();if(!a)return;k.values[a]=t!=null?t:"";let o=(r=n.debounceMs)!=null?r:0;if(!o){let c=be.get(a);c&&clearTimeout(c),be.delete(a),Se(a,t!=null?t:"",de);return}be.has(a)&&clearTimeout(be.get(a));let s=setTimeout(()=>{be.delete(a),Se(a,t!=null?t:"",de)},o);be.set(a,s)}function F(e){return String(e!=null?e:"").trim().toLowerCase()}function Y(e,t="Working..."){let n=document.getElementById(wn);n&&(n.style.display="none")}function nt(){kn()}typeof Office!="undefined"&&Office.onReady?Office.onReady(()=>nt()).catch(()=>nt()):nt();function he(e){return k.steps[e]||{notes:"",reviewer:"",signOffDate:""}}function nn(e){return Be[e]||{}}function Jn(e){return e===0?"config":e===1?"import":e===2?"headcount":e===3?"validate":e===4?"review":e===5?"journal":e===6?"archive":""}async function K(e,t,n){let a=k.steps[e]||{notes:"",reviewer:"",signOffDate:""};a[t]=n,k.steps[e]=a;let o=nn(e),s=t==="notes"?o.note:t==="reviewer"?o.reviewer:o.signOff;if(s&&q())try{await Se(s,n,de)}catch(r){console.warn("PTO: unable to save field",s,r)}}async function an(e,t){k.permanents[e]=t;let n=nn(e);if(n!=null&&n.note&&q())try{await Excel.run(async a=>{var p;let o=a.workbook.tables.getItemOrNullObject(de[0]);if(await a.sync(),o.isNullObject)return;let s=o.getDataBodyRange(),r=o.getHeaderRowRange();s.load("values"),r.load("values"),await a.sync();let c=r.values[0]||[],d=c.map(i=>String(i||"").trim().toLowerCase()),l={field:d.findIndex(i=>i==="field"||i==="field name"||i==="setting"),permanent:d.findIndex(i=>i==="permanent"||i==="persist"),value:d.findIndex(i=>i==="value"||i==="setting value"),type:d.findIndex(i=>i==="type"||i==="category"),title:d.findIndex(i=>i==="title"||i==="display name")};if(l.field===-1)return;let f=(s.values||[]).findIndex(i=>String(i[l.field]||"").trim()===n.note);if(f>=0)l.permanent>=0&&(s.getCell(f,l.permanent).values=[[t?"Y":"N"]]);else{let i=new Array(c.length).fill("");l.type>=0&&(i[l.type]="Other"),l.title>=0&&(i[l.title]=""),i[l.field]=n.note,l.permanent>=0&&(i[l.permanent]=t?"Y":"N"),l.value>=0&&(i[l.value]=((p=k.steps[e])==null?void 0:p.notes)||""),o.rows.add(null,[i])}await a.sync()})}catch(a){console.warn("PTO: unable to update permanent flag",a)}}async function on(e,t){let n=Zt[e];if(n&&(k.completes[e]=t?"Y":"",!!q()))try{await Se(n,t?"Y":"",de)}catch(a){console.warn("PTO: unable to save completion flag",n,a)}}function qt(e,t){e&&(e.classList.toggle("is-active",t),e.setAttribute("aria-pressed",String(t)))}function qn(){let e={};return Object.keys(Be).forEach(t=>{var s;let n=parseInt(t,10),a=!!((s=k.steps[n])!=null&&s.signOffDate),o=!!k.completes[n];e[n]=a||o}),e}function sn(e,{buttonId:t,inputId:n,canActivate:a=null,onComplete:o=null}){var d;let s=document.getElementById(t);if(!s)return;let r=document.getElementById(n),c=!!((d=k.steps[e])!=null&&d.signOffDate)||!!k.completes[e];qt(s,c),s.addEventListener("click",()=>{if(!s.classList.contains("is-active")&&e>0){let f=qn(),{canComplete:p,message:i}=Bt(e,f);if(!p){Lt(i);return}}if(typeof a=="function"&&!a())return;let u=!s.classList.contains("is-active");qt(s,u),r&&(r.value=u?st():"",K(e,"signOffDate",r.value)),on(e,u),u&&typeof o=="function"&&o()})}function w(e){return String(e!=null?e:"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function zn(e){return String(e!=null?e:"").replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function rn(e){let t=String(e!=null?e:"").trim().toLowerCase();return t==="true"||t==="y"||t==="yes"||t==="1"}function we(e){let t=String(e!=null?e:"").trim().toLowerCase();return t==="true"||t==="y"||t==="yes"||t==="1"}function ot(e){if(!e)return null;let t=/^(\d{4})-(\d{2})-(\d{2})$/.exec(String(e));if(!t)return null;let n=Number(t[1]),a=Number(t[2]),o=Number(t[3]);return!n||!a||!o?null:{year:n,month:a,day:o}}function zt(e){if(!e)return"";let t=ot(e);if(!t)return"";let{year:n,month:a,day:o}=t;return`${n}-${String(a).padStart(2,"0")}-${String(o).padStart(2,"0")}`}function Wn(e){let t=ot(e);return t?`${["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"][t.month-1]} ${t.year}`:""}function Yn(e){let t=ot(e);return t?`PTO-AUTO-${t.year}-${String(t.month).padStart(2,"0")}-${String(t.day).padStart(2,"0")}`:""}function st(){let e=new Date,t=e.getFullYear(),n=String(e.getMonth()+1).padStart(2,"0"),a=String(e.getDate()).padStart(2,"0");return`${t}-${n}-${a}`}function Kn(e){let t=String(e!=null?e:"").trim().toLowerCase();return t==="y"||t==="yes"||t==="true"||t==="t"||t==="1"}function Qn(e){if(e instanceof Date)return e.getTime();if(typeof e=="number"){let n=Xn(e);return n?n.getTime():null}let t=new Date(e);return Number.isNaN(t.getTime())?null:t.getTime()}function Xn(e){if(!Number.isFinite(e))return null;let t=new Date(Date.UTC(1899,11,30));return new Date(t.getTime()+e*24*60*60*1e3)}function Zn(){let e=n=>{var a,o;return((o=(a=document.getElementById(n))==null?void 0:a.value)==null?void 0:o.trim())||""};[{id:"config-payroll-date",field:O.payrollDate},{id:"config-accounting-period",field:O.accountingPeriod},{id:"config-journal-id",field:O.journalEntryId},{id:"config-company-name",field:O.companyName},{id:"config-payroll-provider",field:O.payrollProvider},{id:"config-accounting-link",field:O.accountingSoftware},{id:"config-user-name",field:O.reviewerName}].forEach(({id:n,field:a})=>{let o=e(n);a&&X(a,o)})}function N(e,t=[]){let n=t.map(a=>F(a));return e.findIndex(a=>n.some(o=>a.includes(o)))}function ea(e){var I,R,C,x,j,$,A,v,b;let t=he(2),n=(t==null?void 0:t.notes)||"",a=!!k.permanents[2],o=me(t==null?void 0:t.reviewer),s=(t==null?void 0:t.signOffDate)||"",r=!!(we(k.completes[2])||s),c=T.roster||{},d=T.hasAnalyzed,l=(R=(I=T.roster)==null?void 0:I.difference)!=null?R:0,u=!T.skipAnalysis&&Math.abs(l)>0,f=(C=c.rosterCount)!=null?C:0,p=(x=c.payrollCount)!=null?x:0,i=(j=c.difference)!=null?j:p-f,g=Array.isArray(c.mismatches)?c.mismatches.filter(Boolean):[],m="";T.loading?m=((A=($=window.PrairieForge)==null?void 0:$.renderStatusBanner)==null?void 0:A.call($,{type:"info",message:"Analyzing headcount\u2026",escapeHtml:w}))||"":T.lastError&&(m=((b=(v=window.PrairieForge)==null?void 0:v.renderStatusBanner)==null?void 0:b.call(v,{type:"error",message:T.lastError,escapeHtml:w}))||"");let y=(S,z,te,se)=>{let G=!d,fe;G?fe='<span class="pf-je-check-circle pf-je-circle--pending"></span>':se?fe=`<span class="pf-je-check-circle pf-je-circle--pass">
        ...
        ${y("Difference","Should be zero",i,i===0)}
    `,P=g.length&&!T.skipAnalysis&&d?window.PrairieForge.renderMismatchTiles({mismatches:g,label:"Employees Driving the Difference",sourceLabel:"Roster",targetLabel:"PTO Data",escapeHtml:w}):"";return`
        <section class="pf-hero" id="pf-step-hero">
            <p class="pf-hero-copy">${w(ue)} | Step ${e.id}</p>
            <h2 class="pf-hero-title">${w(e.title)}</h2>
            <p class="pf-hero-copy">${w(e.summary||"")}</p>
    ...
    `}function ta(){let e=W.completenessCheck||{},t=W.missingPayRates||[],n=[{key:"accrualRate",label:"Accrual Rate",desc:"\u2211 PTO_Data = \u2211 PTO_Analysis"},{key:"carryOver",label:"Carry Over",desc:"\u2211 PTO_Data = \u2211 PTO_Analysis"},{key:"ytdAccrued",label:"YTD Accrued",desc:"\u2211 PTO_Data = \u2211 PTO_Analysis"},{key:"ytdUsed",label:"YTD Used",desc:"\u2211 PTO_Data = \u2211 PTO_Analysis"},{key:"balance",label:"Balance",desc:"\u2211 PTO_Data = \u2211 PTO_Analysis"}],o=n.every(l=>e[l.key]!==null&&e[l.key]!==void 0)&&n.every(l=>{var u;return(u=e[l.key])==null?void 0:u.match}),s=t.length>0,r=l=>{let u=e[l.key],f=u==null,p;return f?p='<span class="pf-je-check-circle pf-je-circle--pending"></span>':u.match?p=`<span class="pf-je-check-circle pf-je-circle--pass">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3"><polyline points="20 6 9 17 4 12"/></svg>
            ...
            <div class="pf-je-check-row">
                ${p}
                <span class="pf-je-check-desc-pill">${w(l.label)}: ${w(l.desc)}</span>
            </div>
        `},c=n.map(l=>r(l)).join(""),d="";if(s){let l=t[0],u=t.length-1;d=`
            ...
            <div class="pf-readiness-issue">
                ...
                <p class="pf-readiness-issue-desc">
                    Enter hourly rate for <strong>${w(l.name)}</strong> to calculate liability
                </p>
    ...
    `}function na(e){var i,g,m,y,h,P,I,R;let t=he(3),n=!!k.permanents[3],a=me(t==null?void 0:t.reviewer),o=(t==null?void 0:t.signOffDate)||"",s=!!(we(k.completes[3])||o),r=J.hasRun,{balanceIssues:c,zeroBalances:d,accrualOutliers:l,totalEmployees:u}=J,f="";if(J.loading)f=((g=(i=window.PrairieForge)==null?void 0:i.renderStatusBanner)==null?void 0:g.call(i,{type:"info",message:"Analyzing data quality...",escapeHtml:w}))||"";else if(r){let C=c.length,x=l.length+d.length;C>0?f=((y=(m=window.PrairieForge)==null?void 0:m.renderStatusBanner)==null?void 0:y.call(m,{type:"error",title:`${C} Balance Issue${C>1?"s":""} Found`,message:"Review the issues below. Fix in PTO_Data and re-run, or acknowledge to continue.",escapeHtml:w}))||"":x>0?f=((P=(h=window.PrairieForge)==null?void 0:h.renderStatusBanner)==null?void 0:P.call(h,{type:"warning",title:"No Critical Issues",message:`${x} informational item${x>1?"s":""} to review (see below).`,escapeHtml:w}))||"":f=((R=(I=window.PrairieForge)==null?void 0:I.renderStatusBanner)==null?void 0:R.call(I,{type:"success",title:"Data Quality Passed",message:`${u} employee${u!==1?"s":""} checked \u2014 no anomalies found.`,escapeHtml:w}))||""}let p=[];return r&&c.length>0&&p.push(`
        ...
        `),r&&l.length>0&&p.push(`
            <div class="pf-quality-issue pf-quality-issue--warning">
                ...
                <ul class="pf-quality-issue-list">
                    ${l.slice(0,5).map(C=>`<li><strong>${w(C.name)}</strong>: ${C.accrualRate.toFixed(2)} hrs/period</li>`).join("")}
    ...
    `}function aa(e){let t=he(4),n=!!k.permanents[4],a=me(t==null?void 0:t.reviewer),o=(t==null?void 0:t.signOffDate)||"",s=!!(we(k.completes[4])||o);return`
        <section class="pf-hero" id="pf-step-hero">
            <p class="pf-hero-copy">${w(ue)} | Step ${e.id}</p>
            <h2 class="pf-hero-title">${w(e.title)}</h2>
            <p class="pf-hero-copy">${w(e.summary||"")}</p>
    ...
    `}function oa(e){let t=he(5),n=!!k.permanents[5],a=me(t==null?void 0:t.reviewer),o=(t==null?void 0:t.signOffDate)||"",s=!!(we(k.completes[5])||o),r=U.lastError?`<p class="pf-step-note">${w(U.lastError)}</p>`:"",c=U.validationRun,d=U.issues||[],l=[{key:"Debits = Credits",desc:"\u2211 Debit column = \u2211 Credit column"},{key:"Line Amounts Sum to Zero",desc:"\u2211 Line Amount = $0.00"},{key:"JE Matches Analysis Total",desc:"\u2211 Expense line amounts = \u2211 PTO_Analysis Change"}],u=g=>{let m=d.find(P=>P.check===g.key),y=!c,h;return y?h='<span class="pf-je-check-circle pf-je-circle--pending"></span>':m!=null&&m.passed?h=`<span class="pf-je-check-circle pf-je-circle--pass">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3"><polyline points="20 6 9 17 4 12"/></svg>
        ...
        `},f=l.map(g=>u(g)).join(""),p=d.filter(g=>!g.passed),i="";return c&&p.length>0&&(i=`
        ...
        <section class="pf-step-guide">
            <article class="pf-step-card pf-step-detail pf-config-card">
                ...
                <div class="pf-signoff-action">
                    ${H(`<button type="button" class="pf-action-toggle pf-clickable" id="je-create-btn" title="Generate journal entry from PTO_Analysis">${ze}</button>`,"Generate")}
    ...
    `}function sa(){var t,n;return Math.abs((n=(t=T.roster)==null?void 0:t.difference)!=null?n:0)>0}function it(){return!T.skipAnalysis&&sa()}async function at(){if(!q()){T.loading=!1,T.lastError="Excel runtime is unavailable.",ee();return}T.loading=!0,T.lastError=null,ne(document.getElementById("headcount-save-btn"),!1),ee();try{let e=await Excel.run(async t=>{let n=t.workbook.worksheets.getItem("SS_Employee_Roster"),a=t.workbook.worksheets.getItem("PTO_Data"),o=t.workbook.worksheets.getItemOrNullObject("PTO_Analysis"),s=n.getUsedRangeOrNullObject(),r=a.getUsedRangeOrNullObject();s.load("values"),r.load("values"),o.load("isNullObject"),await t.sync();let c=null;o.isNullObject||(c=o.getUsedRangeOrNullObject(),c.load("values")),await t.sync();let d=s.isNullObject?[]:s.values||[],l=r.isNullObject?[]:r.values||[],u=c&&!c.isNullObject?c.values||[]:[],f=u.length?u:l;return ia(d,f)});T.roster=e.roster,T.hasAnalyzed=!0,T.lastError=null}catch(e){console.warn("PTO headcount: unable to analyze data",e),T.lastError="Unable to analyze headcount data. Try re-running the analysis."}finally{T.loading=!1,ee()}}function Wt(e){if(!e)return!0;let t=e.toLowerCase().trim();return t?["total","subtotal","sum","count","grand","average","avg"].some(a=>t.includes(a)):!0}function ia(e,t){let n={rosterCount:0,payrollCount:0,difference:0,mismatches:[]};if(((e==null?void 0:e.length)||0)<2||((t==null?void 0:t.length)||0)<2)return console.warn("Headcount: insufficient data rows",{rosterRows:(e==null?void 0:e.length)||0,payrollRows:(t==null?void 0:t.length)||0}),{roster:n};let a=Yt(e),o=Yt(t),s=a.headers,r=o.headers,c={employee:Kt(s),termination:s.findIndex(i=>i.includes("termination"))},d={employee:Kt(r)};console.log("Headcount column detection:",{rosterEmployeeCol:c.employee,rosterTerminationCol:c.termination,payrollEmployeeCol:d.employee,rosterHeaders:s.slice(0,5),payrollHeaders:r.slice(0,5)});let l=new Set,u=new Set;for(let i=a.startIndex;i<e.length;i+=1){let g=e[i],m=c.employee>=0?ge(g[c.employee]):"";Wt(m)||c.termination>=0&&ge(g[c.termination])||l.add(m.toLowerCase())}for(let i=o.startIndex;i<t.length;i+=1){let g=t[i],m=d.employee>=0?ge(g[d.employee]):"";Wt(m)||u.add(m.toLowerCase())}n.rosterCount=l.size,n.payrollCount=u.size,n.difference=n.payrollCount-n.rosterCount,console.log("Headcount results:",{rosterCount:n.rosterCount,payrollCount:n.payrollCount,difference:n.difference});let f=[...l].filter(i=>!u.has(i)),p=[...u].filter(i=>!l.has(i));return n.mismatches=[...f.map(i=>`In roster, missing in PTO_Data: ${i}`),...p.map(i=>`In PTO_Data, missing in roster: ${i}`)],{roster:n}}function Yt(e){if(!Array.isArray(e)||!e.length)return{headers:[],startIndex:1};let t=e.findIndex((o=[])=>o.some(s=>ge(s).toLowerCase().includes("employee"))),n=t===-1?0:t;return{headers:(e[n]||[]).map(o=>ge(o).toLowerCase()),startIndex:n+1}}function Kt(e=[]){let t=-1,n=-1;return e.forEach((a,o)=>{let s=a.toLowerCase();if(!s.includes("employee"))return;let r=1;s.includes("name")?r=4:s.includes("id")?r=2:r=3,r>n&&(n=r,t=o)}),t}function ge(e){return e==null?"":String(e).trim()}async function ln(e=null){let t=async n=>{let a=n.workbook.worksheets.getItem("PTO_Data"),o=n.workbook.worksheets.getItemOrNullObject("PTO_Analysis"),s=n.workbook.worksheets.getItemOrNullObject("SS_Employee_Roster"),r=n.workbook.worksheets.getItemOrNullObject("PR_Archive_Summary"),c=n.workbook.worksheets.getItemOrNullObject("PTO_Archive_Summary"),d=a.getUsedRangeOrNullObject();d.load("values"),o.load("isNullObject"),s.load("isNullObject"),r.load("isNullObject"),c.load("isNullObject"),await n.sync();let l=d.isNullObject?[]:d.values||[];if(!l.length)return;let u=(l[0]||[]).map(E=>F(E)),f=u.findIndex(E=>E.includes("employee")&&E.includes("name")),p=f>=0?f:0,i=N(u,["accrual rate"]),g=N(u,["carry over","carryover"]),m=u.findIndex(E=>E.includes("ytd")&&(E.includes("accrued")||E.includes("accrual"))),y=u.findIndex(E=>E.includes("ytd")&&E.includes("used")),h=N(u,["balance","current balance","pto balance"]);console.log("[PTO Analysis] PTO_Data headers:",u),console.log("[PTO Analysis] Column indices found:",{employee:p,accrualRate:i,carryOver:g,ytdAccrued:m,ytdUsed:y,balance:h}),y>=0?console.log(`[PTO Analysis] YTD Used column: "${u[y]}" at index ${y}`):console.warn("[PTO Analysis] YTD Used column NOT FOUND. Headers:",u);let P=l.slice(1).map(E=>ge(E[p])).filter(E=>E&&!E.toLowerCase().includes("total")),I=new Map;l.slice(1).forEach(E=>{let V=F(E[p]);!V||V.includes("total")||I.set(V,E)});let R=new Map;if(s.isNullObject)console.warn("[PTO Analysis] SS_Employee_Roster sheet not found");else{let E=s.getUsedRangeOrNullObject();E.load("values"),await n.sync();let V=E.isNullObject?[]:E.values||[];if(V.length){let B=(V[0]||[]).map(_=>F(_));console.log("[PTO Analysis] SS_Employee_Roster headers:",B);let L=B.findIndex(_=>_.includes("employee")&&_.includes("name"));L<0&&(L=B.findIndex(_=>_==="employee"||_==="name"||_==="full name"));let M=B.findIndex(_=>_.includes("department"));console.log(`[PTO Analysis] Roster column indices - Name: ${L}, Dept: ${M}`),L>=0&&M>=0?(V.slice(1).forEach(_=>{let Q=F(_[L]),ie=ge(_[M]);Q&&R.set(Q,ie)}),console.log(`[PTO Analysis] Built roster map with ${R.size} employees`)):console.warn("[PTO Analysis] Could not find Name or Department columns in SS_Employee_Roster")}}let C=new Map;if(!r.isNullObject){let E=r.getUsedRangeOrNullObject();E.load("values"),await n.sync();let V=E.isNullObject?[]:E.values||[];if(V.length){let B=(V[0]||[]).map(M=>F(M)),L={payrollDate:N(B,["payroll date"]),employee:N(B,["employee"]),category:N(B,["payroll category","category"]),amount:N(B,["amount","gross salary","gross_salary","earnings"])};L.employee>=0&&L.category>=0&&L.amount>=0&&V.slice(1).forEach(M=>{let _=F(M[L.employee]);if(!_)return;let Q=F(M[L.category]);if(!Q.includes("regular")||!Q.includes("earn"))return;let ie=Number(M[L.amount])||0;if(!ie)return;let Oe=Qn(M[L.payrollDate]),ke=C.get(_);(!ke||Oe!=null&&Oe>ke.timestamp)&&C.set(_,{payRate:ie/80,timestamp:Oe})})}}let x=new Map;if(!c.isNullObject){let E=c.getUsedRangeOrNullObject();E.load("values"),await n.sync();let V=E.isNullObject?[]:E.values||[];if(V.length>1){let B=(V[0]||[]).map(_=>F(_)),L=B.findIndex(_=>_.includes("employee")&&_.includes("name")),M=N(B,["liability amount","liability","accrued pto"]);L>=0&&M>=0&&V.slice(1).forEach(_=>{let Q=F(_[L]);if(!Q)return;let ie=Number(_[M])||0;x.set(Q,ie)})}}let j=Z(O.payrollDate)||"",$=[],A=[],v=P.map((E,V)=>{var ct,dt,ut,ft,pt,gt,mt;let B=F(E),L=R.get(B)||"",M=(dt=(ct=C.get(B))==null?void 0:ct.payRate)!=null?dt:"",_=I.get(B),Q=_&&i>=0&&(ut=_[i])!=null?ut:"",ie=_&&g>=0&&(ft=_[g])!=null?ft:"",Oe=_&&m>=0&&(pt=_[m])!=null?pt:"",ke=_&&y>=0&&(gt=_[y])!=null?gt:"";(B.includes("avalos")||B.includes("sarah"))&&console.log(`[PTO Debug] ${E}:`,{ytdUsedIdx:y,rawValue:_?_[y]:"no dataRow",ytdUsed:ke,fullRow:_});let Ve=_&&h>=0&&Number(_[h])||0,rt=V+2;!M&&typeof M!="number"&&$.push({name:E,rowIndex:rt}),L||A.push({name:E,rowIndex:rt});let He=typeof M=="number"&&Ve?Ve*M:0,lt=(mt=x.get(B))!=null?mt:0,cn=(typeof He=="number"?He:0)-lt;return[j,E,L,M,Q,ie,Oe,ke,Ve,He,lt,cn]});W.missingPayRates=$.filter(E=>!W.ignoredMissingPayRates.has(E.name)),W.missingDepartments=A,console.log(`[PTO Analysis] Data quality: ${$.length} missing pay rates, ${A.length} missing departments`);let b=[["Analysis Date","Employee Name","Department","Pay Rate","Accrual Rate","Carry Over","YTD Accrued","YTD Used","Balance","Liability Amount","Accrued PTO $ [Prior Period]","Change"],...v],S=o.isNullObject?n.workbook.worksheets.add("PTO_Analysis"):o,z=S.getUsedRangeOrNullObject();z.load("address"),await n.sync(),z.isNullObject||z.clear();let te=b[0].length,se=b.length,G=v.length,fe=S.getRangeByIndexes(0,0,se,te);fe.values=b;let Me=S.getRangeByIndexes(0,0,1,te);et(Me),G>0&&(Mt(S,0,G),ce(S,3,G),ve(S,4,G),ve(S,5,G),ve(S,6,G),ve(S,7,G),ve(S,8,G),ce(S,9,G),ce(S,10,G),ce(S,11,G,!0)),fe.format.autofitColumns(),S.getRange("A1").select(),await n.sync()};q()&&(e?await t(e):await Excel.run(t))}function ra(e=[]){return e.map(t=>(t||[]).map(n=>{if(n==null)return"";let a=String(n);return/[",\n]/.test(a)?`"${a.replace(/"/g,'""')}"`:a}).join(",")).join(`
`)}function la(e,t){let n=new Blob([t],{type:"text/csv;charset=utf-8;"}),a=URL.createObjectURL(n),o=document.createElement("a");o.href=a,o.download=e,document.body.appendChild(o),o.click(),o.remove(),setTimeout(()=>URL.revokeObjectURL(a),1e3)}function Qt(){let e=document.getElementById("headcount-signoff-toggle");if(!e)return;let t=it(),n=document.getElementById("step-notes-input"),a=(n==null?void 0:n.value.trim())||"";e.disabled=t&&!a;let o=document.getElementById("headcount-notes-hint");o&&(o.textContent=t?"Please document outstanding differences before signing off.":"")}function Xt(){let e=document.getElementById("step-notes-input");if(!e)return;let t=e.value||"",n=t.startsWith(pe)?t.slice(pe.length).replace(/^\s+/,""):t.replace(new RegExp(`^${pe}\\s*`,"i"),"").trimStart(),a=pe+(n?`
${n}`:"");e.value!==a&&(e.value=a),K(2,"notes",e.value)}function ca(){let e=document.getElementById("step-notes-input");e&&e.addEventListener("input",()=>{if(!T.skipAnalysis)return;let t=e.value||"";if(!t.startsWith(pe)){let n=t.replace(pe,"").trimStart();e.value=pe+(n?`
${n}`:"")}K(2,"notes",e.value)})}function da(){var n;let e=it(),t=((n=document.getElementById("step-notes-input"))==null?void 0:n.value.trim())||"";if(e&&!t){window.alert("Please enter a brief explanation of the outstanding differences before completing this step.");return}}})();
