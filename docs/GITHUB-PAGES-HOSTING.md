...
The production manifest (`manifest.xml`) already points at `https://prairie-forge-ai.github.io/Customer-ArchCollins-Foundry/module-selector/index.html`, so no edits are required unless you move the site. If Excel caches the add-in, bump `<Version>` in the manifest and re-upload it.

## 4. Test in Excel

1. Insert → Office Add-ins → Upload My Add-in → select `manifest.xml`.
