...
`.trim();function Qe(e){return e==null?"":String(e).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function Wt(e,t){return`
            ...
            <div class="pf-complete-row ${D}">
                ...
                <div class="pf-complete-values">
                    ...
                    <div class="pf-complete-value-row">
                        ...
                        <span class="pf-complete-amount">${n(I)}</span>
                    </div>
                </div>
                ${P}
                `}
    ...
    `}function qo(){if(!Y.loaded)return`
        ...
        `;let e=Pe[0],t=be(Dt()),n=be(N("PR_Accounting_Period")),o=N("PR_Journal_Entry_ID"),a=N("SS_Accounting_Software"),s=an(),l=N("SS_Company_Name"),c=N(tt)||$e(),r=e?N(e.note):"",i=e?xe(e.note):!1,f=(e?N(e.reviewer):"")||$e(),d=e?be(N(e.signOff)):"",u=!!(d||N(de[0]));return`
    ...
    `}function Ko(e){let t=_e(1),n=t?xe(t.note):!1,o=t?N(t.note):"",a=(t?N(t.reviewer):"")||$e(),s=t?be(N(t.signOff)):"",l=!!(s||N(de[1])),c=an();return`
    ...
    `}function Xo(e){var H,Q,W,F,q,ne,ke,oe,Z,ee,ie,ae,te;let t=_e(2),n=t?N(t.note):"",o=t?xe(t.note):!1,a=(t?N(t.reviewer):"")||$e(),s=t?be(N(t.signOff)):"",l=!!(s||N(de[2])),c=Pt(),r=z.roster||{},i=z.departments||{},f=z.hasAnalyzed,d="";z.loading?d='<p class="pf-step-note">Analyzing roster and payroll data\u2026</p>':z.lastError&&(d=`<p class="pf-step-note">${x(z.lastError)}</p>`);let u=(ge,rt,$t)=>{let Oe=!f,Te;Oe?Te='<span class="pf-je-check-circle pf-je-circle--pending"></span>':$t?Te=`<span class="pf-je-check-circle pf-je-circle--pass">
        ...
        ${u("Difference",p,p===0)}
    `,g=(q=i.rosterCount)!=null?q:0,E=(ne=r.rosterCount)!=null?ne:0,_=(ke=r.payrollCount)!=null?ke:0,I=f&&g>0&&g<Math.max(E,_)?`<p class="pf-step-note pf-step-note--info">\u2139\uFE0F Only ${g} employees appear in both lists, so only those can be compared for department alignment.</p>`:"",M=`
    ...
    `}function Zo(e){let t=_e(4),n=t?N(t.note):"",o=(t?N(t.reviewer):"")||$e(),a=t?be(N(t.signOff)):"",s=!!(a||N(de[4])),l=Se.loading?'<p class="pf-step-note">Preparing executive summary\u2026</p>':Se.lastError?`<p class="pf-step-note">${x(Se.lastError)}</p>`:"",c=mn({id:"expense-review-copilot",body:"Want help analyzing your data? Just ask!",placeholder:"Where should I focus this pay period?",buttonLabel:"Ask CoPilot"});return`
        <section class="pf-hero" id="pf-step-hero">
            <p class="pf-hero-copy">${x(Ie)} | Step ${e.id}</p>
            <h2 class="pf-hero-title">${x(e.title)}</h2>
    ...
    `}function ea(e){var A,m,D;let t=_e(5),n=t?N(t.note):"",o=t?xe(t.note):!1,a=(t?N(t.reviewer):"")||$e(),s=t?be(N(t.signOff)):"",l=!!(s||N(de[5])),c=j.lastError?`<p class="pf-step-note">${x(j.lastError)}</p>`:"",r=j.debitTotal!==null,i=(A=j.debitTotal)!=null?A:0,f=(m=j.creditTotal)!=null?m:0,d=i-f,u=(D=j.cleanTotal)!=null?D:0,p=j.cleanTotal!==null,h=r,v=r&&p&&Math.abs(d-u)<.01,w=(P,H)=>{let Q=!r,W;return Q?W='<span class="pf-je-check-circle pf-je-circle--pending"></span>':H?W=`<span class="pf-je-check-circle pf-je-circle--pass">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3"><polyline points="20 6 9 17 4 12"/></svg>
        ...
        `},y=r?le(i):"\u2014",g=r?le(f):"\u2014",E=r?le(d):"\u2014",_=p?le(u):"\u2014",I=p?Math.abs(d-u):0,M=I>=.01?` (diff: ${le(I)})`:"",S=`
        ${w(`Total Debits = ${y}`,h)}
        ${w(`Total Credits = ${g}`,h)}
    ...
    `}function na(e){let t=pt.find(_=>_.id===e)||{id:e!=null?e:"-",title:"Workflow Step",summary:"",description:"",checklist:[]};if(e===1)return Ko(t);if(e===2)return Xo(t);if(e===3)return Qo(t);if(e===4)return Zo(t);if(e===5)return ea(t);if(e===6)return ta(t);let n=!1,o=_e(e),a=o?N(o.note):"",s=o?xe(o.note):!1,l=(o?N(o.reviewer):"")||$e(),c=o?be(N(o.signOff)):"",r=o&&de[e]?!!(c||N(de[e])):!!c,i=(t.highlights||[]).map(_=>`
...
Continue?`)){console.log("[Archive] User cancelled");return}console.log("[Archive] User confirmed, starting archive process...");try{if(console.log("[Archive] Step 1: Creating archive workbook..."),!await Va()){console.log("[Archive] Archive cancelled or failed");return}console.log("[Archive] Step 1 complete: Archive workbook created/user confirmed backup"),console.log("[Archive] Step 2: Updating PR_Archive_Summary..."),await ja(),console.log("[Archive] Step 2 complete: Archive summary updated"),console.log("[Archive] Step 3: Clearing working data..."),await Ha(),console.log("[Archive] Step 3 complete: Working data cleared"),console.log("[Archive] Step 4: Clearing non-permanent notes..."),await Ua(),console.log("[Archive] Step 4 complete: Notes cleared"),console.log("[Archive] Step 5: Resetting config values..."),await Ga(),console.log("[Archive] Step 5 complete: Config reset"),console.log("[Archive] Archive workflow complete!"),await eo(),fe(),window.alert(`\u2705 Archive Complete!
...
Please check the console for details and verify your data.`)}}async function Va(){try{let t=`Payroll_Archive_${Dt()||new Date().toISOString().split("T")[0]}`;return console.log("[Archive] Creating archive copy of workbook..."),await Excel.run(async n=>{let a=n.workbook.worksheets;a.load("items/name"),await n.sync();let s=[$.DATA,$.DATA_CLEAN,$.EXPENSE_MAPPING,$.EXPENSE_REVIEW,$.JE_DRAFT],l=[];for(let c of s){let r=a.items.find(f=>f.name===c);if(!r){console.log(`[Archive] Sheet not found: ${c}`);continue}let i=r.getUsedRangeOrNullObject();i.load("values,rowCount,columnCount"),await n.sync(),!i.isNullObject&&i.values&&i.values.length>0&&(l.push({name:c,values:i.values,rowCount:i.rowCount,columnCount:i.columnCount}),console.log(`[Archive] Collected data from: ${c} (${i.values.length} rows)`))}return l.length===0?(window.alert("No data to archive. Please complete the payroll workflow first."),!1):(console.log("[Archive] Creating new workbook..."),n.application.createWorkbook(),await n.sync(),window.alert(`\u{1F4C1} Archive Workbook Created
...
Click OK to continue clearing this workbook for the next period.`),!0)})}catch(e){return console.error("[Archive] Error creating archive workbook:",e),window.alert(`Archive Error
