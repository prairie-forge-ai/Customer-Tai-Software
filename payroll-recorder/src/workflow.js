...
async function clearPrDataSheet() {
    ...
    const confirmed = await showConfirm(
        "All data in PR_Data will be permanently removed.\n\nThis action cannot be undone.",
        {
...
async function createArchiveWorkbook() {
    try {
        ...
        return await Excel.run(async (context) => {
            ...
            summaryRows.push([]);
            const cfgFields = STEP_CONFIG_FIELDS[0];
            const reviewFields = STEP_CONFIG_FIELDS[4];
            const jeFields = STEP_CONFIG_FIELDS[5];
